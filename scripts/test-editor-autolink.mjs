...
event('paste', { clipboardData: { files: [], getData: type => type === 'text/plain' ? url : '' } });
node.data = url; caret = selectionEnd = node.data.length;
event('input', { inputType: 'insertText', data: null });
assert.equal(offered()[0].href, url, 'Browser command insertion can report null data; exact inserted text still establishes provenance');
reset();
const multiline = 'intro\n'+url+'\ntail';
event('paste', { clipboardData: { files: [], getData: type => type === 'text/plain' ? multiline : '' } });
node.data = multiline; caret = selectionEnd = node.data.length;
event('input', { inputType: 'insertText', data: 'tail' });
assert.equal(offered()[0].href, url, 'Multiline browser paste reports only its last paragraph as data; verify full clipboard against DOM');
reset();
event('paste', { clipboardData: { files: [], getData: type => type === 'text/plain' ? multiline : '' } });
node.data = multiline; caret = selectionEnd = node.data.length;
event('input', { inputType: '', data: null });
assert.equal(offered()[0].href, url, 'Rich HTML browser insertion may report empty inputType; clipboard and inserted DOM establish provenance');
reset(url);
event('paste', { clipboardData: { files: [], getData: type => type === 'text/plain' ? url : '' } });
insert(' ');
