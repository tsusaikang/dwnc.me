...
async function runUploadBrowserTests(){const results=[],check=(condition,message)=>{if(!condition)throw new Error(message)};
  ...
  const output=await normalizeUploadImage(source),bitmap=await createImageBitmap(output);check(Math.max(bitmap.width,bitmap.height)===2560,'2560px edge limit not applied');check(Math.abs(bitmap.width/bitmap.height-1.5)<0.001,'aspect ratio changed');results.push({name:'24MP',type:output.type,inputBytes:source.size,outputBytes:output.size,width:bitmap.width,height:bitmap.height});bitmap.close();
