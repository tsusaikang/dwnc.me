...
const newestSlots = article('.post-sequence').children();
assert.equal(newestSlots.length, 2); assert.equal(newestSlots.eq(0).is('span'), true);
assert.equal(newestSlots.eq(1).attr('rel'), 'prev');
const oldestResponse = await worker(new Request('https://dwnc.me/posts/1?preview=1'), { NATIVE_DB: db }, {});
const oldest = load(await oldestResponse.text()), oldestSlots = oldest('.post-sequence').children();
assert.equal(oldestSlots.length, 2); assert.equal(oldestSlots.eq(0).attr('rel'), 'next');
assert.equal(oldestSlots.eq(1).is('span'), true);
const middlePath = newestSlots.eq(1).attr('href');
const middleResponse = await worker(new Request(`https://dwnc.me${middlePath}?preview=1`), { NATIVE_DB: db }, {});
const middle = load(await middleResponse.text());
