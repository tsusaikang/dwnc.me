...
function closeTokenWriteDescriptor(stream) {
  ...
  try {
    ...
    else if (typeof stream.end === 'function' && !stream.writableEnded) stream.end();
...
export async function runCloudflareStagingControl({
  ...
  const wrangler = assertPinnedWranglerDescriptor(await requireWrangler(root), root);
  try {
    auth = await createAuth();
    const sealedWrangler = assertSealedWranglerDescriptor(
    ...
    const preflightSha256 = sha256(canonicalJson({ ...preWhoamiReceipt, whoami }));
    ...
    try {
      ...
      if (execution.timedOut || execution.value.type === 'spawn-error') {
        await terminateChild(child, lifecycle, tokenPipe, {
          terminationTimeoutMs: childTerminationTimeoutMs,
          forceKillTimeoutMs: childForceKillTimeoutMs,
        });
