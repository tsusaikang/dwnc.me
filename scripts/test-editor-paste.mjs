...
let selectionRange=makeRange(), uploads=[],saved=[],messages=[],htmlRequests=0,failUpload=false,failFlush=false,failHtml=false,failNormalize=false,normalized=0;
...
const ctx=createContext({document:doc,window:win,crypto:webcrypto,Uint8Array,Array,Set,JSON,encodeURIComponent,console,normalizeUploadImage:async file=>{normalized++;if(failNormalize){failNormalize=false;throw new Error('synthetic compression failed')}return{...file,type:'image/jpeg',name:file.name.replace(/\.png$/,'.jpg'),size:2,arrayBuffer:async()=>new Uint8Array([9,8]).buffer}},$:id=>fields[id],status:text=>messages.push(text),bodyRange:()=>selectionRange,clearMediaSelection(){},captureEditorBefore(){},rememberEditorChange(){historyChanges++},closeFormatPanels(){},renderSaveState(){},setTimeout(){},clearTimeout(){},api:async(path,options)=>{
 ...
 assert.equal(path,'/posts/synthetic-post/media');assert.equal(options.method,'POST');assert.equal(options.headers['content-type'],'image/jpeg');assert.equal(Number(options.headers['x-dwnc-file-size']),options.body.byteLength);assert.equal(options.headers['x-dwnc-file-name'].endsWith('.jpg'),true);assert.deepEqual(Array.from(options.body),[9,8]);assert.equal(options.headers['x-dwnc-file-sha256'],Array.from(new Uint8Array(await webcrypto.subtle.digest('SHA-256',Uint8Array.from([9,8])))).map(b=>b.toString(16).padStart(2,'0')).join(''));
...
reset();failNormalize=true;await paste({files:[photo]});assert.equal(uploads.length,0);assert.equal(runInContext('uploadQueue.length',ctx),1);assert.equal(fields.bodyHtml.children.length,2);await runInContext('action(retryAction)',ctx);assert.equal(uploads.length,1);
