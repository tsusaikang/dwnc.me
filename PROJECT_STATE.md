...
최종 갱신: 2026-09-07 KST — 티스토리형 작성 화면 로컬 구현·시험·내장 브라우저 확인 완료 / 새 화면 운영 적용 승인 대기
...
- 현재 운영은 아래 좁은 목록 이동 수정 버전 그대로다. 이번 화면 개편은 로컬 구현·시험·브라우저 확인을 완료했으며 실제 글·사진·DB·공개 프로그램·DNS·route 변경과 Git push는 금지한다. 새 개편의 외부 활성화는 확인 가능한 로컬 결과를 완성한 뒤 변경 직전에 승인받는다. 후속 공개 글 화면 통합·대표이미지 연결보다 이번 사용자 요청을 먼저 처리한다.
...
- 최종 관리자 번들: `/var/folders/ps/kbhx2vz12m76shzc_s9wn30w0000gn/T/dwnc-admin-layout-bundle-7VFZ8D/bundle/admin-worker.js`, 1,590,073바이트, SHA-256 `527c6c3ba0d4b3d202cc42baeaf2d681b1d932e39937c7ffb0e99735f4a25cb8`. 인증·업로드·운영 변경은 없다.
...
- 다음 단계: 사용자가 새 작성 화면의 실제 서비스 적용을 승인하면 이 소스의 관리자 단일 프로그램만 적용한다. 기존 공개 Worker/DB/R2/DNS/route/글 내용은 바꾸지 않는다. 운영에서도 목록·편집 이동/서식 도구/저장 안내를 읽기로 확인한 후 인증 메모리 세션 종료와 공식 상태 갱신까지 마친다. `DWNC-CORE-004`는 로컬 완료·운영 적용 대기로 `in-progress`를 유지한다.
...
비개발자용 현재 요약: 티스토리를 참고한 새 작성 화면은 로컬 구현과 시험을 완료했고 운영 적용 승인만 남았다. 현재 서비스는 기존 관리자 화면을 유지한다. 공개 블로그와 이메일 OTP 관리자, 작업본 자동저장·명시적 공개 반영과 실패·재로그인·충돌 처리는 운영 중이다. 기본 서식 도구도 실제 관리자에 적용해 문단·크기·강조·색·정렬·목록·인용·링크·표를 지정할 수 있다. 기존 글 서식 보존과 저장·미리보기·공개·재열기는 로컬 합성 시험을 통과했고 운영에서는 기존 글과 이미지·서식 도구·공개 화면을 읽기로 확인했다. 실제 글과 사진은 변경하지 않았다. 다음은 새 작성 화면의 운영 적용 승인과 반영이며, 이후 기존·신규 공개 글 화면 통합과 대표이미지 연결이다.
...
- 작업본 자동저장·명시적 공개 반영은 운영에 적용됐고 해당 최우선 작업의 장애물은 없다. 기본 서식 기능과 좁은 글 목록/편집 이동 보완까지 운영 적용했다. 이전 운영 화면은 실제 700×720에서 확인했고, 새 로컬 작성 화면은 1280/700/390px에서 확인을 완료했다. 새 작성 화면의 운영 적용은 승인 대기다. 공개 화면 통합·대표이미지 연결과 아래 후속 문제도 남아 있다. 실제 글의 수정·발행·삭제는 운영 확인에 사용하지 않았다.
...
2. `PLAN-09`의 `DWNC-CORE-002`는 로컬 구현·실행 시험·빈 작업본 테이블 추가·관리자 운영 활성화·실제 화면 읽기 확인까지 완료했다. 이어 요청받은 `DWNC-CORE-003` 기본 서식도 로컬 구현·시험과 승인된 형식 열 추가 → 공개 Worker → 관리자 Worker 적용, 자체 내장 브라우저 확인까지 완료했다. 로그인 동의 차단은 해소됐고 인증 메모리 세션도 종료했다. 바로 다음은 `DWNC-CORE-004` 새 작성 화면의 운영 적용 승인을 받은 뒤 관리자에 반영하는 것이며, 이후 공개 글 화면 통합과 대표이미지 연결을 진행한다. 앞선 작업본 CMS 적용에서 공개 Worker·DNS·route·기존 글·R2 객체와 Git 원격은 변경하지 않았다. 운영에서 열어 둔 이전 관리자 화면은 미저장 입력을 처리한 뒤 새로 열어야 새 UI를 사용한다.
