...
- Cloudflare 계정 선택 안전장치: **계정 번호를 사진 업로드용·검사용 열쇠와 분리해 macOS 안전 보관함에서 읽는 기능과 복구 절차를 구현했다. 계정 번호는 로그인 열쇠가 아니라 올바른 계정을 고르는 확인 표지로만 쓰며, 원래 번호를 파일·명령어·화면 출력에 남기지 않는다. 로컬 시험은 통과했고 실제 계정 번호 저장과 Cloudflare 조회는 아직 0회다.**
...
- Cloudflare Stage 3 deny-only Worker 로컬 검사: **생성 전후 account `workers.dev=dwnc`, 생성 뒤 공개·미리보기 주소 꺼짐, 전체 로그·trace 설정, `content/v2`의 정확히 한 module 바이트·SHA-256, deploy 가능한 version 정확히 1개와 deployment 정확히 1개·100%, 연속 두 현재 상태의 일치를 함께 강제한다. 자동 집계한 전체 Cloudflare 모의시험 26개·8,641 assertions PASS(상태·원본 검증 3,718, 중단 복구 1,023, 실제 runner→복구 자식 139, 실제 실행 흐름 1,819, 전용 관리 열쇠 378) / 실제 Cloudflare 조회·생성·배포·DNS 변경·Keychain·clipboard 접근 0 / 최소 권한 열쇠 전달과 생성 중단 복구의 로컬 안전장치 완료**
...
- Cloudflare 계정 번호는 전용 macOS Keychain 항목 `me.dwnc.cloudflare-account-target.v1` / `dwnc:staging:account-id`에만 저장하도록 분리했다. 값은 버전이 표시된 고정 형식으로 보관하고, repository 밖 mode 700 상위 폴더의 mode 600 확인 파일에는 원래 번호가 아닌 account fingerprint·용도·생성 시각만 create-only로 기록한다. 관리 명령과 읽기 runner는 기본·명시 확인 파일의 정규화 경로와 실제 경로를 먼저 검사해 repository 내부와 ancestor symlink를 정책·Keychain을 읽거나 clipboard 내용을 읽기 전에 거부한다. 실패 정리 과정에서 clipboard를 비우는 호출은 가능하지만 내용을 읽지는 않는다. 초기화는 실제 macOS 클립보드를 정확히 한 번 읽고 바로 비운 다음 tracked staging account fingerprint와 일치할 때만 Keychain 항목을 새로 만든다. 기존 항목·확인 파일·symlink·hardlink·잘못된 권한·파일 경합은 덮어쓰기나 삭제 없이 중단한다. Keychain 저장 뒤 확인 파일이 비거나 일부만 남은 경우에는 실패 파일을 보존하고, 새 클립보드 입력 없이 `cloudflare:account-target:recover`가 primary 이름에서 파생한 exact `-recovery.json` 하나만 create-only로 만들어 이후 loader가 사용한다. 외부 호출자가 다른 recovery 경로를 넣으면 Keychain을 읽기 전에 거부한다. 원래 파일과 recovery 파일이 모두 불완전하면 새 이름을 자동 생성하지 않고 기존 파일과 Keychain 항목을 보존한 채 운영자 판단이 필요한 오류로 중단한다. `cloudflare:account-target:inspect`는 원래 번호를 출력하지 않고 일치 여부만 보여 준다.
- `cloudflare:r2:exposure:fetch`는 부모 작업의 `CLOUDFLARE_ACCOUNT_ID` 입력을 더 이상 받지 않는다. 위 전용 항목을 한 번 읽어 올바른 staging account인지 확인한 뒤에만 계정 번호를 실제 조회 자식 작업에 전달한다. 사진 업로드용·검사용 Keychain 항목은 읽지 않는다. 짧은 수명의 Cloudflare API token은 macOS clipboard에서 한 번 읽고 즉시 비운 뒤 framed anonymous FD 3으로 자식 작업에만 전달한다. credential decoder는 1회만 호출하고 FD 내부에서는 max+1 고정 buffer로 EOF까지 반복 read해 delayed multi-chunk는 허용하되 EOF 전 조기 성공, trailing·length·digest·oversize를 거부한다. token을 환경변수·argv·디스크·Keychain에 두지 않으며 legacy token과 부모 account 환경변수 주입은 첫 요청 전에 거부한다. exact source commit/tree/clean 상태를 요청 전·세 GET 뒤·receipt 직전에 확인하고, bucket→managed domain→custom domains의 재시도 없는 GET 3회 외 모든 method/path를 거부한다. 응답은 최대 1MiB+1 경계까지만 읽고, raw body는 저장소 밖 mode 700 parent에 create-only mode 600으로만 기록한다. capture만 남은 경우 `cloudflare:r2:exposure:recover`가 새 열쇠나 API 호출 없이 확인해 누락 evidence만 만든다. 새 account-target 123·control-plane 81·exposure 145 assertions를 통과했고 실제 browser/API/Keychain/clipboard/network 접근과 원격 변경은 0이다.
...
- 비공개 설정 확인을 위해 만들었던 읽기 전용 API token 두 개는 안전한 전달에 실패해 API GET 0·capture 0 상태로 각각 폐기했다. token 값이나 raw account ID는 문서에 기록하지 않았다. 새 전달 방식은 계정 번호와 짧은 읽기 전용 열쇠를 분리하고 각각 클립보드에서 한 번만 읽도록 로컬 구현·시험을 마쳤다. 실제 계정 번호 저장, 새 token 생성과 API 조회는 아직 0회다.
