...
최종 갱신: 2026-09-07 KST — 작업본 CMS 운영 중 / 기본 서식 로컬 구현·시험·브라우저 확인 완료, 운영 적용 승인 대기
...
- 로컬 구현·통상 시험·내장 브라우저 확인과 최종 빌드를 완료했다. 메인은 범위 통합·내장 브라우저·공식 상태를 맡고 UI, 저장/서식 처리, 시험은 하위 에이전트에 분리했다. 기존 운영 버전은 그대로다. 이번 새 기능의 외부 활성화 직전 승인이 남았다. Git push·실제 글 변경/삭제·DNS/route 변경·R2 덮어쓰기/삭제는 하지 않는다.
...
- 운영 적용 최소 순서는 형식 열 추가 → 공개 Worker → 관리자 Worker다. 구 공개 Worker는 native 이미지 참조를 Markdown에서 찾으므로 새 HTML 본문 이미지를 놓친다. 이번 공개 처리는 마지막 공개 `body_html`을 참조하므로 공개 프로그램 갱신도 필요하다. DNS·route·R2 객체와 실제 글 내용은 변경할 필요 없다. 운영 활성화 승인 전까지 `DWNC-CORE-003`은 `in-progress`다.
...
- 작업본 자동저장·명시적 공개 반영은 운영에 적용됐고 해당 최우선 작업의 장애물은 없다. 기본 서식 기능은 로컬 완료 후 운영 적용 승인이 남아 있고 모바일 크기 실측은 도구 한계로 미확인이다. 공개 화면 통합·대표이미지 연결과 아래 후속 문제도 남아 있다. 실제 글의 수정·발행·삭제는 운영 확인에 사용하지 않았다.
...
2. `PLAN-09`의 `DWNC-CORE-002`는 로컬 구현·실행 시험·빈 작업본 테이블 추가·관리자 운영 활성화·실제 화면 읽기 확인까지 완료했다. 이어 요청받은 `DWNC-CORE-003` 기본 서식은 로컬 구현·시험·브라우저 확인을 완료했다. 바로 다음은 승인 후 형식 열 추가 → 공개 Worker → 관리자 Worker 순서의 운영 적용이다. 앞선 작업본 CMS 적용에서 공개 Worker·DNS·route·기존 글·R2 객체와 Git 원격은 변경하지 않았다. 운영에서 열어 둔 이전 관리자 화면은 미저장 입력을 처리한 뒤 새로 열어야 새 UI를 사용한다.
