...
### 현재 요청 — 연락 이메일 공개와 조회 통계 (2026-09-08)
...
- 로컬 구현: 공통 footer 이메일 안내, 별도 `post_view_daily` 테이블과 조회 API, 관리자 오늘/최근30일/누적·일별상세·글별 검색/정렬/페이지를 구현했다. 새로고침은 새 열람으로 집계하며 알려진 봇·HEAD·관리자·미리보기·보호/비공개·예약전 응답은 제외한다. 날짜는 열람 당시 설정 시간대이며 변경해도 과거 날짜를 이동하지 않는다. 신규 통계 시험·통합 editor:test PASS. 자체 브라우저 합성 관리자에서41개 모두0 → 공개글1회 열람 → 오늘/30일/누적/해당글1을 확인하고 검색·그래프 표시를 확인했다. 새 정적 footer의 이메일 안내와 mailto 주소도 확인했다. 기존 작성 탭 새로고침은 자동 승인 검토가 미저장 입력 손실 우려로 차단하여 기존 탭을 보존하고 새 합성 시험 탭으로 확인했다. 서버 세션47312(4322/4324), 정적 미리보기88380(4325). 최종 source build 검사 중이며 운영은 아직 이전 완료 버전이다.
- DWNC-CORE-007 / PLAN-09 진행 중. 완료 조건은 공개 이메일 안내, 신규 조회 집계와 0 초기값, 관리자 통계, 통상 시험·실제 화면 확인 및 최종 새 빌드 운영 반영이다. 기존 승인 범위로 이어가며 부분 배포나 같은 승인 재요청을 하지 않는다.
- 위임: backend 조회 집계/API, management_ui 관리자 통계, integration_review 연락 안내/통합 검토. 메인은 브라우저·통합·공식 상태를 맡는다. 실제 글 변경/삭제, R2 덮어쓰기/삭제, Git push, raw deploy, DNS/route 변경, 비밀값/비공개 내용 기록은 금지한다.
