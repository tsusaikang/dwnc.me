...
최종 갱신: 2026-09-07 KST — PLAN-09 운영 적용 승인 유지 / 최소 로그인 권한 동의 대기, 운영 변경 미실행
...
- 로컬 구현·수정·통상 시험·문서 갱신·로컬 커밋은 승인됐다. Git push, production DNS/route 변경, raw `wrangler deploy`, 실제 글 수정/삭제, R2 객체 덮어쓰기/삭제는 금지한다. 빈 작업본 저장 공간 추가와 관리자 새 버전 활성화는 이후 사용자 승인을 받았으며, 현재는 최소 로그인 권한 동의만 남았다.
...
### PLAN-09 작업본 CMS 운영 적용 진행 (2026-09-07)
...
- 최소 요청 `account:read`, `workers_scripts:write`, `d1:write`의 표준 OAuth 동의 화면을 준비하고 내장 브라우저에서 Account Read / Workers Scripts Write / D1 Write 세 항목을 확인했다. 별도 DNS·routes·R2·KV·결제·사용자 조회·offline_access scope는 요청하지 않았다. 권한은 계정의 Workers 프로그램과 D1에 적용되며 특정 Worker/DB로 제한된 credential이라고 간주하지 않는다. 실제 작업은 승인된 관리자 Worker와 운영 D1의 빈 작업본 테이블 추가에만 한정한다. 새 Authorize는 아직 실행하지 않았고 이 정확한 권한 범위의 사용자 동의가 남았다. credential 파일·환경변수 저장은 하지 않는다.
- 운영 적용 자체에 대한 사용자 승인은 유지된다. 최소 권한 동의 뒤 대상/현재설정 읽기 확인 → 빈 작업본 테이블 추가 → 같은 소스의 새 admin version 업로드/활성화 → 실제 화면 읽기 확인 순서로 계속한다. 현재 운영 변경은 미실행이며 운영 관리자의 공개 즉시 자동저장 한계도 그대로다.
- 재개 정보: 최소 권한 OAuth 세션 `5915`가 `http://localhost:8976/login`에서 대기 중이다. 비밀이 없는 실행 코드는 `/private/tmp/dwnc-cms-oauth-session.mjs`와 `/private/tmp/dwnc-cms-approved-operations.mjs`, 승인된 소스의 관리자 번들은 `/var/folders/ps/kbhx2vz12m76shzc_s9wn30w0000gn/T/dwnc-admin-approved-bundle-vWCidH/bundle/admin-worker.js`다. callback·인증 수신은 0이며 세 권한 동의 전 Authorize/apply는 실행하지 않는다. 기존 운영 적용 승인은 유지된다. 프로세스가 종료됐거나 페이지가 만료되면 같은 세 scope로 다시 열 수 있다.
...
- 신규 빈 작업본 테이블과 관리자 버전 활성화는 사용자 승인 완료다. 위 운영 적용 진행 절의 최소 로그인 권한 동의 후 기존 공개본을 유지한 순서로 migration → 관리자 활성화 → 합의된 운영 확인을 진행한다. 이전 관리자 탭에서 작성 중인 내용을 새로고침으로 잃지 않도록 먼저 그 작성 상태를 처리하며, 실제 글 수정/삭제는 별도 승인 범위다.
...
2. `PLAN-09`의 `DWNC-CORE-002`은 로컬 구현·관련 실행 시험·내장 브라우저 확인을 완료했다. 다음 실제 서비스 변경은 기존 내용을 복사하거나 바꾸지 않는 `0004_editor_working_copies.sql`의 빈 작업본 테이블 추가와 완성된 관리자 Worker 활성화다. 운영 적용 승인은 유지되며, 최소 로그인 권한 동의 후 이 변경을 이어간다. 공개 Worker·DNS·route·기존 글·R2 객체와 Git 원격 변경은 필요하지 않다. 운영에서 열어 둔 이전 관리자 화면은 미저장 입력을 처리한 뒤 새로 열어야 새 UI를 사용한다.
