...
최종 갱신: 2026-08-28 KST — 시험용 R2 실제 전체 파일 내용 재확인 대기, 같은 프로그램의 인터넷 없는 2,758개 전량 모의점검 완료
...
비개발자용 현재 요약: 공개할 사진과 GIF 2,758개를 시험용 비공개 저장소에 모두 올렸고, 빠짐·내용 차이·불필요한 파일은 0개였다. 기존 파일을 덮어쓰거나 삭제하지 않았다. 실제 저장소를 다시 읽기 전, 로컬 원본 2,758개, 약 2.35GB로 인터넷 없는 예행연습도 통과했다. 파일 목록을 3회에 나눠 읽고 각 파일의 정보와 내용을 한 번씩 확인해 모두 5,519회 읽었으며 추가·변경·삭제는 0회였다. 이 예행연습은 실제 Cloudflare 파일을 확인한 결과는 아니므로, 다음에는 저장소의 파일을 하나씩 다시 내려받아 로컬 원본과 같은지 확인한다. 이후 시험용 사이트 프로그램을 처음 만들 때 잘못된 계정 주소·로그 설정·프로그램 파일이면 자동으로 멈추는 로컬 검사도 보강했다. 예전 주소 349개를 새 주소로 안내하는 동작은 사이트 프로그램이 직접 처리하도록 만들었고 로컬에서 두 가지 방식으로 총 698회 확인했다. 앞으로 이 프로젝트의 브라우저 작업은 Codex 자체 브라우저에서만 하며 Chrome과 Edge는 사용하지 않는다. 자체 브라우저의 로그인 상태가 프로젝트별로 분리된다고 보장할 수 없으므로, 다른 프로젝트에서 쓰던 Cloudflare 계정이 그대로 나타날 수 있다고 보고 매번 이 프로젝트의 계정 확인값과 대조한다. 브라우저 안에 Cloudflare 페이지는 준비했지만 패널이 화면에 나타나지 않은 상태다. 사용자는 앱의 브라우저 버튼이나 `Command+Shift+B`로 패널을 연 뒤 **“자체 브라우저 열림”**이라고 알려 주면 된다. 아직 시험용 사이트 프로그램을 온라인에 만들거나 올려서 켜지 않았고, 실제 `dwnc.me` 주소가 가리키는 곳도 바꾸지 않았다. 앞선 전량 예행연습을 저장할 당시 이번 변경은 로컬 파일 12개에만 한정됐고 인터넷 저장소로 보내지지 않았다. 지금 반영한 브라우저 정책은 로컬 문서 2개에만 한정한다.
...
1. 계정 번호 저장 사전검사는 이미 준비 완료다. Codex 자체 브라우저 안에 Cloudflare 페이지를 준비했지만 브라우저 패널이 화면에 나타나지 않은 상태다. 사용자는 앱의 브라우저 버튼이나 `Command+Shift+B`로 패널을 열고 **“자체 브라우저 열림”**이라고 알려 준다. 그 다음 화면의 계정이 이 프로젝트의 올바른 계정인지 확인한다. 맞는지 알 수 없거나 다른 계정이면 사용자가 직접 로그아웃·로그인하고, 올바른 계정의 `Account ID` 옆 `Copy`를 한 번 누른다. 그 직후 계정 번호를 전용 보관함에 저장한다. Chrome과 Edge는 사용하지 않으며 실제 저장·초기화는 아직 0회다.
