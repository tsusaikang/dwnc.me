...
최종 갱신: 2026-09-07 KST — 좁은 관리자 화면의 글 선택·편집 이동 수정 운영 완료
...
비개발자용 현재 요약: 공개 블로그와 이메일 OTP 관리자, 작업본 자동저장·명시적 공개 반영과 실패·재로그인·충돌 처리는 운영 중이다. 기본 서식 도구도 실제 관리자에 적용해 문단·크기·강조·색·정렬·목록·인용·링크·표를 지정할 수 있다. 기존 글 서식 보존과 저장·미리보기·공개·재열기는 로컬 합성 시험을 통과했고 운영에서는 기존 글과 이미지·서식 도구·공개 화면을 읽기로 확인했다. 실제 글과 사진은 변경하지 않았다. 다음은 기존·신규 공개 글 화면 통합과 대표이미지 연결이다.
...
- 작업본 자동저장·명시적 공개 반영은 운영에 적용됐고 해당 최우선 작업의 장애물은 없다. 기본 서식 기능과 좁은 글 목록/편집 이동 보완까지 운영 적용했다. 실제 700×720 내장 브라우저 확인은 완료했으며 더 작은 휴대전화 폭의 서식 전체 동작은 미확인이다. 공개 화면 통합·대표이미지 연결과 아래 후속 문제도 남아 있다. 실제 글의 수정·발행·삭제는 운영 확인에 사용하지 않았다.
