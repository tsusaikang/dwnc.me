...
최종 갱신: 2026-09-07 KST — 기본 서식 운영 적용 승인 / 자동 승인 검토의 로그인 동의 차단, 운영 변경 미실행
...
- 최신 승인/진행: 사용자가 `실제 서비스 적용하자`라고 명시 승인했다. 소스 `9416bed48e775e4e879a58ae1bccfbc967b6349d`와 clean 시작 상태, 최종 관리자 번들 일치를 확인했고 공개 Worker 단일 모듈도 로컬에서 준비했다. 기존 static assets는 `keep_assets`로 유지하는 순서다. main이 내장 브라우저의 프로젝트 계정 일치와 현재 운영 관리자의 기존 UI를 확인했다.
- 로그인 장애물: 앞서 같은 계정에 명시 승인받은 Account Read / Workers Scripts Write / D1 Write 세 권한만 다시 요청했으나, 자동 승인 검토가 정확한 권한/계정 승인이 없다는 이유로 Authorize를 거절했다. 이전 사용자 승인과 현재 계정 일치 근거를 명시한 동일 동작도 거절돼 더 재시도하거나 다른 수단으로 우회하지 않았다. 현재 사용자 배포 승인은 유효하며 이 로그인 동의의 재확인이 남았다. OAuth 콜백·인증 발급·D1/Worker/R2/DNS/route 변경은 모두 없다.
- 적용 준비물: 공개 `/var/folders/ps/kbhx2vz12m76shzc_s9wn30w0000gn/T/dwnc-public-formatting-bundle-6SdQgO/bundle/worker.js` (SHA-256 `3e847d9bab5efa75f311c2f2ce0f7a33545bab3c4dab6e924676e050cac8b1dd`), 관리자 번들은 아래 최종 파일 그대로다. 임시 실행 파일 `/private/tmp/dwnc-formatting-oauth-session.mjs`, `/private/tmp/dwnc-formatting-operations.mjs`는 비밀이 없는 코드다. 세션 `1258`이 `http://localhost:8976/login`에서 대기하며 요청 scope는 `account:read`, `workers_scripts:write`, `d1:write`다. offline_access와 인증 파일/환경변수 저장은 없다. 권한 자체는 계정의 Workers/D1에 적용되며 실제 작업은 승인된 dwnc.me 대상에만 한정한다.
- 재개: 새 로그인 권한 동의 후 콜백에서 운영 대상 읽기 확인이 수행된다. `readonly_ready` 뒤 `apply`를 보내 형식 열 추가 → 공개 Worker → 관리자 Worker를 적용한다. 기존 글/사진/static asset/연결 설정을 유지한다. `stop`은 메모리 세션과 localhost 서버를 종료한다. 현재는 새 동의 전 인증/적용을 실행하지 않는다.
...
- 기존 HTML에는 서식이 있으나 현재 관리자에 지정 도구가 없고, 새 글의 제한 Markdown 편집은 크기·색·밑줄·정렬·표를 제공하지 못한다. 기존·신규 글에 시각적 HTML 편집과 공통 서식 도구를 제공한다. 문단/제목, 크기, 굵게/기울임/밑줄/취소선, 색/배경색, 정렬, 목록, 인용, 구분선, 링크, 기본 표와 실행취소/다시실행을 구현한다.
...
- 로컬 구현·통상 시험·내장 브라우저 확인과 최종 빌드를 완료했다. 메인은 범위 통합·내장 브라우저·공식 상태를 맡고 UI, 저장/서식 처리, 시험은 하위 에이전트에 분리했다. 기존 운영 버전은 그대로다. 외부 적용은 승인됐으나 위 로그인 동의 차단으로 아직 미실행이다. Git push·실제 글 변경/삭제·DNS/route 변경·R2 덮어쓰기/삭제는 하지 않는다.
...
- 최종 관리자 로컬 번들: `/var/folders/ps/kbhx2vz12m76shzc_s9wn30w0000gn/T/dwnc-admin-formatting-bundle-dinC2e/bundle/admin-worker.js`, SHA-256 `f381406f1135c39e903c9315bfcfe3182f60733aaa3c17cdf9d3aa3aa3789a37` (1,576,805바이트). 마지막 연속 서식 보완을 포함하며 dry-run만 수행했고 업로드·배포는 없다.
- 운영 적용 최소 순서는 형식 열 추가 → 공개 Worker → 관리자 Worker다. 구 공개 Worker는 native 이미지 참조를 Markdown에서 찾으므로 새 HTML 본문 이미지를 놓친다. 이번 공개 처리는 마지막 공개 `body_html`을 참조하므로 공개 프로그램 갱신도 필요하다. DNS·route·R2 객체와 실제 글 내용은 변경할 필요 없다. 운영 적용은 승인됐으며 로그인 권한 동의 재확인과 실제 적용 완료 전까지 `DWNC-CORE-003`은 `in-progress`다.
...
비개발자용 현재 요약: 공개 블로그와 이메일 OTP 관리자, 작업본 자동저장·명시적 공개 반영과 실패·재로그인·충돌 처리는 운영 중이다. 사용자가 추가 요청한 기본 서식 도구도 로컬에 완성해 제목·크기·굵게·색·목록·인용·링크·표를 지정하고 저장·미리보기·공개·재열기 후 유지되는 것을 확인했다. 이 새 기능의 운영 적용은 승인됐으나 자동 승인 검토가 로그인 권한 동의를 차단해 실제 관리자에는 아직 서식 도구가 없다. 로그인 권한 동의 재확인을 기다리고 있다. 기존 글과 사진은 변경하지 않았다. 다음은 서식 기능의 저장 구조와 공개/관리자 프로그램 적용이며, 이후 기존·신규 공개 글 화면 통합·대표이미지 연결과 독립 사본 등 후속 작업을 진행한다.
...
- 웹 편집기: **이메일 OTP·애플리케이션 identity 확인·D1 초안/발행/수정·신규 R2 이미지·597 이후 순번·기존 349편 편집·이미지/링크 카드 UX·동적 공개와 작업본 자동저장·명시적 공개 반영·저장 실패/재로그인/충돌 처리까지 운영 중**
...
- 작업본 자동저장·명시적 공개 반영은 운영에 적용됐고 해당 최우선 작업의 장애물은 없다. 기본 서식 기능은 로컬 완료와 운영 적용 승인 후 로그인 권한 동의 재확인을 기다리고 있으며 모바일 크기 실측은 도구 한계로 미확인이다. 공개 화면 통합·대표이미지 연결과 아래 후속 문제도 남아 있다. 실제 글의 수정·발행·삭제는 운영 확인에 사용하지 않았다.
...
2. `PLAN-09`의 `DWNC-CORE-002`는 로컬 구현·실행 시험·빈 작업본 테이블 추가·관리자 운영 활성화·실제 화면 읽기 확인까지 완료했다. 이어 요청받은 `DWNC-CORE-003` 기본 서식은 로컬 구현·시험·브라우저 확인을 완료했다. 바로 다음은 차단된 로그인 권한 동의를 재확인받은 뒤 이미 승인된 형식 열 추가 → 공개 Worker → 관리자 Worker 순서의 운영 적용이다. 앞선 작업본 CMS 적용에서 공개 Worker·DNS·route·기존 글·R2 객체와 Git 원격은 변경하지 않았다. 운영에서 열어 둔 이전 관리자 화면은 미저장 입력을 처리한 뒤 새로 열어야 새 UI를 사용한다.
