...
- Cloudflare Stage 3 deny-only Worker 로컬 검사: **생성 전후 account `workers.dev=dwnc`, 생성 뒤 공개·미리보기 주소 꺼짐, 전체 로그·trace 설정, `content/v2`의 정확히 한 module 바이트·SHA-256, 연속 두 상태의 같은 deployment ID와 같은 version 100%를 함께 강제한다. 전체 Cloudflare 모의시험 24개·6,041 assertions PASS(최초 생성 검증·실행 코어 4,126, 전용 관리 열쇠 352) / 실제 Cloudflare 조회·생성·배포·DNS 변경·Keychain·clipboard 접근 0 / 최소권한 token runner의 로컬 바탕은 완료했고, 상태 확인만으로 이어가는 복구 전에는 bootstrap이 열쇠를 읽기 전에 무조건 중단**
...
- `PLAN-06` 시험용 Worker·버전·사이트 점검: **대기**. deny-only Worker 최초 생성 전후 로컬 검사는 미리 보강했지만 실제 Worker·version·activation은 0이고, 최초 생성 도중 중단 복구가 남아 있다.
...
- production·staging 최초 service는 signed 존재 증거와 one-time authorization을 요구하는 외부 요청 0 deny-only bootstrap으로 분리했다. 최초 읽기 전용 확인은 raw Cloudflare JSON과 5분 evidence 후보를 함께 보존하고, executor는 생성 직전 15초 이내의 service-not-found 404(code 10007/10090)만 다시 인정한다. 계정의 `workers.dev` 이름도 별도 API로 생성 전·직전·생성 뒤 읽어 정확히 `dwnc`인지 계정 확인값과 함께 허가·결과에 묶는다. deny 설정은 공개 주소·미리보기·Logpush·tail·tag·trace·외부 log destination을 끄고 observability/logs를 정해진 값으로 고정한다. API가 `logs.persist`를 생략한 경우에만 기본 `true`로 인정하고 명시적인 `false`·`null`은 거부한다. 생성 뒤 현재 deployment를 먼저 읽고, account subdomain·script subdomain·`/script-settings`·`/content/v2`·version detail 다섯 응답을 읽은 뒤 deployment를 다시 읽는 7개짜리 상태 묶음을 연속 두 번 확인한다. 모든 GET은 압축하지 않은 응답만 요구하고 정상·오류 응답 모두 최대 1MiB까지만 읽으며, 원본은 저장소 밖 고정 primary/recovery 위치에 create-only로 보존한다. 표준 multipart 표현 차이는 허용하지만 실제 한 module 바이트·SHA-256은 로컬 차단 코드와 같아야 하며, 같은 version 100%라도 deployment ID나 목록이 중간에 바뀌면 거부한다. 결과 기록은 단순 boolean이 아니라 생성 직전 두 확인, Wrangler 원본 결과, 생성 뒤 두 상태의 총 5종 원본 자료를 고정 위치에서 다시 읽어 역할·요청 확인값·status·시각·byte 수·SHA, module SHA·바이트 수, deployment/version과 결속한다. 실제 생성 부분은 외부 기능 기본값이 없는 별도 실행 코어로 분리했다. 최초 API 전에 1회용 허가와 원본·후보 primary/recovery 등 고정 출력 13개의 안전성과 부재를 모두 확인하고, 첫 GET 직전·허가 claim 직전·claim 뒤 생성 명령 직전에 승인 만료·exact Git HEAD·tracked/일반 untracked clean 상태를 세 번 확인한다. 가짜 API 85회·가짜 Wrangler 8회로 정상 흐름, 기존·부분·recovery 출력, 확인 또는 claim 중의 Git 변경·승인 만료 경합, 임시 출력 닫기·삭제 실패를 포함한 중단 경계를 끝까지 시험했다. staging smoke token은 암호학적 난수 32바이트를 padding 없는 base64url 43문자로 만들고, 평문을 argv·env·receipt에 노출하지 않는다. staging media·release signing key는 macOS Keychain에 생성했지만 token·endpoint·version·receipt는 아직 없다. 최초 생성 명령 성공 뒤 증거 기록 전에 중단되는 경우, 같은 명령을 반복하지 않고 상태만 확인하는 복구 절차는 별도 P0로 남아 있다. 정확한 staging 계정 한 곳의 `Workers Scripts Edit` 한 권한만 가진 짧은 수명 Account API token을 Keychain→익명 FD 3→격리된 Wrangler 자식으로 전달하는 로컬 바탕은 완성했다. bootstrap은 verified runner를 요구하지만 복구 P0 뒤까지 token을 읽지 않으며, 아직 연결하지 않은 staging 변경 명령도 새 runner에서 token 읽기 전에 중단한다. 직접 실행 코어에는 OAuth·`HOME`·live network/명령 기본값이 없다. staging은 production과 다른 exact version ID를 fresh status CAS·pending/invoking/outcome lock 아래 100% 활성화하며 ambiguous 결과는 owner-dead·5분 뒤 status-only recovery에서만 확정한다. Bearer로 보호한 `workers.dev` endpoint에서 GET·HEAD·304·206·416·MIME·ETag·static·349 redirects·cache·미인증 deny를 증명한다.
...
- 2026-08-28 deny-only bootstrap foundation 보완 뒤 Cloudflare 관련 전체 23 suites·5,689 assertions가 통과했다. bootstrap 응답·정책·원본 결속 검증은 3,235개, 실제 실행 흐름 모의시험은 891개로 합계 4,126개다. 실행 흐름 시험은 가짜 API 85회·가짜 Wrangler 8회를 사용했고 실제 API·Wrangler·Cloudflare 변경·Keychain·clipboard는 0회다.
...
- deny-only 최초 생성이 실제로 성공했지만 사후 확인 자료를 모두 쓰기 전에 프로세스가 중단될 수 있다. 이때 같은 생성 명령을 다시 실행하지 않고 기존 one-time claim과 Cloudflare 현재 상태만 읽어 결과를 확정하거나 중단하는 bootstrap 전용 status-only recovery는 아직 없다. 이것이 실제 최초 생성 전에 남은 별도 P0다.
...
4. 전체 파일 내용 확인을 마치면 deny-only Worker 최초 생성이 중간에 끊긴 경우 같은 생성 명령을 다시 실행하지 않고 상태만 확인하는 복구 절차를 먼저 완성한다.
5. 완성된 짧은 수명 관리 열쇠 도구를 version upload·상세 확인·적용·workers.dev 활성화의 기존 로컬 검증 흐름에 하나씩 연결한다. 현재 이 네 종류는 연결 전이라 열쇠를 읽기 전에 중단한다.
6. 상태 확인 전용 복구와 관리 열쇠 연결, 보호된 서명 증거가 모두 준비되면 계정 주소·설정·실제 차단 코드·적용 version을 생성 전후로 확인하면서 외부 요청을 모두 거부하는 시험용 Worker를 최초 생성한다.
7. 확정된 Git 기록과 R2 감사 결과에 묶인 staging Worker version만 올리고 적용한 뒤 정적 페이지·349 redirects·media GET/HEAD/304/206/416을 `workers.dev`에서 검증한다.
8. production 이름의 Cloudflare 자원·version 작업이 필요하면 같은 안전 절차로 계속한다. 실제 `dwnc.me` 도메인·DNS·route는 연결하지 않는다.
9. 공유·스크랩 추정 10개, 새 글 편집 방식, 과거 댓글과 private backup 정책을 별도 결정한다. 실제 도메인·DNS 연결을 후속으로 결정하면 `docs/URL_CONTRACT.md`의 edge redirect를 실제 응답으로 검증한다.
