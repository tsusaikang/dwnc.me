...
최종 갱신: 2026-08-29 KST — 시험용 R2 실제 전체 파일 내용 재확인 대기, 같은 프로그램의 인터넷 없는 2,758개 전량 모의점검 완료, 장기 작업 오케스트레이션·컨텍스트 관리 규칙 반영
...
비개발자용 현재 요약: 공개할 사진과 GIF 2,758개를 시험용 비공개 저장소에 모두 올렸고, 빠짐·내용 차이·불필요한 파일은 0개였다. 기존 파일을 덮어쓰거나 삭제하지 않았다. 실제 저장소를 다시 읽기 전, 로컬 원본 2,758개, 약 2.35GB로 인터넷 없는 예행연습도 통과했다. 파일 목록을 3회에 나눠 읽고 각 파일의 정보와 내용을 한 번씩 확인해 모두 5,519회 읽었으며 추가·변경·삭제는 0회였다. 이 예행연습은 실제 Cloudflare 파일을 확인한 결과는 아니므로, 다음에는 저장소의 파일을 하나씩 다시 내려받아 로컬 원본과 같은지 확인한다. 이후 시험용 사이트 프로그램을 처음 만들 때 잘못된 계정 주소·로그 설정·프로그램 파일이면 자동으로 멈추는 로컬 검사도 보강했다. 예전 주소 349개를 새 주소로 안내하는 동작은 사이트 프로그램이 직접 처리하도록 만들었고 로컬에서 두 가지 방식으로 총 698회 확인했다. 앞으로 이 프로젝트의 브라우저 작업은 Codex 자체 브라우저에서만 하며 Chrome과 Edge는 사용하지 않는다. 자체 브라우저의 로그인 상태가 프로젝트별로 분리된다고 보장할 수 없으므로, 다른 프로젝트에서 쓰던 Cloudflare 계정이 그대로 나타날 수 있다고 보고 매번 이 프로젝트의 계정 확인값과 대조한다. Codex 자체 브라우저는 현재 열려 있지만 화면의 Cloudflare 계정이 이 프로젝트에서 사용할 계정인지는 아직 확인하지 않았다. 다음 Cloudflare 작업 전에 프로젝트에 기록된 계정 확인값과 다시 대조하고, 다르거나 알 수 없으면 사용자가 직접 로그아웃·로그인한다. 아직 시험용 사이트 프로그램을 온라인에 만들거나 올려서 켜지 않았고, 실제 `dwnc.me` 주소가 가리키는 곳도 바꾸지 않았다. 앞선 전량 예행연습을 저장할 당시 이번 변경은 로컬 파일 12개에만 한정됐고 인터넷 저장소로 보내지지 않았다. 지금 반영한 브라우저 정책은 로컬 문서 2개에만 한정한다.
...
- Cloudflare 계정 선택 안전장치: **계정 번호를 사진 업로드용·검사용 열쇠와 분리했다. 복사 전 사전검사는 클립보드를 읽거나 비우지 않아 기존 내용을 보존하고, 초기화는 계정 번호 읽기 시도 뒤에만 즉시 비운다. 저장 직전 상태 재검사·새 항목으로만 저장·성공 뒤 재검증, Mac 보관함 값 두 번 대조, 15초 제한과 단계적 종료, Mac이 직접 알려 준 필수 환경값 7개만 전달하는 격리까지 로컬 시험 610개 PASS. 확인만 할 때 원문 문자열 생성 0이며, 모든 성공·실패에서 임시 바이트를 0으로 덮는다. 개발 중 가짜 연결 누락으로 실제 보관함 읽기 명령 1회가 있었지만 두 번째 읽기·저장·변경·클립보드·Cloudflare는 0회였다. 감시 보강 뒤 추가 실제 실행은 0회다. 실제 Keychain·metadata 초기화와 Cloudflare 조회는 아직 0회다.**
...
- `PLAN-05` 시험용 R2 업로드와 전체 내용 확인: **진행 중**. 업로드와 목록 확인, 실제 감사 진입점의 인터넷 없는 2,758개 전량 예행연습은 완료했다. 비공개 설정을 새로 확인한 뒤 실제 staging R2의 전체 2,758개 내용을 비교한다.
- `PLAN-06` 시험용 Worker·버전·사이트 점검: **대기**. deny-only Worker 최초 생성과 중단 복구, 예전 주소 349개 직접 처리와 원격 점검 목록의 로컬 안전장치는 완료했지만 실제 Worker·version·activation은 0이다.
...
- 메인 세션은 작업 순서·범위·승인 경계를 조정하고, 결과 통합검토·검증·공식 상태 갱신·사용자 보고를 맡는다. 주요 실무는 내용에 맞는 하위 에이전트에 위임한다.
...
- 같은 사이트에서 서로 다른 계정을 사용하는 프로젝트의 자체 브라우저 작업은 동시에 하지 않고 하나씩 진행한다. 브라우저 로그인 상태는 올바른 계정의 증거가 아니며, Cloudflare 작업을 시작할 때마다 프로젝트에 저장된 계정 확인값과 화면의 계정을 다시 대조한다. 다르거나 알 수 없으면 설정을 읽거나 바꾸고 열쇠를 만들기 전에 멈추고 사용자에게 직접 로그아웃·로그인을 요청한다.
...
63. 앞으로 이 프로젝트의 모든 브라우저 작업은 Codex 자체 브라우저에서만 하고 Chrome과 Edge를 사용하지 않는다. 자체 브라우저의 로그인 상태가 프로젝트·작업마다 분리된다는 보장은 확인되지 않았으므로, 같은 사이트에서 서로 다른 계정을 쓰는 프로젝트 작업은 동시에 하지 않고 하나씩 진행한다. 다른 프로젝트 작업 뒤에는 계정을 다시 확인한다. 브라우저 로그인 상태 자체는 올바른 계정의 증거로 삼지 않으며, 모든 Cloudflare 브라우저 작업 시작 직전에 화면의 `Account ID` 또는 그 확인값이 이 프로젝트에 안전하게 저장된 계정 확인값과 일치하고 대상 계정이 정확히 한 개뿐인지 확인한다. 다르거나 확인할 수 없으면 설정 조회·변경과 열쇠 생성 전에 멈추고 사용자가 직접 로그아웃·로그인한다. 로그인 정보와 비밀번호는 저장하지 않는다. 이 결정은 상시 요구사항 `DWNC-OPS-006`으로 관리한다.
64. 장기 작업은 메인 세션의 오케스트레이션과 하위 에이전트의 주요 실무를 분리하고, 새 주요 단계의 결과 수신부터 사용자 보고까지 현재 컨텍스트에서 안전하게 마칠 수 있을 때만 시작한다. 사용자 완료조건에 직접 기여하지 않는 보조 작업과 충분한 근거 뒤의 반복 검증은 중단한다.
...
- 현재 로컬 기준은 commit `fb693f3bbbab0e88b205f9de944646057bdd49ea`·tree `5e7169ca35a61a077265c98f6819fa28ccc5126d`·parent `d0b4a6c68c8a51b3211c97e320c5d7ea0310f437`다. 정확히 12개 파일의 `+1,990/-42` 변경을 담고, 변경 파일 집합 확인값은 `94fb0e5a8c2e67def5f4b1460431170eaae5f9505f78c7d900bea9446ead8374`다. commit 직후 파일 상태는 깨끗했고 push는 0회다.
...
- 현재 최종 공개 미디어는 2,758개·2,346,220,246바이트·manifest SHA-256 `61bb577d609f97cdb014ef3a14681045fbb3bec616f2b04c8d058519b640c532`·full object-set SHA-256 `9345d2f06c8bd7cda457a9d4335cdc2213e71dcd30bb9e11e6f3e1f8e11ae467`로 검증 완료다. 2,889개는 2026-08-25 정리 전 역사 기준선이고 2,785개는 B 결정 전 중간 수치다. private staging bucket은 exact 2,758·missing 0·mismatch 0·orphan 0이며 overwrite·delete는 0이다. 로컬 전량 예행연습은 통과했지만 전체 원격 파일 내용을 다시 읽은 signed full-set receipt·binding은 아직 없다.
...
1. 계정 번호 저장 사전검사는 이미 준비 완료다. Codex 자체 브라우저는 현재 열려 있지만 화면의 Cloudflare 계정이 이 프로젝트에서 사용할 계정인지는 아직 확인하지 않았다. 다음 Cloudflare 작업 전에 프로젝트에 기록된 계정 확인값과 대조한다. 다르거나 알 수 없으면 사용자가 직접 로그아웃·로그인한다. 올바른 계정임을 확인한 뒤에만 계정 번호를 전용 보관함에 저장하는 절차를 계속한다. Chrome과 Edge는 사용하지 않으며 실제 저장·초기화는 아직 0회다.
...
4. 전체 파일 내용 확인을 마치면 보호된 서명 증거와 새 준비·시작·결과·상태 기록 절차 아래 외부 요청을 모두 거부하는 시험용 Worker를 최초 생성한다. 중단되면 생성 명령을 반복하지 않고 완성된 status-only recovery만 사용한다.
5. 완성된 짧은 수명 관리 열쇠 도구를 version upload·상세 확인·적용·workers.dev 활성화의 기존 로컬 검증 흐름에 하나씩 연결한다. 현재 이 네 종류는 연결 전이라 열쇠를 읽기 전에 중단한다.
6. 확정된 Git 기록과 R2 감사 결과에 묶인 staging Worker version만 올리고 적용한 뒤 `/about`과 `/404.html`을 각각 두 방식으로 확인하고, 예전 주소 349개를 두 방식으로 총 698회 확인하며, 미디어의 일반 표시·변경 없음 응답·부분 전송·범위를 벗어난 요청, cache와 무인증 차단을 `workers.dev`에서 검증한다.
7. production 이름의 Cloudflare 자원·version 작업이 필요하면 같은 안전 절차로 계속한다. 실제 `dwnc.me` 도메인·DNS·route는 연결하지 않는다.
8. 공유·스크랩 추정 10개, 새 글 편집 방식, 과거 댓글과 private backup 정책을 별도 결정한다. 실제 도메인·DNS 연결을 후속으로 결정하면 `docs/URL_CONTRACT.md`의 edge redirect를 실제 응답으로 검증한다.

## 중요한 제약과 주의사항

- 기존 네이버·티스토리 글을 수정하거나 삭제하지 않는다.
- Codex 자체 브라우저의 로그인 상태를 프로젝트별로 분리됐다고 가정하지 않는다. 같은 사이트의 서로 다른 계정 작업을 동시에 진행하지 않으며, 모든 Cloudflare 브라우저 작업 직전에 이 프로젝트의 계정인지 다시 확인한다.
