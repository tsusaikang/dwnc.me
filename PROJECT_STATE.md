...
최종 갱신: 2026-09-03 KST — 자체 브라우저와 Mac 사이의 지원되는 자동 계정 번호 전달 경로를 찾지 못해 `PLAN-05` 중단, 실제 2,758개 파일 내용 전수 비교 대기
...
비개발자용 현재 요약: 공개할 사진과 GIF 2,758개를 시험용 비공개 저장소에 모두 올렸고, 업로드 뒤 목록·크기·저장정보를 확인한 결과 빠진 항목과 불필요한 항목은 없었다. Codex 자체 브라우저의 Cloudflare 계정도 이 프로젝트 계정과 정확히 일치함을 확인했다. 이전 탭 확인 과정에서 계정 식별자가 포함된 URL이 내부 도구 출력에 1회 표시된 사실은 그대로 유지하되, 원문과 URL을 다시 기록하지 않았다. localhost 전달 실패 뒤 비민감 시험값으로 자체 브라우저 클립보드 쓰기 1회와 macOS `pbpaste` 읽기 1회를 대조했으나 값은 일치하지 않았고 Mac 시스템 클립보드 쓰기는 0회였다. 설치된 구현에서 자체 브라우저는 virtual clipboard를 사용하며 시스템 클립보드 공유는 공식 보장되지 않는다. 그래서 계정 번호를 표시하지 않고 받는 로컬 stdin receiver를 commit `4467a7e468a6aa72f3f26c4e12e9ce3b8d5c9779`·tree `9c6505ef46393b139c923ad5a43f0605c4b14611`에 구현했다. 전용 112개·계정 대상 698개 확인과 변경 형식 검사를 통과했고 독립 검토 지적은 0건, push는 0회다. 비민감 자체 브라우저→Terminal 시험은 receiver의 `READY` 뒤 Computer Use가 `com.apple.Terminal`로 값을 넘기기 전에 안전 정책으로 차단했다. 로컬 paste·receiver 입력은 0회였고 receiverStopped·echoRestored는 true, 브라우저 시험 클립보드는 비웠으며 재시도나 다른 앱 우회는 0회였다. 따라서 현재 지원되는 자동 전달 경로가 없어 `PLAN-05`를 중단했다. 이번 stdin/Terminal 경로의 Account ID copy·read·paste·initialize는 모두 0회다. 앞선 localhost 경로의 누계는 browser memory 처리·client send 시도 1회, host accepted·success 0회이며 Keychain·metadata 작성은 전체 0회다. Cloudflare·R2·API·전체 내용 감사도 0회이고 `dwnc.me` 주소·트래픽도 바꾸지 않았다.
...
- `PLAN-05` 시험용 R2 실제 내용 확인과 열쇠 정리: **중단**. localhost와 시스템 클립보드는 자체 브라우저에서 Mac 로컬로 값을 전달하지 못했고, secure non-echo stdin receiver를 준비한 뒤의 비민감 Terminal 시험도 값 전달 전에 제품 안전 정책으로 차단됐다. 현재 지원되는 자동 전달 경로가 없으므로 재시도·대체 앱 우회를 하지 않는다. 제품이 안전한 로컬 receiver target을 허용하거나, 사용자가 원문을 도구 출력 없이 non-echo receiver에 직접 입력하는 명시적 handoff가 있을 때만 재개한다. 계정 보관·저장소 비공개 확인·실제 2,758개 GET/SHA-256 비교·열쇠 정리는 수행하지 않았다.
...
- 2026-09-03 실제 staging full audit는 객체 2,758개·2,346,220,246바이트·orphan 0·LIST 3·GET 2,758·PUT/DELETE 0과 full object-set SHA-256 `9345d2f06c8bd7cda457a9d4335cdc2213e71dcd30bb9e11e6f3e1f8e11ae467`로 끝났지만 HEAD가 2,760이었다. 이는 full audit client의 기본 자동 재시도 때문에 논리적 HEAD 2,758개 중 2건이 다시 요청된 결과이므로 retry 0·HEAD exact 2,758 완료조건을 충족하지 않는다. 기존 create-only receipt는 수정·삭제하지 않고 후보로만 보존하며, `maxAttempts: 1` 수정 검증·새 Git 결속·fresh exposure capture·새 receipt 경로 뒤 한 번 다시 실행해야 한다.
- exact-count·`maxAttempts: 1`을 적용한 2026-09-04 final2 감사는 약 2분 뒤 `MEDIA_E_R2_TIMEOUT`으로 중단됐고 receipt는 생성되지 않았다. 당시 30초 제한은 응답 헤더 대기뿐 아니라 각 LIST/GET 전체 body streaming에도 고정 wall-clock으로 적용돼 진행 중인 큰 객체도 끊을 수 있었으며, 실패 단계·객체·부분 요청 수는 남지 않았다. 따라서 이 실행은 전수 감사 근거가 아니다. full audit에만 헤더와 전체 body를 각각 최대 120초로 늘리고 공용 client·bulk retry는 유지한 뒤, 새 Git 결속·fresh exposure capture·새 receipt 경로로 다시 실행해야 한다.
...
- bulk 안전장치와 실행 기준은 commit `ccec8bb855e45ef679a4b99faf0f0633a99e089e`·tree `241a654acc387233d4e2d5e076754b345619e9ac`에 기록했고 push는 0이다. 실제 감사 진입점의 인터넷 없는 2,758개 전량 예행연습은 완료했지만 private-exposure canonical API evidence·실제 staging R2 전체 2,758개 full GET/SHA-256 감사·signed R2 receipt·staging Worker/version upload·activation·smoke는 미완료다.
...
- 실제 staging full audit 후보는 HEAD 자동 재시도 2회로 완료조건에서 제외했고, no-retry final2는 30초 요청 제한으로 receipt 없이 중단됐다. full audit 전용 `maxAttempts: 1`·120초 제한을 검증한 뒤 새 Git·fresh capture·새 create-only receipt 경로로 재실행해야 한다.
...
- 현재 최종 공개 미디어의 로컬 원본은 2,758개·2,346,220,246바이트·manifest SHA-256 `61bb577d609f97cdb014ef3a14681045fbb3bec616f2b04c8d058519b640c532`·full object-set SHA-256 `9345d2f06c8bd7cda457a9d4335cdc2213e71dcd30bb9e11e6f3e1f8e11ae467`로 검증 완료다. 2,889개는 2026-08-25 정리 전 역사 기준선이고 2,785개는 B 결정 전 중간 수치다. private staging bucket은 목록·크기·저장정보 기준 exact 2,758·missing 0·mismatch 0·orphan 0이며 overwrite·delete는 0이다. 로컬 전량 예행연습은 통과했지만 실제 원격 파일 내용을 다시 읽는 전수 GET/SHA-256 비교와 그 최종 기록은 아직 없다.
...
- 최초 생성 전용 열쇠 전달 도구의 로컬 바탕은 완성했다. 열쇠는 정확한 staging 계정 한 곳의 새 53자 Account API token이고 접두사 없는 예전 형식은 받지 않는다. Cloudflare 화면 권한은 `계정 > Workers Scripts > Edit` 한 가지여야 한다. 유효기간은 다음 날 만료를 기본으로 하되 총 48시간 이하, 실행 시 남은 시간이 60분 이상이어야 하며 R2·DNS·Zone·KV·route·삭제·production 작업은 허용하지 않는다. Keychain 항목과 프로젝트 밖의 기본/고정 예비 확인 파일은 새로 만들기만 하며 덮어쓰기·삭제하지 않는다. 두 확인 파일의 안전성과 상태를 열쇠/API보다 먼저 분류하고 실제 쓰기 직전에 다시 확인한다. 현재 payload는 402바이트·536자이고 Keychain 운반 상한은 1,024자다. 원래 계정 번호·token·token ID·계정 이름은 파일과 출력에 남기지 않는다. 실행 전 token API의 성공과 빈 오류 목록, 활성·시작/만료·확인값, exact account의 `workers.dev=dwnc`, Wrangler 4.125 `whoami --json`의 Account API Token과 계정 1개 일치를 확인한다. Wrangler의 3KB 안내 launcher가 아니라 실제 CLI와 최소 실행 묶음 660파일·207,434,085바이트 전체를 고정 tree SHA로 검증해 격리 폴더에 새 읽기 전용 사본을 만든 뒤 현재 Node 절대경로로 직접 실행한다. 변조·symlink·확인 직후 교체가 있으면 열쇠를 읽거나 자식을 시작하기 전에 중단한다. 열쇠는 익명 FD 3으로 허용된 작업에만 넘기고 실제 Wrangler 자식만 token 환경변수를 받는다. HOME·XDG·임시 폴더, 고정 PATH와 빈 env file을 격리하고 `NODE_OPTIONS`·로그·사용량 전송·로그인 정보 자동 대체를 막는다. FD가 없거나 부분 쓰기·실행 오류가 나면 자식을 bounded 종료하고 출력 통로가 닫힌 뒤 임시 폴더와 buffer를 정리한다. JavaScript 문자열은 메모리를 직접 덮을 수 없으므로 반환 객체와 자식 환경의 열쇠 필드를 사용 직후 빈 문자열로 바꾸고 참조를 버린다. 과거 workers.dev 읽기 runner 분기와 공개 FD 전용 npm 명령은 제거했다. 실제 Keychain 항목 생성, token 검증과 Cloudflare 요청은 아직 0회다. 사용이 끝나면 이 권한으로는 자기 자신을 폐기할 수 없으므로 사용자가 Cloudflare 화면에서 token을 폐기해야 한다.
...
1. `PLAN-05`는 현재 중단한다. localhost를 다시 시도하거나 다른 앱으로 우회하지 않는다. 제품이 안전한 로컬 receiver target을 허용하거나, 사용자가 계정 번호 원문을 도구 출력 없이 준비된 non-echo receiver에 직접 입력하겠다는 명시적 handoff를 제공할 때만 재개한다. 전달이 실제로 성공한 경우에만 최소 권한·짧은 수명 열쇠로 시험용 저장소의 비공개 상태를 한 번 읽고, 실제 파일 2,758개를 한 번 전수 GET해 개별 SHA-256과 총 2,346,220,246바이트를 원본과 비교한다. 통과 뒤 이번 작업용 열쇠와 사용 불가능한 기존 활성 열쇠를 정확히 식별해 정리한다.
