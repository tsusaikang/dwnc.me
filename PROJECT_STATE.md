...
최종 갱신: 2026-08-28 KST — 시험용 R2 전체 파일 내용 재확인 대기, Worker의 예전 주소 349개 직접 처리와 원격 점검 프로그램 안전 보강의 로컬 검증 완료
...
비개발자용 현재 요약: 공개할 사진과 GIF 2,758개를 시험용 비공개 저장소에 모두 올렸고, 빠짐·내용 차이·불필요한 파일은 0개였다. 기존 파일을 덮어쓰거나 삭제하지 않았다. 다음에는 저장소의 파일을 하나씩 다시 내려받아 로컬 원본과 같은지 확인한다. 이후 시험용 사이트 프로그램을 처음 만들 때 잘못된 계정 주소·로그 설정·프로그램 파일·적용 version이면 자동으로 멈추는 로컬 검사도 보강했다. 예전 주소 349개를 새 주소로 안내하는 동작은 사이트 프로그램이 직접 처리하도록 만들었고 로컬에서 두 가지 방식으로 총 698회 확인했다. 아직 시험용 Worker를 실제로 올리지 않았고 실제 `dwnc.me` 주소나 DNS도 연결하지 않았다.
...
- Cloudflare Stage 3 deny-only Worker 로컬 검사: **생성 전후 account `workers.dev=dwnc`, 생성 뒤 공개·미리보기 주소 꺼짐, 전체 로그·trace 설정, `content/v2`의 정확히 한 module 바이트·SHA-256, deploy 가능한 version 정확히 1개와 deployment 정확히 1개·100%, 연속 두 현재 상태의 일치를 함께 강제한다. 자동 집계한 전체 Cloudflare 모의시험 26개·15,024 assertions PASS(Worker 전수 5,803, staging 원격 점검 179, 상태·원본 검증 3,718, 중단 복구 1,023, 실제 runner→복구 자식 139, 실제 실행 흐름 1,819, 전용 관리 열쇠 378, 계정 대상 610) / 실제 Cloudflare 조회·생성·배포·DNS 변경·clipboard 접근 0 / 계정 보관함 read command 사고 1, second read·저장·변경 0, guard 뒤 추가 native spawn 0 / 최소 권한 열쇠 전달과 생성 중단 복구의 로컬 안전장치 완료**
...
- `PLAN-05` 시험용 R2 업로드와 전체 내용 확인: **진행 중**. 업로드와 목록 확인은 완료했다. 비공개 설정을 새로 확인한 뒤 전체 2,758개 내용을 비교한다.
...
1. `cloudflare:account-target:preflight`는 이미 준비 완료다. 다음 사용자 동작은 올바른 Cloudflare 계정 화면에서 **Account ID 옆 Copy를 한 번 누르는 것**이다. 그 직후 `cloudflare:account-target:init`으로 계정 번호를 전용 보관함에 저장한다. 실제 저장·초기화는 아직 0회다.
