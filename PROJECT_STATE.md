...
최종 갱신: 2026-09-06 KST — PLAN-05부터 PLAN-08까지 완료 / PLAN-09 저장소 연결·초기 migration·production public Worker 비활성 version 준비 완료, Access·관리자 연결·공개 전환 전
...
비개발자용 현재 요약: `PLAN-08` 운영 전환과 실제 화면 확인은 완료됐다. 사용자는 저장소를 직접 고치거나 재배포하는 방식 대신 Cloudflare 로그인형 웹 편집기를 선택했다. 별도 관리자 화면의 로컬 구현·시험을 마쳤고 staging·production D1과 각각의 별도 private R2를 만든 뒤 두 D1에 초기 구조를 적용했다. 첫 발행은 597번부터 DB에서 한 번만 배정하며, 공개 글은 재배포 없이 홈·아카이브·카테고리·태그·검색·RSS·사이트맵에 나타나게 한다. 기존 1–596번 글과 2,758개 미디어는 바꾸지 않는다. Zero Trust Free plan은 활성화됐고 오늘 결제액은 $0이다. 사용자는 무료 한도를 넘으면 등록 카드에 요금이 청구될 수 있다는 조건을 승인했다. production public Worker의 새 version은 올렸지만 이용자 traffic에는 아직 적용하지 않았다. Google OAuth에는 별도 Client ID와 Client secret이 필요하고 아직 만들거나 입력하지 않았으므로 Access 앱·관리자 주소·관리자 Worker와 공개 전환은 남아 있다.
...
- 웹 편집기: **Access JWT 이중 확인·별도 관리자 Worker·D1 초안/발행/수정·신규 R2 이미지·597 이후 원자적 순번·동적 공개 탐색 로컬 구현 완료 / Zero Trust Free 활성화 / staging·production D1·private R2 생성과 초기 migration 완료 / production public Worker 새 비활성 version 업로드 완료 / Google OAuth·Access 앱·관리자 주소·admin Worker·public promotion 연결 전**
...
- Cloudflare Access의 Google identity provider 설정 화면에서 별도 OAuth Client ID와 Client secret이 필요함을 확인했다. 두 값은 아직 생성하거나 입력하지 않았다. 따라서 Access application·audience·허용 이메일·관리자 hostname, admin Worker version upload·route, 준비된 public version의 traffic 적용은 아직 완료되지 않았다.

## 미해결 문제

- 콘텐츠 이전 정확성·완전성 측면의 알려진 문제는 없다. Stage 3 R2 전체 2,758개 업로드, 사후 목록 확인, 실제 전체 내용·bytes·SHA-256 검증이 완료됐다.
- 웹 편집기를 실제로 열기 위한 직접 선행 조건은 Google OAuth Client ID·Client secret 생성과 Cloudflare Access 입력이다. 그 다음 Access 앱의 audience·허용 이메일·관리자 hostname을 확정해야 admin Worker를 올리고 route를 연결할 수 있다. production public Worker의 새 version은 준비됐지만 승인된 public promotion 전까지 현재 이용자에게 반영되지 않는다.
...
2. `PLAN-06`·`PLAN-07`·`PLAN-08`과 `DWNC-S3-009`·`DWNC-S3-010`·`DWNC-S3-011`·`DWNC-S3-014`는 완료됐다. `PLAN-09`의 로컬 구현·시험, Zero Trust Free 활성화, staging·production D1·private R2·초기 migration, production public Worker의 비활성 version 준비까지 완료했다. 다음은 Google OAuth 자격을 만들고 Access에 입력한 뒤 Access 앱·허용 이메일·관리자 주소를 확정하고 admin Worker version·route를 연결하는 단계다. 준비된 public version의 traffic 적용은 별도 공개 전환 경계이며, 비공개 자료 백업·복구 방식도 별도 사용자 결정으로 남는다.
