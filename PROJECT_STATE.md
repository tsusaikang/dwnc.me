...
- Cloudflare Stage 3 deny-only Worker 로컬 검사: **생성 전후 account `workers.dev=dwnc`, 생성 뒤 공개·미리보기 주소 꺼짐, 전체 로그·trace 설정, `content/v2`의 정확히 한 module 바이트·SHA-256, 연속 두 상태의 같은 deployment ID와 같은 version 100%를 함께 강제한다. 전체 Cloudflare 모의시험 23개·5,689 assertions PASS(최초 생성 검증·실행 코어 4,126) / 실제 Cloudflare 조회·생성·배포·DNS 변경 0 / 상태 확인만으로 이어가는 복구와 전용 최소권한 token runner 전에는 실제 실행이 열쇠를 읽기 전에 무조건 중단**
...
- 2026-08-25 Stage 3 로컬 admission checkpoint의 12 suites·463 assertions PASS, 2026-08-27 signing identity 이전의 18 suites·833 assertions PASS, validator inspection commit 전의 19 suites·922 assertions PASS, S3 version-id 호환 commit 전의 19 suites·954 assertions PASS, 단일 객체 validator 변경의 20 suites·1,144 assertions PASS, private-exposure read-only 변경의 20 suites·1,299 assertions PASS는 역사 기록으로 보존한다. bulk hardening commit `ccec8bb855e45ef679a4b99faf0f0633a99e089e` 후보는 Cloudflare 관련 21 suites·1,452 assertions를 통과했다. 이번 계정 번호 분리 변경은 Cloudflare 관련 22 suites·1,590 assertions를 통과했다. 새 account-target fixture 123 assertions는 올바른 계정, 클립보드 단일 읽기·즉시 비우기, 잘못되거나 빈 값, 기존 Keychain 항목, 확인 파일의 기존 파일·symlink·hardlink·권한·경합, Keychain 성공 뒤 확인 파일 실패, HOME 변경에도 시스템 사용자 홈 기반 기본 위치 유지, repository 내부·실제 경로 containment·ancestor symlink 거부, 임의 recovery 경로 거부, 0바이트 실패 파일 보존과 고정 recovery 파일 복구·재확인, 두 확인 파일이 모두 불완전할 때 자동 진행 중단과 Keychain 보존, metadata·fingerprint·purpose·service 변조, 사진 업로드용 열쇠 접근 0과 원래 계정 번호 비노출을 검증한다. control-plane auth 81 assertions는 부모 account 환경변수와 repository 내부 확인 파일 거부, 전용 항목 단일 읽기, 자식 환경과 token FD 분리, 잘못 보관된 계정의 clipboard/API 0을 추가로 확인한다. 기존 bulk fixture 142, signing 104, 단일 객체 178, exposure 145 assertions도 그대로 통과했다. 이 로컬 검증에서 live network·실제 Keychain·clipboard 호출·원격 객체 변경·overwrite·delete는 0이다.
...
- 최초 생성 전용 열쇠 전달 도구도 다음 독립 구현으로 남아 있다. 열쇠는 정확한 staging 계정 한 곳의 Account API token이고 권한은 Cloudflare 공식 명칭 `Workers Scripts Write` 하나뿐이어야 한다. 유효기간은 다음 날 만료를 기본으로 하되 최소 60분·최대 48시간으로 제한하고, R2·DNS·Zone·KV·route 등 다른 권한은 포함하지 않는다. 임시 Keychain 항목을 새로 만들 때만 허용하고 익명 FD 3으로 허용된 staging 작업에 넘긴 뒤 Wrangler 자식만 token 환경변수를 받는다. 빈 `HOME`/XDG·명시적 account ID·빈 env file·로그/telemetry off로 OAuth fallback을 막고, 활성 상태·시작/만료·token 확인값·exact account·가능한 경우 `wrangler whoami --json`의 Account API Token/계정 1개와 별도 account GET을 claim 전에 확인해야 한다. 이 도구와 status-only recovery가 모두 끝날 때까지 실제 wrapper는 항상 중단한다.
...
5. 정확한 staging 계정과 `Workers Scripts Write` 한 권한에만 묶인 짧은 수명 Account API token을 임시 Keychain→익명 FD 3→격리된 Wrangler 자식 순서로만 전달하고, OAuth fallback 없이 대상 계정과 token 상태를 실행 전에 확인하는 도구를 별도 변경으로 완성한다.
6. 위 두 안전장치와 보호된 서명 증거가 모두 준비되면 계정 주소·설정·실제 차단 코드·적용 version을 생성 전후로 확인하면서 외부 요청을 모두 거부하는 시험용 Worker를 최초 생성한다.
