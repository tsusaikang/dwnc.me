...
최종 갱신: 2026-09-07 KST — 티스토리형 작성 화면 운영 적용 완료 / 관리자 재로그인 후 실제 화면 확인 대기
...
- 실제 화면 확인 상태: 메인이 새 운영 관리자 탭을 열었으나 Cloudflare Access 이메일 OTP 로그인 화면이 나타났다. 사용자에게 자체 내장 브라우저 재로그인을 요청했고 글 선택·서식·미리보기의 운영 확인은 로그인 완료 후 이어간다. 적용 자체는 완료됐으며 재배포할 필요가 없다. 실제 글을 변경하지 않는다.
...
- 다음 단계: 자체 내장 브라우저에서 사용자 관리자 로그인이 끝나면 새 운영 화면의 목록·편집 이동/서식 도구/저장 안내·미리보기를 읽기로 확인하고 공식 상태를 마감한다. 재배포와 실제 글 수정은 하지 않는다. `DWNC-CORE-004`는 운영 적용 완료·실제 화면 확인 대기로 현재 `in-progress`다.
...
비개발자용 현재 요약: 티스토리를 참고한 새 작성 화면을 승인 후 실제 관리자에 적용했다. 관리자 로그인이 만료되어 사용자 재로그인 후 실제 화면 확인만 남았다. 공개 블로그와 이메일 OTP 관리자, 작업본 자동저장·명시적 공개 반영과 실패·재로그인·충돌 처리는 운영 중이다. 기본 서식 도구도 실제 관리자에 적용해 문단·크기·강조·색·정렬·목록·인용·링크·표를 지정할 수 있다. 기존 글 서식 보존과 저장·미리보기·공개·재열기는 로컬 합성 시험을 통과했고 운영에서는 기존 글과 이미지·서식 도구·공개 화면을 읽기로 확인했다. 실제 글과 사진은 변경하지 않았다. 다음은 재로그인 후 새 운영 작성 화면의 읽기 확인이며, 이후 기존·신규 공개 글 화면 통합과 대표이미지 연결이다.
...
- 작업본 자동저장·명시적 공개 반영은 운영에 적용됐고 해당 최우선 작업의 장애물은 없다. 기본 서식 기능과 좁은 글 목록/편집 이동 보완까지 운영 적용했다. 이전 운영 화면은 실제 700×720에서 확인했고, 새 로컬 작성 화면은 1280/700/390px에서 확인을 완료했다. 새 작성 화면의 운영 적용은 승인 후 완료했으며 관리자 재로그인 후 실제 화면 확인이 남았다. 공개 화면 통합·대표이미지 연결과 아래 후속 문제도 남아 있다. 실제 글의 수정·발행·삭제는 운영 확인에 사용하지 않았다.
...
2. `PLAN-09`의 `DWNC-CORE-002`는 로컬 구현·실행 시험·빈 작업본 테이블 추가·관리자 운영 활성화·실제 화면 읽기 확인까지 완료했다. 이어 요청받은 `DWNC-CORE-003` 기본 서식도 로컬 구현·시험과 승인된 형식 열 추가 → 공개 Worker → 관리자 Worker 적용, 자체 내장 브라우저 확인까지 완료했다. 로그인 동의 차단은 해소됐고 인증 메모리 세션도 종료했다. 바로 다음은 `DWNC-CORE-004` 새 운영 작성 화면을 관리자 재로그인 후 읽기로 확인하는 것이며, 이후 공개 글 화면 통합과 대표이미지 연결을 진행한다. 앞선 작업본 CMS 적용에서 공개 Worker·DNS·route·기존 글·R2 객체와 Git 원격은 변경하지 않았다. 운영에서 열어 둔 이전 관리자 화면은 미저장 입력을 처리한 뒤 새로 열어야 새 UI를 사용한다.
