...
비개발자용 현재 요약: 공개할 사진과 GIF 2,758개를 시험용 비공개 저장소에 모두 올렸고, 빠짐·내용 차이·불필요한 파일은 0개였다. 기존 파일을 덮어쓰거나 삭제하지 않았다. 실제 저장소를 다시 읽기 전, 로컬 원본 2,758개, 약 2.35GB로 인터넷 없는 예행연습도 통과했다. 파일 목록을 3회에 나눠 읽고 각 파일의 정보와 내용을 한 번씩 확인해 모두 5,519회 읽었으며 추가·변경·삭제는 0회였다. 이 예행연습은 실제 Cloudflare 파일을 확인한 결과는 아니므로, 다음에는 저장소의 파일을 하나씩 다시 내려받아 로컬 원본과 같은지 확인한다. 이후 시험용 사이트 프로그램을 처음 만들 때 잘못된 계정 주소·로그 설정·프로그램 파일이면 자동으로 멈추는 로컬 검사도 보강했다. 예전 주소 349개를 새 주소로 안내하는 동작은 사이트 프로그램이 직접 처리하도록 만들었고 로컬에서 두 가지 방식으로 총 698회 확인했다. 앞으로 이 프로젝트의 브라우저 작업은 Codex 자체 브라우저에서만 하며 Chrome과 Edge는 사용하지 않는다. 자체 브라우저에서는 잘못 들어간 계정에서 로그아웃했고 현재 Cloudflare 공식 로그인 화면에서 대기 중이다. 다음에는 사용자가 올바른 계정으로 직접 로그인한다. 아직 시험용 사이트 프로그램을 온라인에 만들거나 올려서 켜지 않았고, 실제 `dwnc.me` 주소가 가리키는 곳도 바꾸지 않았다. 이번 변경은 로컬 파일 12개에만 한정됐고 하나의 저장 이력으로 정리했으며, 저장 직후 파일 상태도 깨끗했다. 인터넷 저장소로 보내지는 않았다.
...
63. 앞으로 이 프로젝트의 모든 브라우저 작업은 Codex 자체 브라우저에서만 하고 Chrome과 Edge를 사용하지 않는다. 자체 브라우저의 로그인이 풀렸을 때만 로그인 페이지를 열며 사용자가 직접 로그인한다. 로그인 정보와 비밀번호는 문서·로그·출력·파일에 남기지 않는다. 다음 계정 번호 확인도 자체 브라우저에서 올바른 Cloudflare 계정을 먼저 확인한 뒤 `Account ID` 옆 `Copy`를 한 번만 누른다. 이 결정은 상시 요구사항 `DWNC-OPS-006`으로 관리한다.
...
1. `cloudflare:account-target:preflight`는 이미 준비 완료다. Codex 자체 브라우저에서는 잘못 들어간 계정에서 로그아웃했고 Cloudflare 공식 로그인 화면에서 대기 중이다. 다음 사용자 동작은 **올바른 계정으로 직접 로그인 → Account ID 옆 Copy 한 번**이다. 그 직후 `cloudflare:account-target:init`으로 계정 번호를 전용 보관함에 저장한다. Chrome과 Edge는 사용하지 않으며 실제 저장·초기화는 아직 0회다.
