...
최종 갱신: 2026-09-04 KST — PLAN-05 완료 유지 / PLAN-06 v3 승인 실행 실패·정리 완료, observer 보강과 새 승인 대기
...
비개발자용 현재 요약: 공개할 사진과 GIF 2,758개를 시험용 비공개 저장소에 모두 올렸고, 저장소가 외부에 공개되지 않은 상태와 실제 파일 내용 전체를 확인했다. 보존된 영수증을 현재 원본 목록과 안전 계약으로 다시 검사한 결과 2,758개·2,346,220,246바이트·불필요한 파일 0개와 전체 내용 확인값이 일치했다. 감사용 token 정리까지 마쳐 `PLAN-05`는 완료 상태를 유지한다. `PLAN-06 / DWNC-S3-009`의 승인된 v3 관리 token은 생성·복사·direct stdin 초기화를 각각 한 번 수행했지만 정상 결과 없이 끝났다. 메인 관측기가 실패 결과가 나오는 통로가 아닌 정상 결과 통로를 해석해 결과 형식 오류로도 표시했으며, 실제 실패 결과를 읽지 않아 허용 오류 다섯 가지 중 어느 것인지 특정할 수 없다. 성공 영수증과 Worker 생성은 없고 재시도 없이 token 삭제·목록 부재·clipboard clear·로컬 기록 부재까지 정리했다. bootstrap·version·deployment·공개 endpoint·DNS·route·traffic·R2 변경은 모두 0회다. `DWNC-S3-009`는 미완료·NO-GO이고, observer 보강과 새 사용자 승인 전에는 다시 실행하지 않으며 `DWNC-S3-010`은 계속 차단한다.
...
- `PLAN-06` 시험용 사이트 프로그램·버전·사이트 점검: **진행 중**. 승인된 `DWNC-S3-009` v3 실행은 exit 1로 끝났지만 메인 관측기가 실패 결과용 stderr가 아니라 정상 결과용 stdout을 해석해 추가 형식 오류로 표시했다. 실제 stderr를 해석하지 않아 허용 오류 다섯 가지 중 정확한 원인은 특정할 수 없다. 자동 재시도 없이 원격 token 삭제·목록 부재·clipboard clear·로컬 기록 부재까지 정리했고 승인 묶음은 소진됐다. 다음 observer 보강과 새 승인 전에는 재실행하지 않는다. `DWNC-S3-009`는 미완료·NO-GO이고 `DWNC-S3-010`은 차단 상태다.
...
인수인계 상태: `PLAN-05`는 완료 상태를 유지한다. `PLAN-06 / DWNC-S3-009`의 승인된 v3 실행은 원인을 특정하지 못한 exit 1과 관측 채널 선택 오류 뒤 원격·로컬 terminal cleanup을 완료했고 재시도는 0회다. 이번 승인 묶음은 소진됐다. observer를 보강하고 새 승인을 받기 전에는 새 token 생성·전달·초기화를 하지 않으며, `DWNC-S3-009`는 미완료·NO-GO이고 `DWNC-S3-010`은 차단 상태다.
...
- 현재 staging 최초 service는 signed 존재 증거와 one-time authorization을 요구하는 외부 요청 0 deny-only bootstrap으로 분리했다. 최초 읽기 전용 확인은 raw Cloudflare JSON과 5분 evidence 후보를 함께 보존하고, executor는 생성 직전 15초 이내의 service-not-found 404(code 10007/10090)만 다시 인정한다. 계정의 `workers.dev` 이름도 별도 API로 생성 전·직전·생성 뒤 읽어 정확히 `dwnc`인지 계정 확인값과 함께 허가·결과에 묶는다. deny 설정은 공개 주소·미리보기·Logpush·tail·tag·trace·외부 log destination을 끄고 observability/logs를 정해진 값으로 고정한다. API가 `logs.persist`를 생략한 경우에만 기본 `true`로 인정하고 명시적인 `false`·`null`은 거부한다. 생성 뒤 현재 deployment를 먼저 읽고, deploy 가능한 version 목록·account subdomain·script subdomain·`/script-settings`·`/content/v2`·version detail을 읽은 뒤 deployment를 다시 읽는 8개짜리 상태 묶음을 연속 두 번 확인한다. 모든 GET은 압축하지 않은 응답만 요구하고 정상·오류 응답 모두 최대 1MiB까지만 읽으며, 원본은 저장소 밖 고정 primary/recovery 위치에 create-only로 보존한다. 표준 multipart 표현 차이는 허용하지만 실제 한 module 바이트·SHA-256은 로컬 차단 코드와 같아야 하며, 같은 version 100%라도 deployment ID나 목록이 중간에 바뀌면 거부한다. 결과 기록은 단순 boolean이 아니라 생성 직전 두 확인, Wrangler 원본 결과, 생성 뒤 두 상태의 총 5종 원본 자료를 고정 위치에서 다시 읽어 역할·요청 확인값·status·시각·byte 수·SHA, module SHA·바이트 수, deployment/version과 결속한다. 실제 생성 부분은 외부 기능 기본값이 없는 별도 실행 코어로 분리했다. 최초 API 전에 1회용 허가와 준비·시작·결과·상태 기록, 원본·후보 primary/recovery 등 고정 출력 18개의 안전성과 부재를 모두 확인하고, 첫 GET 직전·허가 claim 직전·claim 뒤 생성 명령 직전에 승인 만료·exact Git HEAD·tracked/일반 untracked clean 상태를 세 번 확인한다. 실제 실행 흐름 시험은 가짜 API 213회·가짜 Wrangler 8회로 정상 흐름, 기존·부분·recovery 출력, 확인 또는 claim 중의 Git 변경·승인 만료 경합, 임시 출력 닫기·삭제 실패를 포함한 중단 경계를 끝까지 검사했다. staging smoke token은 암호학적 난수 32바이트를 padding 없는 base64url 43문자로 만들고, 평문을 argv·env·receipt에 노출하지 않는다. staging media·release signing key는 macOS Keychain에 생성했지만 token·endpoint·version·receipt는 아직 없다. 최초 생성 명령 성공 뒤 결과 기록 전에 중단되더라도 같은 명령을 반복하지 않고 GET만으로 상태 A·B를 대조하는 복구 절차를 완성했다. 준비만 한 상태, 시작 뒤 Worker가 없는 상태, 정확한 차단 Worker를 복구한 상태, 판단할 수 없는 상태를 구분하고 마지막 상태에서는 자료를 보존한 채 중단한다. 정확한 staging 계정 한 곳의 `Workers Scripts Edit` 한 권한만 가진 짧은 수명 Account API token을 Keychain→익명 FD 3→격리된 Wrangler 자식으로 전달하는 로컬 바탕과 bootstrap·복구 runner 연결을 완성했다. 아직 연결하지 않은 staging version upload·상세 확인·적용·workers.dev 활성화 명령은 token을 읽기 전에 중단한다. 직접 실행 코어에는 OAuth·`HOME`·live network/명령 기본값이 없다. staging은 production과 다른 exact version ID를 fresh status CAS·pending/invoking/outcome lock 아래 100% 활성화하며 ambiguous 결과는 owner-dead·5분 뒤 status-only recovery에서만 확정한다. Bearer로 보호한 `workers.dev` endpoint에서 redirect GET·HEAD 698건, `/about` GET·HEAD 2건, `/404.html` GET·HEAD 2건, media GET·HEAD·304·206·416 5건, cache 1~3건과 미인증 차단 1건을 정확히 세고 모두 candidate version인지 증명한다.
...
- bulk 안전장치와 실행 기준은 commit `ccec8bb855e45ef679a4b99faf0f0633a99e089e`·tree `241a654acc387233d4e2d5e076754b345619e9ac`에 기록했고 push는 0이다. private-exposure canonical API evidence와 실제 staging R2 전체 2,758개 full GET/SHA-256 감사는 완료됐다. staging Worker/version upload·activation·smoke와 `PLAN-06`용 receipt 서명·소비 검사는 미완료다.
...
- 실제 staging R2의 비공개 상태와 전체 2,758개 GET/SHA-256 및 `PLAN-05` token 정리는 완료됐다. `PLAN-06 / DWNC-S3-009`의 승인된 v3 실행은 원인을 특정하지 못한 exit 1 뒤 관리 token 삭제·목록 부재·clipboard clear·로컬 기록 부재까지 정리했다. 현재 확정된 P0은 exit 0이면 stdout, exit 1이면 stderr를 제한된 크기로 해석하고 schema를 확인하도록 메인 observer를 보강하는 일이다. 새 승인과 `PLAN-06` receipt 서명·start-only freshness 소비 규칙도 미해결이다.
...
- 승인된 v3 실행에서는 `dwnc-me-staging-worker-control-20260904-v3`를 Workers Scripts Write·2026-09-05 만료·All IPs로 UI에서 한 번 생성하고 browser copy 1회→direct stdin 초기화 1회를 수행했다. 호출자는 정확히 53바이트를 전달한 뒤 자체 buffer를 0으로 덮고 clipboard를 비웠으며 overflow는 없었다. 직접 관측값은 child exit 1·stderr 296바이트와 메인 observer의 stdout JSON parse 실패이며, stderr 내용은 읽거나 해석하지 않았다. runner 정적 계약상 처리된 exit 1 실패 JSON은 stderr로 나가고 stdout은 비지만, 296바이트가 될 수 있는 허용 오류는 `CLOUDFLARE_E_ACCOUNT_STORE_KEYCHAIN`, `CLOUDFLARE_E_ACCOUNT_STORE_LOCATION`, `CLOUDFLARE_E_ACCOUNT_STORE_METADATA`, `CLOUDFLARE_E_STAGING_CONTROL_EXISTS`, `CLOUDFLARE_E_STAGING_CONTROL_VERIFY` 다섯 가지이므로 이번 오류는 특정할 수 없다. 성공 영수증·재시도·외부 bootstrap은 0회다. 해당 exact UI 행은 즉시 한 번 삭제했고 즉시·새로고침 후 모두 0건, 목록은 기존 두 행으로 돌아왔다. 하위 감사에서 `durableLocalState=none`, control-token primary/recovery metadata와 고정 Keychain 항목은 모두 부재였고 Account ID·token 원문 read/output은 0회였다. 감사 자체의 외부 write/delete·파일 수정·retry도 0회이며 authorization creator는 사용하지 않았다. 이번 승인 묶음은 소진됐고 `DWNC-S3-009`는 미완료·NO-GO, `DWNC-S3-010`은 차단 상태다.
- staging Worker·endpoint가 없으므로 실제 Worker GET/HEAD/Range 점검은 아직 실행하지 않았다. 로컬 점검 프로그램의 안전 보강은 완료했다. 실행 전에 기준 주소 목록과 봉인 파일을 확인하고, 그 뒤에만 임시 출입 비밀번호를 읽는다. 각 응답은 시간과 크기를 제한하고 필요 없는 본문을 닫는다. 점검 항목은 예전 주소 GET·HEAD 698건, `/about` GET·HEAD 2건, `/404.html` GET·HEAD 2건, media 5건, cache 1~3건, 무인증 차단 1건이다. 첫 cache 요청이 성공하면 총 709건이다. `/about`과 cache 사진의 실제 내용·크기·파일 종류, 304·416의 ETag·크기 표시·빈 본문, 모든 인증 응답의 staging version 표시, 예전 주소의 상대 새 주소·빈 본문·query 제거, 404 no-store와 각 요청 수가 맞아야 한다. bootstrap→version-only upload→100% activation→임시 비밀번호로 보호한 `workers.dev` 점검을 승인된 순서대로 이어가며, 중간에 단순히 추가 승인을 받기 위해 멈추지 않는다.
...
2. `PLAN-06 / DWNC-S3-009`의 v3 승인 묶음은 원인을 특정하지 못한 exit 1과 terminal cleanup으로 종료됐다. 다음 시도 전 메인 observer가 exit 0의 stdout과 exit 1의 stderr를 각각 제한된 크기로 해석하고 고정 schema를 확인하도록 보강한다. 나머지 원인과 수정 범위는 그 결과로 판단하며, 새 사용자 승인을 받은 뒤에만 새 token 생성·direct stdin 초기화·bootstrap을 다시 진행하고 실패하거나 결과가 모호하면 재시도하지 않는다.
   terminal local cleanup은 새 도구를 만들지 않는다. Cloudflare UI에서 exact token 행을 한 번 삭제하고 새로고침 뒤 부재가 확인된 경우에만 고정 service `me.dwnc.cloudflare-staging-worker-control.v1`·account `dwnc:staging:workers-scripts-edit`의 `security delete-generic-password`를 정확히 한 번 실행한다. 이어 같은 identity를 `-w` 없이 presence-only로 조회해 exit 44를 확인하며 primary/recovery metadata와 immutable evidence는 보존한다. 원격 삭제 대상이나 부재가 모호하면 local delete는 0회다.
3. `DWNC-S3-009` 완료 전에는 `DWNC-S3-010`을 시작하지 않는다.
4. 시험용 확인이 통과하면 최종 운영에 실제 필요한 Cloudflare 자원만 같은 방식으로 준비한다. 새 보조 도구·복구 체계·서명·영수증·장애주입·모의훈련·반복 독립감사는 추가하지 않는다. 새 글 작성 방식과 비공개 백업은 최종 운영 전에 정하되 Stage 3를 막지 않으며, 공유 글 10개·댓글·추가 개선은 사용자가 원할 때만 진행한다. 실제 `dwnc.me` 도메인·DNS·route 연결은 사용자가 결정할 때만 수행한다.
