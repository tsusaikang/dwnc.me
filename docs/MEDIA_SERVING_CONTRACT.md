...
- 신규 이미지 준비 정책(`DWNC-CORE-031`, 로컬 브라우저·관련 회귀·빌드·양쪽 배포 및 운영 화면 반영 확인 완료): 파일 첨부와 클립보드는 공통 `uploadImage`에서 `src/lib/admin-image-upload.ts`의 변환 결과를 업로드한다. 사용자 로컬 파일·기존 이관 원본·기존 저장 객체는 변경하지 않는다. 이는 새 업로드 바이트 준비 규칙이며 기존 manifest 파일의 원본 불변 계약과 구분한다.
- 정지 불투명 이미지는 브라우저 JPEG 품질 `0.85`, 최대 16,000,000픽셀로 종횡비를 유지해 처리하며 확대하지 않는다. 해상도를 줄이지 않고 JPEG 결과가 입력보다 같거나 크면 입력을 유지한다. 이 예외는 작은 불투명 PNG에도 적용하며 불필요한 팽창·추가 손실을 막는다. 파일당 1MB나 특정 평균 크기는 보장하지 않는다.
- JPEG 외 정지 이미지의 투명도는 축소 전에 원래 해상도를 최대 1024×1024 타일로 읽어 확인한다. 실제 투명 PNG는 16MP 이내이면 입력 바이트를 유지하고, 더 큰 경우 비율을 줄인 PNG로 보존한다. 다른 정지 투명 이미지는 PNG로 출력한다. 16비트 PNG에서 알파/tRNS로 투명도를 표현하는 경우 8비트 canvas 반올림 오판을 피하기 위해 원형을 유지한다.
