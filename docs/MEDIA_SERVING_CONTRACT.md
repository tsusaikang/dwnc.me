...
- 2026-09-07 로컬 작업본 CMS 구현은 `editor_working_copies`에만 자동저장하고 기존 공개 테이블을 명시적 공개 반영 때만 갱신한다. 따라서 작업본에 새로 참조한 이미지는 아직 공개하지 않고, 작업본에서 참조를 지운 기존 공개 이미지도 공개 반영 전까지 유지한다. 공개 Worker의 기존 참조 검사는 변경하지 않는다. 운영 적용은 별도 승인 전까지 미실행이다.
