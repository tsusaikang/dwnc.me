...
private-exposure 설정 확인에 쓰려고 만든 읽기 전용 API token 두 개는 안전한 전달에 실패해 각각 API GET 0·capture 0 상태로 폐기했다. 이후 계정 번호와 짧은 읽기 전용 열쇠를 서로 분리하고, 각 값을 macOS 클립보드에서 한 번만 읽은 직후 지우는 로컬 전달 방식을 구현했다. 전용 계정 번호 보관·복구 시험 123개와 control-plane 전달 시험 81개, 기존 private-exposure 시험 145개가 모두 통과했다. 여기에 시험용 Worker 관리 열쇠 보관·복구·계정 확인·FD 전달·격리된 Wrangler 시험 378개, 중단 복구 1,023개, 실제 runner→복구 자식 139개를 추가했다. `npm run cloudflare:test`가 각 묶음의 실제 PASS와 assertion 수를 자동 집계한 최신 전체 Cloudflare 로컬 검사는 26개 묶음·8,641개를 통과했다. 실제 Keychain·클립보드·Cloudflare·R2 접근 0회는 각 전용 시험이 별도로 센 결과이며 token 값과 raw account ID는 기록하지 않았다. 실제 계정 번호 저장, 새 token 생성과 API 조회는 아직 하지 않았다.
