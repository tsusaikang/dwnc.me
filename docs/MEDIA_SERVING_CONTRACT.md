...
- 신규 이미지 준비 정책(`DWNC-CORE-031`, 구현 후 브라우저·빌드·배포 확인 중): 파일 첨부와 클립보드는 공통 `uploadImage`에서 `src/lib/admin-image-upload.ts`의 변환 결과를 업로드한다. 사용자 로컬 파일·기존 이관 원본·기존 저장 객체는 변경하지 않는다. 이는 새 업로드 바이트 준비 규칙이며 기존 manifest 파일의 원본 불변 계약과 구분한다.
