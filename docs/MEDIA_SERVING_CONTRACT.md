...
- 공개 URL `/media/...`는 유지한다.
- 이 절의 정적 manifest 미디어는 R2 object key가 URL에서 선행 `/` 하나만 제거한 값이다: `key = publicPath.slice(1)`. 웹 편집기 native 미디어는 아래 D1 매핑 규칙을 따른다.
...
- 신규 이미지 준비 정책(`DWNC-CORE-032`, 운영 반영 상태는 공식 상태 참조): 파일 첨부와 클립보드는 공통 `uploadImage`에서 `src/lib/admin-image-upload.ts`의 변환 결과를 업로드한다. 사용자 로컬 파일·기존 이관 원본은 변경하지 않는다. 기존 native 사진 최적화는 아래 승인 범위의 별도 매핑 교체로 수행하며 정적 manifest 원본 불변 계약과 구분한다.
- 정지 불투명 이미지는 JPEG 품질 `80`(canvas 경로 `0.8`), 긴 변 최대 2560픽셀로 종횡비를 유지해 처리하며 확대하지 않는다. 해상도를 줄이지 않고 JPEG 결과가 입력보다 같거나 크면 입력을 유지한다. 이 예외는 작은 불투명 PNG에도 적용하며 불필요한 팽창·추가 손실을 막는다. 파일당 1MB나 특정 평균 크기는 보장하지 않는다.
- JPEG는 브라우저 module Worker의 libultrahdr/libjpeg WASM으로 처리한다. sRGB 전달곡선의 sRGB/P3 JPEG를 지원하며 검증된 SDR 기본사진·단일채널 gain map HDR의 ICC·원본 표시 밝기 범위를 유지한다. HDR/SDR 양쪽을 선형광 Lanczos로 줄인 뒤 gain map을 재생성한다. 지원 밖 ICC/HDR·손상·메모리 제한은 오류로 중단하고 일반 SDR로 우회하지 않는다. JPEG 50MP 입력 상한·25MiB 제한, 한 장씩 처리·120초 시간 제한·Worker 종료를 적용한다. [구현과 지원 범위](../scripts/image-codecs/README.md).
- JPEG 외 정지 이미지의 투명도는 축소 전에 원래 해상도를 최대 1024×1024 타일로 읽어 확인한다. 실제 투명 PNG는 긴 변2560px 이내이면 입력 바이트를 유지하고, 더 큰 경우 비율을 줄인 PNG로 보존한다. 다른 정지 투명 이미지는 PNG로 출력한다. 16비트 PNG에서 알파/tRNS로 투명도를 표현하는 경우 8비트 canvas 반올림 오판을 피하기 위해 원형을 유지한다.
- GIF·APNG·애니메이션 WebP·애니메이션/시퀀스 AVIF는 컨테이너 기록으로 판별해 원형을 보존한다. 첫 프레임만 JPEG로 바꾸지 않는다. 정지 WebP·AVIF는 위 투명도·압축 규칙에 따른다.
- 업로드 입력 한도 25MiB는 변환 전에도 유지한다. 읽기·형식 검사·canvas 변환에 실패하면 해당 파일은 업로드하지 않고 원본 파일은 바뀌지 않았다는 오류를 표시한다. 변환 후에도 25MiB 한도를 검사하며 최종 파일의 바이트를 기준으로 MIME·확장자·크기·SHA-256을 맞춘다. 원본과 변환본을 중복 업로드하지 않는다.
...
- 허용 형식은 AVIF·GIF·JPEG·PNG·WebP, 한 파일 최대 25MiB다. 브라우저가 최종 업로드할 파일 바이트로 계산한 SHA-256을 R2 checksum으로 검사하고, 응답 size와 metadata가 맞은 뒤 D1의 해당 글에 media row를 추가한다.
...
- 기존 key와 새 native key 모두 덮어쓰지 않는다. 일반 업로드는 원본 객체·실패 객체를 자동 삭제하지 않는다.
- 사용자 승인에 따른 기존 native 사진 최적화는 새 R2 객체에 변환본을 준비한 뒤 `native_media`의 `object_key/sha256/bytes/mime`만 기존 값 조건부로 교체할 수 있다. `id/public_path`·소유 글·대체텍스트·생성일·글 본문·반영본/작업본·revision·공개정책을 그대로 둔다. 새 객체는 R2 MIME·크기·custom metadata·SHA checksum을 기존 서빙 검증과 일치시킨다. 한 대상이라도 메타데이터가 달라지면 전체 교체를 중단한다. 제공 응답은 `no-store`이며 이전에 공개되지 않은 사진을 새로 공개하지 않는다. 오래된 객체 제거는 실제 정상 교체 확인과 승인 범위에 한정하며 무관한 orphan을 삭제하지 않는다.
