...
- 생성 뒤 [현재 deployment 목록](https://developers.cloudflare.com/api/resources/workers/subresources/scripts/subresources/deployments/methods/list/)을 먼저 읽고, account subdomain·script 공개/미리보기 상태·전체 script settings·content·version detail 다섯 응답을 읽은 뒤 deployment를 다시 읽는다. 이 7개짜리 묶음을 연속 두 번 확인한다. 각 묶음 안에서 앞뒤 deployment 원문과 ID·strategy·전체 version 배분이 같아야 하고, 두 묶음의 의미 상태도 같으며 승인된 하나의 version ID가 100%일 때만 기록을 만든다. 같은 version 100%여도 deployment ID가 바뀌거나 중간 목록이 잠깐 달라졌다 돌아오면 실패한다.
...
- 실제 생성 이후의 전체 흐름은 별도 실행 코어로 분리했다. 코어는 network·명령 실행·시각·Git 검사·보호 파일 검사와 읽기/쓰기·임시 파일 기능을 모두 호출자가 빠짐없이 전달해야 하며, 실제 기능으로 자동 대체하거나 OAuth·`HOME`에 기대지 않는다. 빈 값·일부만 전달한 경우 첫 API와 Wrangler 실행 전에 거부한다. 정상 흐름은 생성 직전 GET 2회, Wrangler 생성 명령 정확히 1회, 생성 뒤 7개 응답 묶음 2회로 고정한다. 1회용 허가 파일과 원본 5종의 primary/recovery, 최종 후보까지 현재 절차가 만들 수 있는 고정 출력 13개는 첫 API 전에 모두 안전성과 부재를 확인한다. 생성 직전 두 원본을 쓴 뒤에도 아직 만들지 않은 출력과 recovery 자리를 다시 확인한다. 기존 후보·부분 파일·recovery·허가 파일이나 안전하지 않은 상위 폴더가 있으면 API·Wrangler를 0회로 유지한다.
...
- 가짜 API·가짜 Wrangler와 프로젝트 밖 임시 보호 폴더로 정상 완료, 비밀값 유입, 생성 도구 실패, 1MiB 초과 출력, 생성 뒤 설정 불일치, entrypoint 불일치, 모든 고정 출력 충돌, Git dirty·중간 변경·허가 만료, 임시 파일 닫기·삭제 실패를 끝까지 시험했다. 로컬 모의 전체 Cloudflare 시험 23개·5,689개 확인이 통과했고, 이 중 최초 생성 검증·실행 코어 시험은 4,126개다. 실행 코어 시험은 가짜 API 85회·가짜 Wrangler 8회를 사용했으며 실제 network·Cloudflare 변경·배포는 0회다.
- 아직 남은 별도 `P0`는 생성 명령이 성공한 뒤 결과 기록 전에 작업이 끊기는 상황의 status-only 복구다. 이 경우 같은 생성 명령을 다시 실행하지 않고 현재 상태만 읽어 기존 시도를 확정하거나 중단해야 한다. 다음 직렬 구현에는 이 복구와 별도로, 정확한 대상 계정 한 곳에만 유효하고 Cloudflare 공식 명칭 [`Workers Scripts Write`](https://developers.cloudflare.com/fundamentals/api/reference/permissions/) 권한 하나만 가진 짧은 수명의 Account API token 전달 도구가 필요하다. token은 다음 날 만료를 기본으로 하되 총 유효시간을 최소 60분·최대 48시간으로 제한한다. R2·DNS·Zone·KV·route 등 다른 권한은 넣지 않고, token은 임시 Keychain 항목에서 익명 FD 3으로 허용된 staging 작업에만 넘기며 Wrangler 자식만 `CLOUDFLARE_API_TOKEN`을 받는다. 자식은 빈 `HOME`/XDG와 명시적 계정 번호·빈 env 파일을 사용해 OAuth fallback을 차단하고, token 활성·시작/만료 시각·확인값·대상 계정과 `wrangler whoami --json`/계정 GET 결과를 먼저 확인해야 한다. 현재 실제 wrapper와 CLI는 이 token 전달 도구와 status-only 복구가 모두 준비되기 전 credential을 읽거나 Cloudflare 요청·생성 명령에 도달하기 전에 항상 `CLOUDFLARE_E_BOOTSTRAP_STATUS_RECOVERY_REQUIRED`로 멈춘다. 실행 코어를 직접 불러도 같은 중단을 우회하는 실제 기능 기본값은 없으며, 환경변수나 실행 옵션으로 우회할 수 없다.
...
- staging Worker가 아직 없으므로 deny-all 최초 생성의 실제 실행, raw service-existence·계정 `workers.dev=dwnc` capture, 두 signed evidence와 실행 직전 fresh capture. 로컬 검증기는 준비됐지만 성공 뒤 기록 전 중단을 다루는 status-only 복구가 먼저 필요함
