...
**`DWNC-CORE-022`의 관리자 목록 날짜·정렬 보완을 진행 중이다.** 생성일과 수정일을 구분하고 각 날짜의 최신순·오래된순 정렬을 추가한다. `DWNC-CORE-012`의 기존 결함6건은 별도 착수 대기다. 진행 중인 계획은 `PLAN-09` 하나다.

사용자에게 이는 글 목록에서 원래 작성된 때와 최근 수정한 때를 구분해 보고 원하는 날짜순으로 찾을 수 있게 바꾸는 중이라는 뜻이다.
...
다음에는 날짜 표시·정렬 구현과 관련 시험, 내장 브라우저 확인을 마치고 기존 Git 자동 배포로 운영에 반영한다. 글의 저장 날짜나 공개 상태는 바꾸지 않는다. 외부 위키와 별도 결함·독립 사본은 기존 후속 조건을 따른다.
