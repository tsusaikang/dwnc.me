...
**`DWNC-CORE-021`은 사용자 추가 수정 승인에 따라 `in-progress`로 재개했다.** 대표27건·공개요약29편을 반영했으며, 사용자가 남은 공개115·비공개178요약과 카드·화면보완·배포 및 운영읽기의 제시된 범위를 명시 승인해 계속 진행한다. `DWNC-CORE-012`는 별도 착수 대기이며 진행 중인 계획은 `PLAN-09` 하나다.
