...
| `PLAN-06` | 실제 도메인과 연결되지 않은 시험용 사이트를 올려 점검한다. | 새 Worker 버전에서 일반 페이지, 예전 주소 349개를 두 가지 요청 방식으로 총 698회, 오류 주소와 사진 응답을 모두 확인한다. | `대기` |
...
**진행 중인 계획은 `PLAN-05` 하나다.** 다음 단계인 `PLAN-06`은 아직 대기 상태지만, 시험용 Worker를 안전하게 만들기 위한 로컬 모의검사는 미리 보강했다. 공개 미디어 2,758개의 업로드와 업로드 뒤 목록 확인은 끝났지만, 모든 파일을 다시 내려받아 원본과 비교하는 검사는 남아 있다. Cloudflare 계정 번호는 사진 업로드용 열쇠와 섞지 않고 Mac의 안전한 보관함에서 불러오도록 만들었다. 이 번호는 로그인 열쇠가 아니라 올바른 계정을 고르는 확인 표지다. 계정 번호 저장 전 준비 확인은 이미 완료됐지만 실제 번호 저장과 초기화는 아직 0회다. 최초 Worker 생성 도중 프로그램이 멈춰도 같은 생성 명령을 다시 실행하지 않고 현재 상태만 두 번 확인하는 로컬 복구 장치도 완성했다. Worker가 예전 주소 349개를 직접 새 주소로 안내하는 기능과 원격 점검 프로그램의 로컬 안전 보강은 완료했다. 실제 시험용 주소에서 이 점검을 실행한 것은 아니다.

사용자에게 이는 **파일은 모두 시험용 저장소에 들어갔지만 실제 내용을 한 개씩 다시 확인하는 마지막 검사가 남았다는 뜻**이다. 시험용 Worker 자체를 올리거나 적용하지는 않았지만, 나중에 만들 때 잘못된 계정·주소·설정·코드가 끼어들면 멈추는 로컬 안전장치는 강화했다.
...
1. 모든 파일의 실제 내용을 확인하기 전에, 저장소가 외부에 공개되지 않았는지 Cloudflare에서 한 번 더 읽어 와야 한다. 이를 위해 계정 번호와 잠깐만 쓰는 읽기 전용 열쇠가 필요하다. 두 값은 서로 다른 용도이며 따로 전달한다.
2. 앞서 만든 읽기 전용 열쇠 두 개는 전달 과정이 안전하지 않아 사용하지 않았고 모두 폐기했다. 두 시도 모두 Cloudflare 정보를 읽은 횟수와 결과 파일 수가 0이다. 새 방식과 계정 번호 저장 전 준비 확인은 로컬 시험을 통과했다. 다음에는 올바른 Cloudflare 계정에서 계정 번호를 한 번 복사해 전용 보관함에 저장하고 새 읽기 전용 열쇠를 사용한다.
...
1. 계정 번호 저장 사전검사는 이미 준비 완료다. 다음 사용자 동작은 올바른 Cloudflare 계정 화면에서 **Account ID 옆 Copy를 한 번만 누르는 것**이다. 그 직후 초기화를 실행해 계정 번호를 Mac의 전용 보관함에 저장한다. 원래 번호는 화면 출력이나 프로젝트 파일에 남기지 않고, 프로젝트에 기록된 계정과 일치하는지만 확인한다.
2. 짧게만 사용할 새 읽기 전용 열쇠를 안전한 전달 방식으로 사용해 저장소가 비공개 상태인지 읽기만 해서 확인하고, 사용한 열쇠는 바로 폐기한다. Cloudflare 화면에서 열쇠 생성이나 폐기 직전에 사용자의 클릭이 꼭 필요할 때만 버튼 이름과 순서를 안내한다.
3. 저장소의 파일 2,758개를 하나씩 다시 내려받아 원본과 내용 및 전체 용량이 같은지 확인한다.
4. 확인이 끝나면 실제 도메인과 연결되지 않은 시험용 주소에 사이트 새 버전을 올린다.
5. 시험용 주소에서 일반 페이지, 기존 주소 349개를 두 가지 요청 방식으로 총 698회, 직접 열 수 없어야 하는 `/404.html`, 이미지의 일반 표시·변경 없음 응답·부분 전송·범위를 벗어난 요청을 점검한다.
6. 운영용 이름의 Cloudflare 자원이 더 필요하면 같은 순서로 준비하되 실제 `dwnc.me` 주소와 DNS는 연결하지 않는다.

사용자가 다음에 할 일은 올바른 Cloudflare 계정에서 **Account ID 옆 Copy를 한 번 누르는 것**이다. 실제 `dwnc.me` 주소 연결은 `PLAN-08`에서 사용자 결정이 있어야만 진행한다. 이미 승인된 시험용 작업은 안전 조건이 충족되면 단순히 승인을 다시 받기 위해 멈추지 않는다.
...
- 시험용 Worker 최초 생성이 중간에 끊겨도 생성 명령을 반복하지 않고 현재 상태만 두 번 읽어 결과를 판정하는 복구 장치를 만들었다.
...
- 계정 번호 전용 사전검사·보관·복구 시험 610개, 읽기 전용 열쇠 전달 시험 81개와 시험용 Worker 관리 열쇠 시험 378개를 통과했다. 중단 복구 전용 1,023개와 실제 runner→복구 자식 139개를 포함해, 자동 집계한 최신 전체 Cloudflare 회귀시험 26개 묶음·15,024개 확인을 모두 통과했다. 개발 중 가짜 연결 누락으로 Mac 보관함 읽기 명령이 실제 1회 실행됐지만 두 번째 읽기·저장·변경·클립보드·Cloudflare·R2 접근은 0회였고, 감시 보강 뒤 추가 실제 시스템 프로그램 실행은 0회였다.
...
- **Evidence:**
  ...
  - 위 로컬 점검 전용 시험 179개와 전체 Cloudflare 시험 26개 묶음·15,024개가 통과했다. 실제 시험용 주소 점검과 Cloudflare 요청은 아직 0회다.
...
- **Priority:** `P0`
- **Acceptance:**
  - 계정 번호는 로그인 열쇠나 사진 업로드용 열쇠가 아님을 사용자에게 쉽게 설명하고, 업로드용·검사용 열쇠와 다른 macOS 보관 항목에 둔다.
  - 먼저 계정 번호를 복사하지 않은 상태에서 저장 위치와 기존 보관 상태를 검사한다. 이 사전검사는 클립보드를 읽거나 지우지 않고, 여기서 실패하면 기존 클립보드 내용을 그대로 보존한다. 준비 완료 뒤 초기화가 계정 번호 읽기를 시도한 경우에만 클립보드를 즉시 비운다. 프로젝트에 기록된 올바른 계정과 일치할 때만 새 항목으로 저장하며, 기존 항목·파일은 덮어쓰거나 삭제하지 않는다. 저장된 번호를 사용할 때는 Mac 보관함 값을 확인 파일 재검사 전과 후에 두 번 읽어 같은지 확인한다. 그 사이 값이 바뀌거나 사라지면 Cloudflare 조회, 새 열쇠 읽기, 하위 작업 시작과 새 파일 쓰기를 모두 하지 않고 멈춘다.
  - 프로젝트와 일반 작업 기록에는 원래 계정 번호를 남기지 않는다. 보호된 확인 파일에도 원래 번호 대신 확인값, 용도와 생성 시각만 기록한다. 기본 위치와 사용자가 명시한 위치 모두 실제 경로가 프로젝트 밖이어야 하며, 프로젝트 안 경로와 다른 위치를 가리키는 중간 폴더 연결은 계정 번호나 열쇠를 읽기 전에 거부한다.
  - Cloudflare 정보를 읽는 작업은 부모 환경에서 계정 번호를 받지 않고, 확인 파일을 다시 읽기 전과 후에 Mac 보관함의 전용 항목을 두 번 확인한다. 사진 업로드용·검사용 열쇠는 읽지 않으며, 원래 번호와 짧은 읽기 전용 열쇠를 명령어나 화면 출력에 넣지 않는다.
  - 저장 뒤 확인 파일이 비어 있거나 일부만 남아도 Keychain 항목과 실패 파일을 지우거나 덮어쓰지 않는다. 새 클립보드 입력 없이 첫 파일 이름에서 자동으로 정해지는 단 하나의 두 번째 확인 파일만 만들어 안전하게 복구하며, 다른 이름을 대신 넣을 수 없다. 두 확인 파일이 모두 불완전하면 새 파일 이름을 계속 만들지 않고, 기존 파일과 보관 항목을 보존한 채 사용자의 판단이 필요한 상태로 멈춘다.
- **Evidence:**
  ...
  - 계정 번호를 받기 전의 저장 준비 확인은 이미 완료됐다. 실제 Keychain 항목과 확인 파일 초기화, 계정 번호 저장, Cloudflare 조회는 아직 모두 0회이므로 상태는 진행 중이다. 다음 순서는 올바른 Cloudflare 계정의 **Account ID 옆 Copy를 한 번 누르기 → 초기화**다.
...
- **Evidence:**
  ...
  - 현재 Cloudflare 관련 26개 시험 묶음·15,024개 확인, 정적 페이지 1,404개, redirect 349개, request surface 1,409경로·SHA-256 `e143cefe01de35de47495e3fbd036773eb550e58f377e07467014d45f944d594`가 통과했다. `/404.html`은 내부 오류 화면으로는 남지만 공개 요청 목록에는 들어가지 않는다.
