...
사용자가 두 수정의 실서비스 반영을 승인했고, 앞으로도 요청받은 수정은 반영과 실제 확인까지 완료하도록 확정했다. 새 배포 파일과 검사는 준비됐다. 기존 배포 연결의 계정 정보 조회·사이트 프로그램 수정·데이터베이스 수정 권한 동의가 자동 승인 검토에 차단되어 해당 범위만 사용자 동의를 기다린다. 이번 배포에서 실제 글·사진·데이터베이스 변경은 없고 사이트 프로그램만 반영한다.

공개 카테고리 이름이 번호용 좁은 칸에 들어가 세로로 줄바꿈되는 문제와 분리된 `갈래 +` 버튼을 로컬에서 보완했다. 기존 계층 카드·메뉴 구조에 맞추며 분류 자료와 주소는 유지한다. 관리자에 없는 `/archive` 주소에는 내부 JSON 대신 관리자 홈·공개 전체 글 이동 안내를 구현했다. 관련 시험·독립 검토와 내장 브라우저에서 제목 가로 표시·하위 갈래 펼치기·관리자 홈 이동을 확인했다. 두 수정 모두 아직 운영 미반영이며 기존 운영 기능은 유지한다.
...
공개 카테고리 표시와 잘못 입력한 관리자 주소 안내의 새 빌드까지 완료했다. 연결 권한에 대한 동의를 받으면 기존 배포 방식으로 두 프로그램을 반영하고 내장 브라우저에서 카테고리·관리자 안내를 확인한다. 이번 수정은 아직 운영 미반영이다.
