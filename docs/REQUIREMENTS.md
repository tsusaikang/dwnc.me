...
현재 콘텐츠 이전과 실제 도메인 연결, 기본 웹 편집기 운영 반영까지 완료됐다. **공개 블로그와 이메일 OTP 관리자 화면은 운영 중이지만 지속 운영용 CMS는 보완 중이다.** 기존 글 편집·새 글 발행과 이미지/링크 카드 선택·삭제는 사용할 수 있다. 작업본 자동저장과 명시적 공개 반영을 분리하고 실패·로그인 만료·충돌에도 작성 내용을 유지하는 기능의 로컬 구현·시험을 완료했다. 실제 글·원본·R2 객체와 운영 서비스는 변경하지 않는다. 과거 운영 전환과 비용 확인 이력은 [`PROJECT_STATE.md`](../PROJECT_STATE.md)에 보존한다.
...
| `PLAN-09` | 로그인형 편집기를 지속 운영할 CMS로 완성하고 공개 화면과 새 글·사진 보관을 보완한다. | 작업본 자동저장과 명시적 공개 반영을 분리하고 저장 실패·로그인 만료·수정 충돌에도 작성 내용을 유지한다. 기존 편집·이미지·미리보기·주소·597부터의 발행을 유지하며 공개 화면과 탐색을 보완하고 독립 사본 한 곳을 마련한다. | `진행 중` |

### 현재 위치

**현재 진행 중인 계획은 `PLAN-09` 하나다.** 공개 블로그와 이메일 OTP 관리자 화면, 기존 글 편집·신규 발행·이미지와 링크 카드 선택/삭제는 운영 중이다. 최우선인 작업본 자동저장·명시적 공개 반영과 저장 실패·재로그인·충돌 처리는 로컬 구현·시험을 완료했다. 실제 서비스 활성화는 아직 하지 않았다. 공개 화면 보완과 독립 사본은 후속으로 남는다.

사용자에게 이는 **운영 서비스의 자동저장은 아직 공개 글에 바로 반영되지만, 새 버전을 적용하면 작업본에만 저장하고 사용자가 공개 시점을 정할 수 있다는 뜻**이다. 저장 실패·로그인 만료·다른 탭의 수정 충돌도 같은 편집 화면에서 처리한다. 사용자는 운영 적용을 승인했으나 적용용 인증을 다시 연결해야 하므로 실제 서비스는 아직 바뀌지 않았다. 실제 글과 사진은 변경하지 않는다. 다음에는 최소 로그인 권한에 동의한 뒤 이미 승인된 운영 적용을 이어간다.
...
1. 로컬 구현과 시험은 완료됐고, 빈 작업본 저장 공간 추가와 관리자 새 버전의 운영 적용은 승인됐다. 인증 갱신 처리 중 실행 오류로 다시 로그인이 필요해졌다. 기존의 넓은 로그인 권한 동의를 자동 승인 검토가 거절해, 계정 조회·이 계정의 Workers 프로그램 변경·D1 데이터베이스 변경 세 권한만 요청하는 화면을 준비했다. 이 정확한 권한 범위에 대한 동의가 남았으며 실제 작업은 승인된 관리자와 작업본 저장 공간에만 한정한다. 운영 변경은 아직 없고 활성화 전까지 운영 관리자는 기존 자동저장 방식을 사용한다.
2. 새 글·사진의 독립 사본 위치는 후속 순서에서 정한다. 공유·스크랩 글 10개와 과거 댓글의 처리도 별도 선택이며 현재 CMS 구현을 막지 않는다.

### 바로 다음 작업

1. 최소 세 가지 로그인 권한에 대한 동의 후, 이미 승인된 빈 작업본 저장 공간 추가와 관리자 새 버전 활성화를 진행한다. 운영 적용 자체의 재승인은 요구하지 않는다. 공개 사이트 프로그램이나 주소 연결을 바꿀 필요는 없다.
2. 운영 적용 후 이전 관리자 화면은 미저장 입력을 처리한 뒤 새로 열어 새 UI를 사용한다. Git push와 실제 글 수정/삭제, 사진 저장소의 덮어쓰기/삭제, 주소 연결 설정 변경은 하지 않는다.
3. 이후 공개 글 화면 통합, 대표이미지의 홈·목록·글·SNS 연결, query 일관성, 목록·검색 조회 경량화, RSS·사이트맵·공유 최신화 순으로 진행한다.
4. 이어 링크 삽입·수정, 관리자 검색·필터, 빈 초안 삭제, 신규 이미지 최적화와 새 글·사진의 단순한 독립 사본 한 곳을 마련한다.
...
- **Evidence:**
  ...
  - 로컬 구현과 시험은 완료됐고 운영 활성화는 미실행이므로 이 요구사항은 운영 적용까지 `in-progress`를 유지한다.
