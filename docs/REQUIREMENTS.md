...
**글 하단의 같은 카테고리 목록에 페이지 탐색을 추가하고 있다.** `DWNC-CORE-012`는 별도 착수 지시 전까지 `planned`를 유지한다. 진행 중인 계획은 `PLAN-09` 하나다.

사용자에게 이는 글을 읽던 자리에서 같은 카테고리의 모든 글을 5개씩 찾아볼 수 있게 된다는 뜻이다.
...
다음에는 목록 페이지 탐색 구현과 로컬 확인을 마치고 Git 자동 배포·운영 확인을 진행한다. CORE-012와 백업·보관 정책은 별도 지시를 기다린다.
