...
1. 티스토리를 참고한 작성 화면은 로컬 구현·시험·내장 브라우저 확인을 완료했다. 현재 실제 관리자는 기존 화면이며, 검토용 새 화면의 운영 적용 승인만 남았다. 기본 서식 기능의 운영 적용을 막던 로그인 권한 동의는 사용자 동의 후 해결됐다. 운영 적용은 완료됐으며 현재 이 기능의 남은 적용 장애물은 없다. 이전 운영 화면은 700px에서 확인했고, 새 로컬 작성 화면은 1280/700/390px 내장 브라우저 확인을 완료했다.
2. 새 글·사진의 독립 사본 위치는 후속 순서에서 정한다. 공유·스크랩 글 10개와 과거 댓글의 처리도 별도 선택이며 현재 CMS 구현을 막지 않는다.

### 바로 다음 작업

1. 내장 브라우저의 완성된 로컬 작성 화면을 검토받은 뒤, 사용자 승인 후 관리자 프로그램만 실제 서비스에 반영한다. 기존 입력 보존·작업본 자동저장·명시적 공개 반영과 오류 처리는 그대로 유지한다. 공개 프로그램·DB·사진·글 내용·주소 연결은 변경하지 않는다.
...
- **Evidence:**
  ...
  - 로컬 runtime source는 `347758395dece6868b39750e344cda60d8f14893`이며 운영은 직전 목록/편집 이동 수정 버전을 유지한다. 검토용 새 화면과 준비 번들 위치는 PROJECT_STATE.md에 기록했다. 새 화면 운영 적용 승인 대기로 미완료 상태를 유지한다.
