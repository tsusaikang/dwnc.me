...
**모든 글 대표이미지와 최근 기록2열 카드 구성을 운영에 반영했다.** `DWNC-CORE-012`는 별도 착수 지시 전까지 `planned`를 유지한다. 진행 중인 계획은 `PLAN-09` 하나다.

사용자에게 이는 사진과 제목을 함께 보며 글을 찾고, 최신 글 여러 편을 고르게 탐색할 수 있게 된다는 뜻이다.
...
다음에는 사용자가 요청하는 후속 보완을 진행한다. 현재 승인된 목록 변경은 운영 확인까지 마쳤고, 다른 후속 항목은 별도 지시를 기다린다.
