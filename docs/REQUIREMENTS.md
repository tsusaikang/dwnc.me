...
**일반 본문을 18px로 키우고 크기 프리셋도 비례 확대하고 있다.** `DWNC-CORE-012`는 별도 착수 지시 전까지 `planned`를 유지한다. 진행 중인 계획은 `PLAN-09` 하나다.
...
다음에는 크기 변경·서식 보존 확인을 마치고 최종 Git 자동 배포와 운영 확인을 진행한다. 다른 후속 항목은 별도 지시를 기다린다.
