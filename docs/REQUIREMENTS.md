...
기존 글·서식·사진·공개 주소를 보존하면서 블로그를 지속 운영한다. 현재는 GitHub 소스로 Cloudflare가 빌드·배포하도록 전환을 준비한다. 별도로 등록한 결함 6건은 착수 대기이며, 세부 범위와 완료 조건은 [결함 보완 요구사항](결함_보완_요구사항_20260909_223512.md)에 둔다.
...
**Git 배포 전환 준비 중.** `DWNC-OPS-007`의 소스 빌드·배포 연결을 검토하고 로컬 준비한다. `DWNC-CORE-012`의 결함 6건은 수정 가능한 것으로 판단했지만 별도 착수 대기이며 아직 구현·신규 시험·빌드·배포하지 않았다. 진행 중인 계획은 `PLAN-09` 하나다.

사용자에게 이는 현재 서비스를 유지하면서 소스 전달 방식을 Git 기반으로 바꾸는 준비 단계라는 뜻이다. 연결 설정은 저장했지만 실제 자동 빌드·배포 성공은 확인 전이다. 결함 중 사진 업로드 응답 유실은 소스 확인 단계이므로 별도 착수 후 합성 재현부터 확인한다.

### 아직 결정할 일과 진행을 막는 조건

- 결함 보완은 사용자의 후속 착수 지시를 기다린다.
- 두 Worker의 Git 연결 설정은 저장했다. 최종 소스 전달과 실제 Cloudflare 빌드·배포 성공 확인이 남아 있다.
...
다음에는 `DWNC-OPS-007`의 로컬 배포 준비·시험과 내장 대시보드 연결 확인을 진행한다. 이후 별도 착수 지시를 받으면 `DWNC-CORE-012`의 F01~F06을 보완하고 관련 회귀·합성 화면을 확인한다. 결함 전체 보완을 최종 소스에서 새로 빌드해 운영 반영하고 읽기 전용으로 확인한다. 결함 일부만 먼저 배포하거나 이전 번들을 재사용하지 않는다. 독립 백업·휴지통·신규 이미지 최적화는 이번 결함 범위에 포함하지 않는다.

### 이번 작업에서 하지 않는 것

- 착수 지시 전 결함 구현·신규 시험·빌드·운영 반영
...
- **Evidence:**
  - 2026-09-10 사용자가 Git 소스에서 Cloudflare가 가져가도록 전환을 요청하고 내장 브라우저 사용을 허용했다. GitHub 로그인·대상 공개 저장소를 확인했고 기존 소스 전용 빌드 경로가 있다. 계정을 확인하고 양쪽 Worker를 기존 토큰으로 저장소/main에 연결했다. push·실제 CI 실행은 아직 하지 않았다.
  - 대상별 배포 진입점과 합성 시험을 로컬 준비했고 시험을 통과했다. 실제 연결 설정과 미확인 인증 공급은 [Git 배포 안내](GIT_DELIVERY_20260910_142007.md)에 둔다. 실제 Cloudflare 빌드·배포 확인은 남아 있다.
