...
**`DWNC-CORE-021`은 사용자 추가 수정 승인에 따라 `in-progress`로 재개했다.** 대표27건·공개요약29편을 반영했으며, 자동 승인 검토의 차단 이후 남은 공개115·비공개178요약과 카드·화면보완 및 운영읽기의 명시 범위 승인을 기다린다. `DWNC-CORE-012`는 별도 착수 대기이며 진행 중인 계획은 `PLAN-09` 하나다.

사용자에게 이는 깨진 사진과 잘못된 카드·요약을 현재 글 내용에 맞춰 고치는 중이라는 뜻이다.

### 아직 결정할 일과 진행을 막는 조건

- 기존 결함 6건 보완은 사용자의 후속 착수 지시를 기다린다.
- 확인된 오류의 콘텐츠 수정과 기존 공개 글의 수정 반영은 승인됐다. 재이관·비공개 글 공개는 하지 않는다. 외부 위키는 연결 복구, 네이버 카페는 로그인과 해당 글 접근권한이 있어야 내용을 추가 확인할 수 있다. 두 목적지를 정상으로 처리하지 않는다.
...
다음에는 승인된 콘텐츠 수정 범위에서 공식 상태의 조사 결과에서 대상을 정하고 최신 반영본·작업본을 다시 조회한다. 외부 두 목적지는 접근 조건이 바뀌었을 때만 재확인하며 같은 실패 요청을 반복하지 않는다. 결함 보완과 독립 사본은 별도 착수 지시를 따른다.
...
- **진행·검증·재개 근거:** [오류 수정 진행 기록](history/운영콘텐츠_오류수정_진행_20260916_211008.md). 추가 반영은 자동 승인 검토 이후 명시 범위 승인을 기다린다.
