...
**자동 링크 핵심 구현·로컬 확인 완료, 운영 반영 전.** `DWNC-CORE-017`은 티스토리 계정 선택과 관리자 로그인 확인 답변을 기다린다. `DWNC-CORE-012`는 별도 착수 지시 전까지 `planned`를 유지한다. 진행 중인 계획은 `PLAN-09` 하나다.

사용자에게 이는 앞으로 입력하는 웹주소에 자동 링크를 붙이고 원하면 해제할 수 있도록 개선하며, 자체 작성 글의 누락만 보완한다는 뜻이다.

### 아직 결정할 일과 진행을 막는 조건

- 자동 승인 검토가 로그인·계정 선택을 차단해 티스토리 사용할 계정과 관리자·Cloudflare 기존 계정 로그인 허용 답변이 필요하다. 기능 구현·자체 글 보완 자체는 이미 승인됐다.
...
다음에는 로그인 확인 답변을 받아 티스토리 링크 UX 확인, 필요한 UI 보완, 자체 작성 글의 누락 링크 보완, 최종 소스 Git 자동 배포와 운영 확인을 진행한다. 원래 링크가 없던 이전 글은 그대로 둔다. CORE-012는 별도 착수 지시를 기다린다.
...
- **Evidence:**
  - 후속 승인에 따라 핵심 구현과 로컬 회귀·브라우저·source-only 빌드를 마쳤다. [구현 진행 기록](history/자동링크_구현_진행_20260913_201937.md)에 운영 미반영·자체 글 미수정·티스토리 관찰 미완료를 구분했다.
  - 2026-09-13 사용자 요청은 “일단 파악만”이다. [조사 상세](history/자동링크_및_이전글_조사_20260913_174723.md)에 편집기 원인, 보관 596편 전수 대조, 운영 확인의 범위와 한계를 기록했다.
  - 실제 참조로 읽히는 미링크 URL 10편 12곳은 모두 보관 원본부터 일반 텍스트였다. 조사 대상에서 이관 중 anchor 손실은 발견하지 못했으며 과거 명시적 링크 해제 의도는 확정할 수 없다.
