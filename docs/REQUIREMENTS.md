...
기존 글·서식·사진·공개 주소를 보존하면서 블로그를 지속 운영한다. 운영 최신 콘텐츠 598편의 초기 데이터 분석과 보고서는 마련했으며, 미확인 화면·이미지·외부 카드의 재확인을 이어간다. 확인된 수정 대상은 공식 상태의 조사 보고서에 있으며 실제 콘텐츠 변경은 하지 않았다. 별도로 등록한 결함 6건은 착수 판단을 기다린다. 세부 결함 범위와 완료 조건은 [결함 보완 요구사항](결함_보완_요구사항_20260909_223512.md)에 둔다. 이미 완료된 기능과 배포 결과는 완료 원장과 공식 상태에서 확인한다.
...
**`DWNC-CORE-021`은 초기 데이터 분석 완료·미확인 항목 재확인 대기 상태로 `in-progress`를 유지한다.** 완료 원장에서 현재 요구사항으로 되돌렸다. 사용자가 2026-09-16 미확인 항목의 재확인을 요청했으므로 새 작업에서 추가 승인 없이 읽기 전용 조사를 계속한다. 현재 세션은 컨텍스트 누적을 고려해 공식 상태와 인수인계만 정리한다. 실제 콘텐츠 변경은 하지 않는다. `DWNC-CORE-012`는 별도 착수 전까지 `planned`이며 진행 중인 계획은 `PLAN-09` 하나다.

사용자에게 이는 이미 찾은 문제를 보존하면서 아직 화면을 보지 못한 항목까지 확인을 이어간다는 뜻이다.

### 아직 결정할 일과 진행을 막는 조건

- 기존 결함 6건 보완은 사용자의 후속 착수 지시를 기다린다.
- CORE-021 재확인 착수는 승인된 조사 범위다. 반복된 HTTP 오류와 프레임·앱 응답 때문에 못 본 부분을 내장 브라우저에서 우선 확인한다. 실제 수정·공개·재이관은 승인되지 않았다.
...
다음에는 새 작업에서 공식 상태의 CORE-021 재확인 인수인계를 읽고 최신 운영 목록·수정시각·revision·작업본을 갱신한다. 이전 미확인259편을 우선 내장 브라우저로 확인하고 이미지·비공개 화면·외부 카드의 남은 항목을 조사한다. 메인은 운영 접근·브라우저·통합을, 하위 에이전트는 독립 분석을 맡는다. 오류 응답만 같은 방식으로 반복하거나 완료한 데이터 분석을 처음부터 되풀이하지 않는다.
