...
**모든 글 대표이미지와 최근 기록2열 카드 구성을 구현하고 있다.** `DWNC-CORE-012`는 별도 착수 지시 전까지 `planned`를 유지한다. 진행 중인 계획은 `PLAN-09` 하나다.
...
다음에는 목록 구성과 모바일 화면을 확인하고 최종 소스 저장·자동 배포·운영 확인까지 마친다. 다른 후속 항목은 별도 지시를 기다린다.
