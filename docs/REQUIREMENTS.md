...
사용자에게 이는 **입력은 작업본에 자동저장되고, 공개 반영을 눌렀을 때만 방문자에게 보이는 글이 바뀐다는 뜻**이다. 저장 실패·로그인 만료·수정 충돌 때는 현재 입력을 유지한 채 같은 화면에서 이어갈 수 있다. 기본 서식 도구와 기존 서식 보존도 로컬 시험 후 실제 관리자와 공개 프로그램에 적용했다. 자체 내장 브라우저에서 서식 도구·저장 안내·미리보기·기존 이미지·긴 본문의 삭제 도구와 공개 글 화면을 확인했다. 실제 글과 사진의 수정이나 시험 발행은 하지 않았다. 다음에는 기존·신규 공개 글 화면을 통합하고 대표이미지를 연결한다.
...
1. 기본 서식 기능의 운영 적용을 막던 로그인 권한 동의는 사용자 동의 후 해결됐다. 운영 적용은 완료됐으며 현재 이 기능의 남은 적용 장애물은 없다. 모바일 실제 화면 크기 확인은 도구 한계로 미확인이다.
