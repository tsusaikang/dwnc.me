...
사용자에게 이는 **입력은 작업본에 자동저장되고, 공개 반영을 눌렀을 때만 방문자에게 보이는 글이 바뀐다는 뜻**이다. 저장 실패·로그인 만료·수정 충돌 때는 현재 입력을 유지한 채 같은 화면에서 이어갈 수 있다. 실제 글과 사진을 수정하거나 시험 발행하지 않았다. 사용자가 요청한 기본 서식 도구와 기존 서식 보존도 로컬 구현·실행 시험과 브라우저 확인을 마쳤다. 다음에는 이 기능을 실제 관리자와 공개 사이트 프로그램에 적용한 뒤 공개 글 화면 통합과 대표이미지 연결을 진행한다. 새 서식 기능은 아직 운영 미적용이다.
...
1. 기본 서식 편집은 로컬 구현·통상 시험·브라우저 확인과 최종 빌드를 마쳤다. 실제 사용을 위해 형식 저장 구조와 관리자·공개 사이트 프로그램의 적용 승인이 남았다. 기존 글과 사진, 주소 연결은 바꾸지 않는다.
2. 새 글·사진의 독립 사본 위치는 후속 순서에서 정한다. 공유·스크랩 글 10개와 과거 댓글의 처리도 별도 선택이며 현재 CMS 구현을 막지 않는다.

### 바로 다음 작업

1. 완성한 기본 서식 기능의 운영 적용을 승인받는다. 승인 후 형식 열 추가 → 공개 사이트 프로그램 → 관리자 프로그램 순서로 적용해 새 HTML 글의 본문 사진도 정상 표시되도록 한다.
