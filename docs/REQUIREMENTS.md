...
**`DWNC-CORE-021`은 사용자 추가 수정 승인에 따라 `in-progress`다.** 대표27건·공개요약144편·비공개요약178편 반영과 기존 공개상태·작업본 보존을 확인했다. 카드6편은 HTML 편집기의 전체본문 중첩 부작용을 발견해 원인 보완과 정상 구조 복원을 진행한다. 카페 원문 미확인은 해소됐고 위키 내용 미확인1개를 구분한다. `DWNC-CORE-012`는 별도 착수 대기이며 진행 중인 계획은 `PLAN-09` 하나다.
...
다음에는 HTML 전체 편집의 중첩 문제를 보완·배포하고 카드6편을 원래 구조로 복원한 뒤 최신 운영 반영본·작업본과 실제 공개 화면을 다시 확인한다. 외부 위키는 접근 조건이 바뀌었을 때 재확인하며 같은 실패 요청을 반복하지 않는다. 별도 결함 보완과 독립 사본은 별도 착수 지시를 따른다.
