...
- 티스토리 공개 글 164개와 네이버 공개 글 185개를 새 사이트에서 제공한다. 네이버 비공개 글247개는 원본을 로컬에 보존하며, 사용자 승인에 따라 운영 CMS에도 비공개로 이전해 로그인한 관리자만 조회·편집한다.
...
원본 비공개247편의 관리자 목록 누락은 `DWNC-CORE-011`에서 해결했다. 본문과 참조 사진·영상을 운영 CMS에 비공개로 이전했으며 관리자 목록247편·본문·사진 표시와 일반 방문자 차단을 확인했다. 원본·기존 공개 글·작업본·예약번호를 유지한다.

카테고리 표시와 잘못된 관리자 주소 안내를 최종 소스에서 새로 빌드해 운영 반영했다. 사용자가 배포 연결의 동일 세 권한에 직접 동의했고 기존 인증 차단은 해소됐다. 두 프로그램의 운영 반영과 내장 브라우저 실제 화면 확인까지 완료했다. 실제 글·사진·데이터베이스와 주소 연결 설정은 변경하지 않았다.

공개 카테고리 이름은 가로로 정상 표시되고 편수·하위 갈래가 계층 카드에 배치된다. `갈래 +`와 하위 갈래 펼치기를 운영 화면에서 확인했다. 관리자에 없는 `/archive` 주소는 한국어 안내와 관리자 홈·공개 글 목록 이동 링크를 제공하며 관리자 홈 이동도 확인했다. 관련 시험·독립 검토를 통과했고 분류 자료와 기존 주소·운영 기능은 유지한다.

공개 사이트의 관리자 바로가기·글별 편집 연결까지 운영 반영했다. 관리자로 로그인하면 공개 목록과 본문에서 정확한 작업본을 새 탭으로 열고 글 관리·새 글 안내·통계·설정으로 바로 이동할 수 있다.
...
공개 카테고리 표시와 잘못 입력한 관리자 주소 안내는 운영 반영과 실제 확인까지 완료됐다. 수정된 화면을 바로 사용할 수 있으며 이번 좁은 수정에 남은 작업은 없다.
...
공개 사이트의 관리자 바로가기와 글 편집 연결도 운영 반영을 완료했다. 로그인한 관리자는 공개 목록·본문에서 새 탭으로 편집할 수 있다.
...
- **Evidence:**
  ...
  - 이전 후 공개 canonical/alias/미디어 대표12요청404와 공개 목록·검색·RSS·sitemap 등6면 노출0을 확인했다. 제목·본문·개별ID·원본경로·비밀은 기록하지 않았다. 상세 결과는 PROJECT_STATE.md 최신 완료 절을 따른다.
...
- **Evidence:**
  - 로그인 확인·정확한 글 선택·미저장 입력 유지·새 초안 자동생성 없음·비로그인 숨김과 통합 시험을 통과했다. 최종 소스로 새 빌드해 운영 반영했다. 실제 공개 본문의 편집하기 클릭으로 해당 작업본이 새 탭에 열리고, 홈의 글별 연결과 통계 바로 열림을 확인했다. 실제 글 내용·사진·주소·Access 설정은 유지했다. 상세 운영 결과와 시험 범위는 PROJECT_STATE.md 최신 완료 절을 따른다.
