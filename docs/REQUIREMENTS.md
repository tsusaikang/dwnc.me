...
1. 기본 서식 기능의 운영 적용을 막던 로그인 권한 동의는 사용자 동의 후 해결됐다. 운영 적용은 완료됐으며 현재 이 기능의 남은 적용 장애물은 없다. 700px 폭의 글 목록과 편집 이동은 실제 내장 브라우저에서 확인했다. 더 작은 휴대전화 폭의 서식 전체 동작은 이번에 확인하지 않았다.
2. 새 글·사진의 독립 사본 위치는 후속 순서에서 정한다. 공유·스크랩 글 10개와 과거 댓글의 처리도 별도 선택이며 현재 CMS 구현을 막지 않는다.

### 바로 다음 작업

1. 이전 관리자 탭의 미저장 입력을 먼저 처리한 뒤 새 관리자 화면을 사용한다. 입력은 작업본에만 자동저장되며 공개 반영은 사용자가 실행한다.
