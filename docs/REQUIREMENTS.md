...
1. 계정 번호 저장 사전검사는 이미 준비 완료다. Codex 자체 브라우저에서는 잘못 들어간 계정에서 로그아웃했고 현재 Cloudflare 공식 로그인 화면에서 대기 중이다. 사용자가 **올바른 계정으로 직접 로그인한 뒤 Account ID 옆 Copy를 한 번만 누르면**, 초기화를 실행해 계정 번호를 Mac의 전용 보관함에 저장한다. Chrome과 Edge는 사용하지 않으며 로그인 정보와 비밀번호는 문서·로그·출력·파일에 남기지 않는다.
...
사용자가 다음에 할 일은 현재 열려 있는 Cloudflare 공식 로그인 화면에서 **올바른 계정으로 직접 로그인한 뒤 Account ID 옆 Copy를 한 번 누르는 것**이다. Chrome과 Edge는 앞으로 이 프로젝트에 사용하지 않는다. 실제 `dwnc.me` 주소 연결은 `PLAN-08`에서 사용자 결정이 있어야만 진행한다. 이미 승인된 시험용 작업은 안전 조건이 충족되면 단순히 승인을 다시 받기 위해 멈추지 않는다. 이번 변경은 로컬 파일 12개에만 한정됐고 하나의 저장 이력으로 정리했으며, 저장 직후 파일 상태도 깨끗했다. 인터넷 저장소로 보내지는 않았다.
...
- **Evidence:**
  ...
  - 깨끗한 commit `fb693f3bbbab0e88b205f9de944646057bdd49ea`에서 계정 번호를 받기 전 준비 확인을 실제로 1회 실행했고 결과는 `ready`였다. 이때 Mac 보관함의 기존 상태는 읽기 1회만 했고 저장·변경은 0회였다. 클립보드는 읽기·비우기·쓰기가 모두 0회였고 실제 초기화도 0회다. Codex 자체 브라우저에서는 잘못 들어간 계정에서 로그아웃했고 Cloudflare 공식 로그인 화면에서 대기 중이다. 다음 순서는 **사용자가 올바른 계정으로 직접 로그인 → Account ID 옆 Copy 한 번 → 초기화**다. Chrome과 Edge는 사용하지 않는다.
...
- **Acceptance:**
  ...
  - 자체 브라우저의 로그인이 풀렸을 때만 로그인 페이지를 열며, 사용자가 로그인 정보와 비밀번호를 직접 입력한다.
...
- **Evidence:**
  ...
  - Codex 자체 브라우저에서 잘못 들어간 계정의 로그아웃을 마쳤고 Cloudflare 공식 로그인 화면에서 사용자 로그인을 기다리고 있다.
