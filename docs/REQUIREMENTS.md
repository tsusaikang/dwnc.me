...
1. 티스토리를 참고한 작성 화면은 로컬 구현·시험·내장 브라우저 확인을 완료했다. 사용자 승인 후 실제 관리자 프로그램에도 반영했다. 사용자 재로그인 후 실제 화면 확인까지 완료했다. 기본 서식 기능의 운영 적용을 막던 로그인 권한 동의는 사용자 동의 후 해결됐다. 운영 적용은 완료됐으며 현재 이 기능의 남은 적용 장애물은 없다. 이전 운영 화면은 700px에서 확인했고, 새 로컬 작성 화면은 1280/700/390px 내장 브라우저 확인을 완료했다.
2. 새 글·사진의 독립 사본 위치는 후속 순서에서 정한다. 공유·스크랩 글 10개와 과거 댓글의 처리도 별도 선택이며 현재 CMS 구현을 막지 않는다.

### 바로 다음 작업

1. 로컬 수정과 서비스 반영용 파일 준비를 마친 공개 글 표시 수정본의 실제 공개 화면 적용을 승인받는다. 읽는 시간 표시를 빼고 흰 읽기면과 원래 본문 글꼴을 적용하며 원문과 실제 글 내용은 변경하지 않는다.
2. 운영에 반영한 새 작성 화면을 사용한다. 미저장 입력이 있는 이전 탭은 내용을 먼저 처리한 뒤 새로 연다. 기존 입력 보존·작업본 자동저장·명시적 공개 반영과 오류 처리는 그대로 유지한다. 이번 공개 표시 수정에서 관리자 프로그램·DB·사진·글 내용·주소 연결은 변경하지 않는다.
3. 기존·신규 공개 글 화면을 통합해 대표이미지·경로 안내·관련 글·이전/다음 글·목록 카드 이미지를 제공하고 홈·목록·글·SNS 연결, query 일관성, 목록·검색 조회 경량화, RSS·사이트맵·공유 최신화 순으로 진행한다.
4. 이어 관리자 검색·필터, 빈 초안 삭제, 신규 이미지 최적화와 새 글·사진의 단순한 독립 사본 한 곳을 마련한다. Git push와 실제 글 수정/삭제, 사진 저장소 덮어쓰기/삭제, 주소 연결 설정 변경은 하지 않는다.
...
- **Acceptance:**
  ...
  - 원문/정규화 자료나 실제 저장 본문을 바꾸지 않고 공개 표현에서 수정한다. 로컬 통상 시험·빌드·내장 브라우저 비교·문서 갱신 후 실제 공개 화면 적용 직전에 승인받는다.
- **Evidence:**
  ...
  - 공개 CSS 및 정적/동적 글 템플릿 수정을 마쳤고 관련 회귀 시험을 통과했다. 내장 브라우저에서 #595의 배경·글꼴·읽는 시간 제거, 다른 글의 실제 굵기·개별 글꼴·표 배경·코드 보존을 확인했다. 서비스 반영용 정적 파일과 공개 프로그램 준비도 완료했다. 실제 공개 사이트·글·사진은 변경하지 않았으며 공개 적용 승인을 기다린다. 세부 검증 상태는 PROJECT_STATE.md를 따른다.
