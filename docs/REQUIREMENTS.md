...
사용자에게 이는 **입력은 작업본에 자동저장되고, 공개 반영을 눌렀을 때만 방문자에게 보이는 글이 바뀐다는 뜻**이다. 저장 실패·로그인 만료·수정 충돌 때는 현재 입력을 유지한 채 같은 화면에서 이어갈 수 있다. 실제 글과 사진을 수정하거나 시험 발행하지 않았다. 다음에는 기존 글과 새 글의 공개 화면을 통합하고 대표이미지를 연결한다.
...
1. 작업본 CMS의 운영 적용을 막던 인증 연결은 사용자가 최소 세 권한에 동의해 해결됐다. 빈 작업본 저장 공간과 관리자 새 버전 적용을 완료했으며 이 최우선 작업의 남은 장애물은 없다. 실제 수정·실패·충돌·신규 발행 흐름은 합성 데이터로 시험했고 운영에서는 기존 글을 읽기와 선택만으로 확인했다.
2. 새 글·사진의 독립 사본 위치는 후속 순서에서 정한다. 공유·스크랩 글 10개와 과거 댓글의 처리도 별도 선택이며 현재 CMS 구현을 막지 않는다.

### 바로 다음 작업

1. 기존·신규 공개 글 화면을 통합해 대표이미지·경로 안내·관련 글·이전/다음 글·목록 카드 이미지를 제공한다. 이번 관리자 적용에 대한 추가 승인은 필요하지 않다.
2. 운영 적용 후 이전 관리자 화면은 미저장 입력을 처리한 뒤 새로 열어 새 UI를 사용한다. Git push와 실제 글 수정/삭제, 사진 저장소의 덮어쓰기/삭제, 주소 연결 설정 변경은 하지 않는다.
3. 이어 대표이미지의 홈·목록·글·SNS 연결, query 일관성, 목록·검색 조회 경량화, RSS·사이트맵·공유 최신화 순으로 진행한다.
