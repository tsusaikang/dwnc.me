...
**현재 진행 중인 계획은 `PLAN-09` 하나다.** 공개 블로그와 이메일 OTP 관리자, 기존 글 편집·신규 발행·이미지와 링크 카드 선택/삭제는 운영 중이다. 최우선인 작업본 자동저장·명시적 공개 반영과 저장 실패·재로그인·충돌 처리는 로컬 시험 후 실제 관리자에 적용했다. 운영 화면에서 기존 글과 저장 상태·이미지·삭제 도구막대를 확인했고 공개 글도 정상이다. 공개 화면 보완과 독립 사본은 후속으로 남는다.

사용자에게 이는 **입력은 작업본에 자동저장되고, 공개 반영을 눌렀을 때만 방문자에게 보이는 글이 바뀐다는 뜻**이다. 저장 실패·로그인 만료·수정 충돌 때는 현재 입력을 유지한 채 같은 화면에서 이어갈 수 있다. 기본 서식 도구와 기존 서식 보존도 로컬 시험 후 실제 관리자와 공개 프로그램에 적용했다. 자체 내장 브라우저에서 서식 도구·저장 안내·미리보기·기존 이미지·긴 본문의 삭제 도구와 공개 글 화면을 확인했다. 좁은 화면에서 글 목록 아래에 편집 화면이 숨어 보이던 문제도 고쳤다. 이제 글을 선택하면 편집칸으로 바로 이동하고 `글 목록` 버튼으로 돌아갈 수 있다. 실제 글과 사진의 수정이나 시험 발행은 하지 않았다. 사용자가 이어 요청한 티스토리 편집 화면을 실제 내장 브라우저에서 확인했다. 목록과 작성 화면을 분리하고 상단 서식·사진 도구, 넓은 본문, 하단 저장·미리보기·공개 반영으로 개편한 로컬 화면과 PC/모바일 미리보기를 완성했다. 입력 보존과 실제 작은 화면 확인도 마쳤다. 사용자 승인 후 새 화면을 실제 관리자에 적용했다. 사용자 재로그인 후 운영 화면의 목록·편집 왕복, 기존 이미지·저장 상태, PC/모바일 미리보기와 작은 화면 표시도 확인했다. 사용자가 지적한 공개 글의 읽는 시간 표시 제거와 흰 읽기면·본문 글꼴 정리도 로컬에서 완료하고 원래 글과 비교했다. 실제 공개 화면에는 아직 적용하지 않았다. 사용자는 이 수정본의 즉시 반영 대신 네이버·티스토리 공개 글 표시, 티스토리 편집기와 카테고리 등 관리화면 전체의 차이를 다시 검토하고 필요한 개발 후 함께 실 사이트에 반영하도록 요청했다. 원본 관리 메뉴 전반과 공개 글 서식 유형을 비교했고, 공개 서식·도해 복원과 글 검색/분류/대표사진·설정·편집 도구의 1차 구현과 관련 통상 시험·실제 조작 확인을 마쳤고 로컬 커밋으로 보관했다. 페이지·공지·재사용 서식, 공개 상태/예약·보호 및 추가 설정 등은 아직 남아 있어 전체 완료나 운영 반영 상태가 아니다. 다음에는 통합된 1차 구현에서 이어 남은 기본 운영 기능과 부가 기능 범위를 정리해 개발한 뒤 승인된 운영 반영까지 진행한다.
...
1. 현재 확대 요청의 남은 범위는 페이지·공지·재사용 서식·예약/보호/공개 상태·추가 설정과 댓글/통계/플랫폼 전용 기능의 포함 여부다. 부가 기능 범위 질문은 아직 답변 대기이며 독립적인 핵심 구현은 계속 진행한다. 앞선 티스토리를 참고한 작성 화면은 로컬 구현·시험·내장 브라우저 확인을 완료했다. 사용자 승인 후 실제 관리자 프로그램에도 반영했다. 사용자 재로그인 후 실제 화면 확인까지 완료했다. 기본 서식 기능의 운영 적용을 막던 로그인 권한 동의는 사용자 동의 후 해결됐다. 운영 적용은 완료됐으며 현재 이 기능의 남은 적용 장애물은 없다. 이전 운영 화면은 700px에서 확인했고, 새 로컬 작성 화면은 1280/700/390px 내장 브라우저 확인을 완료했다.
2. 새 글·사진의 독립 사본 위치는 후속 순서에서 정한다. 공유·스크랩 글 10개와 과거 댓글의 처리도 별도 선택이며 현재 CMS 구현을 막지 않는다.

### 바로 다음 작업

1. 로컬 통합·커밋한 1차 공개 서식·핵심 편집/관리 보완에서 이어간다. 남은 페이지·공지·재사용 서식·공개 상태·추가 설정 등을 설계·개발하고, 댓글/통계/광고/구독/팀블로그 범위 질문의 답변을 반영한다. 통상 시험·내장 브라우저 확인 후 실 사이트에 함께 반영한다. 직전 읽는 시간·배경·글꼴 수정도 포함하며 이번 범위의 운영 반영 승인은 받았다. 실제 글과 원본 내용은 변경하지 않는다.
2. 운영에 반영한 새 작성 화면을 사용한다. 미저장 입력이 있는 이전 탭은 내용을 먼저 처리한 뒤 새로 연다. 기존 입력 보존·작업본 자동저장·명시적 공개 반영과 오류 처리는 그대로 유지한다. 이번 보완에서도 기존 작업본과 공개 글 내용·사진·주소 연결은 유지한다.
3. 이번에 로컬 구현한 기존·신규 공개 글의 대표이미지·경로/관련/이전다음·목록 카드·SNS 연결, query 일관성, 목록/검색 조회 경량화, RSS/사이트맵 최신화를 최종 통합·빌드·운영 확인한다. 기존 글 본문과 공개 주소는 유지한다.
4. 1차 구현한 관리자 검색·필터를 유지하며 빈 초안 삭제, 신규 이미지 최적화와 새 글·사진의 단순한 독립 사본 한 곳을 마련한다. Git push와 실제 글 수정/삭제, 사진 저장소 덮어쓰기/삭제, 주소 연결 설정 변경은 하지 않는다.

현재 `PLAN-05`부터 `PLAN-08`까지와 기본 웹 편집기 운영 반영은 완료됐다. `PLAN-09`는 지속 운영용 CMS와 공개 화면 보완을 진행하며, 백업 결정만 남은 상태가 아니다.
...
- **Evidence:**
  - 원본 서식·도해, 분류/검색/대표사진/설정/편집 도구의 1차 보완에 이어 페이지·공지·서식, 예약·비공개·보호·빈 초안 삭제, 아이콘·시간대·CCL의 2차 구현과 통합 시험을 완료했다. 합성 내장 브라우저 조작·최종 정적 빌드·새 공개/관리 번들도 마쳤고 로컬 커밋했다. 기존 작업본·서식·사진 소유·공개 주소를 유지하며 실제 글은 변경하지 않았다. 운영 반영은 이미 승인됐지만 자동 승인 검토가 기존 동일 세 권한 동의를 이번 OAuth Authorize에 인정하지 않아 실행 전 차단됐다. 운영 변경은 없으며 동일 인증 제출의 직접 동의를 받은 뒤 반영/실제 확인을 이어간다. 댓글·방명록·통계·광고·구독·팀블로그는 미답변 범위로 남아 있다. 세부 구현/시험과 브라우저 확인의 한계는 PROJECT_STATE.md 최신 절을 따른다.
...
- **Evidence:**
  - 사용자 첨부 #595와 원래 티스토리 /172를 비교했다. 원래 일반 문장과 대비된 문장은 같은 system 고딕이며, 공개 사이트의 기본 명조와 남은 span 고딕 지정이 불일치를 만들었다. 흰 문단 배경 역시 원문 서식이다.
  - 공개 CSS 및 정적/동적 글 템플릿 수정을 마쳤고 관련 회귀 시험을 통과했다. 내장 브라우저에서 #595의 배경·글꼴·읽는 시간 제거, 다른 글의 실제 굵기·개별 글꼴·표 배경·코드 보존을 확인했다. 서비스 반영용 정적 파일과 공개 프로그램 준비도 완료했다. 실제 공개 사이트·글·사진은 변경하지 않았다. 후속 DWNC-CORE-006 비교·보완과 함께 적용하도록 승인됐으며 전체 보완 후 반영한다. 세부 검증 상태는 PROJECT_STATE.md를 따른다.
