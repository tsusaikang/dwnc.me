...
**진행 중인 계획은 `PLAN-05` 하나다.** 다음 단계인 `PLAN-06`은 아직 대기 상태지만, 시험용 Worker를 안전하게 만들기 위한 로컬 모의검사는 미리 보강했다. 공개 미디어 2,758개의 업로드와 업로드 뒤 목록 확인은 끝났지만, 모든 파일을 다시 내려받아 원본과 비교하는 검사는 남아 있다. Cloudflare 계정 번호는 사진 업로드용 열쇠와 섞지 않고 Mac의 안전한 보관함에서 불러오도록 만들었다. 이 번호는 로그인 열쇠가 아니라 올바른 계정을 고르는 확인 표지다.
...
2. 앞서 만든 읽기 전용 열쇠 두 개는 전달 과정이 안전하지 않아 사용하지 않았고 모두 폐기했다. 두 시도 모두 Cloudflare 정보를 읽은 횟수와 결과 파일 수가 0이다. 새 방식은 로컬 시험을 통과했으며, 다음에는 올바른 계정 번호를 전용 보관함에 한 번 저장한 뒤 새 읽기 전용 열쇠를 사용한다.
...
1. 올바른 Cloudflare 계정의 계정 번호를 한 번만 복사해 Mac의 전용 보관함에 저장한다. 원래 번호는 화면 출력이나 프로젝트 파일에 남기지 않고, 프로젝트에 기록된 계정과 일치하는지만 확인한다.
...
- 계정 번호 전용 보관·복구 시험 123개와 읽기 전용 열쇠 전달 시험 81개를 통과했다. 실제 Keychain·클립보드·Cloudflare·R2 접근은 0회다.
...
- **Evidence:**
  ...
  - 각 Cloudflare 응답은 최대 1MiB까지만 읽고, 원본은 계정 번호와 열쇠를 제외한 채 프로젝트 밖의 정해진 보호 폴더에 새 파일로만 보존한다. 첫 파일 쓰기가 중간에 끊긴 경우에만 자동으로 정해진 두 번째 파일 하나를 쓸 수 있으며, 완성됐지만 내용이 다른 첫 파일을 두 번째 파일로 우회할 수 없다. 실행 전에 허가 파일과 모든 원본·복구·최종 결과 파일 13곳을 전부 확인해 하나라도 이미 있거나 일부만 기록됐거나 상위 폴더가 안전하지 않으면 Cloudflare 조회와 생성 명령을 시작하지 않는다.
  ...
  - 로컬 모의시험은 전체 Cloudflare 묶음 23개 시험·5,689개 확인을 모두 통과했고, 그중 최초 생성 검증·실행 코어 시험은 4,126개다. 정상 흐름은 생성 전 Cloudflare 확인 2회, 승인·Git 확인 3회, 생성 명령 정확히 1회, 생성 뒤 7개 응답 묶음 2회이며, 비밀값·압축 응답·크기 초과·중간 상태 변경·기록 충돌·Git 변경·승인 만료·임시 파일 정리 실패 때는 성공으로 넘어가지 않는다. 이 과정의 실제 Cloudflare 조회·Worker 생성·설정 변경·배포·DNS 변경은 0회다.
  - 실제 smoke token·Worker는 아직 생성하지 않아 완료 조건은 남아 있다. Worker 생성은 성공했지만 확인 자료를 쓰기 전에 작업이 끊기는 경우, 같은 생성 명령을 다시 실행하지 않고 현재 상태만 확인해 이어가는 복구 절차가 별도 `P0`로 남아 있다. 또한 정확한 staging 계정 한 곳에만 유효하고 `Workers Scripts Write` 한 권한만 가진 60분 이상·48시간 이하의 짧은 수명 Account API token을 임시 Keychain에서 익명 FD 3으로 격리된 Wrangler 자식에만 전달하는 도구가 다음 독립 구현으로 남아 있다. R2·DNS·Zone 등 다른 권한과 OAuth fallback은 허용하지 않는다. 두 기능이 완성되기 전에는 실제 실행 파일과 직접 호출 wrapper가 열쇠를 읽거나 Cloudflare에 요청하기 전에 항상 중단되며, 설정값이나 실행 옵션으로 우회할 수 없다. production account/media/release fingerprint는 `null`을 유지한다.
...
- **Acceptance:**
  ...
  - 계정 번호는 클립보드에서 한 번만 읽고 바로 지운다. 프로젝트에 기록된 올바른 계정과 일치할 때만 새 항목으로 저장하며, 기존 항목·파일은 덮어쓰거나 삭제하지 않는다.
  ...
  - Cloudflare 정보를 읽는 작업은 부모 환경에서 계정 번호를 받지 않고 전용 항목을 한 번만 읽는다. 사진 업로드용·검사용 열쇠는 읽지 않으며, 원래 번호와 짧은 읽기 전용 열쇠를 명령어나 화면 출력에 넣지 않는다.
...
- **Evidence:**
  ...
  - 올바른 값, 빈 값·형식 오류·다른 계정, 클립보드 비우기 실패, 이미 있는 보관 항목과 확인 파일, 다른 파일을 가리키는 연결, 느슨한 접근 권한, 작업 도중 파일이 바뀌는 경우, 프로젝트 안 기본·명시 위치와 실제 위치·중간 폴더 연결 거부, 임의 복구 파일 이름 거부, 저장 뒤 빈 파일이 남은 경우의 고정된 두 번째 확인 파일 복구와 안전한 재확인, 두 확인 파일이 모두 불완전할 때의 안전 중단과 보관 항목 보존, 확인값·용도·항목 이름 변조를 포함한 123개 시험을 통과했다.
  - 읽기 전용 열쇠와 계정 번호를 분리하고 조회 명령도 프로젝트 안 확인 파일을 열지 못하게 하는 81개 시험을 통과했고, 실제 Keychain·클립보드·Cloudflare·R2 접근과 사진 업로드용 열쇠 읽기는 모두 0회였다.
  - 실제 계정 번호 저장과 Cloudflare 조회는 아직 하지 않았으므로 상태는 진행 중이다.
