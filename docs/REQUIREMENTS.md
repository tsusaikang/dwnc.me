...
- 계정 번호 전용 보관·복구 시험 123개, 읽기 전용 열쇠 전달 시험 81개와 시험용 Worker 관리 열쇠 시험 352개를 통과했다. 실제 Keychain·클립보드·Cloudflare·R2 접근은 0회다.
...
- **Evidence:**
  ...
  - 로컬 모의시험은 전체 Cloudflare 묶음 24개 시험·6,041개 확인을 모두 통과했고, 그중 최초 생성 검증·실행 코어 시험은 4,126개다. 정상 흐름은 생성 전 Cloudflare 확인 2회, 승인·Git 확인 3회, 생성 명령 정확히 1회, 생성 뒤 7개 응답 묶음 2회이며, 비밀값·압축 응답·크기 초과·중간 상태 변경·기록 충돌·Git 변경·승인 만료·임시 파일 정리 실패 때는 성공으로 넘어가지 않는다. 이 과정의 실제 Cloudflare 조회·Worker 생성·설정 변경·배포·DNS 변경은 0회다.
  ...
  - 실제 관리 열쇠·smoke token·Worker는 아직 만들지 않았다. Worker 생성은 성공했지만 확인 자료를 쓰기 전에 작업이 끊기는 경우, 같은 생성 명령을 다시 실행하지 않고 현재 상태만 확인해 이어가는 복구 절차가 별도 `P0`로 남아 있다. 이 복구가 완성되기 전 bootstrap은 runner 확인을 먼저 통과하더라도 열쇠를 읽거나 Cloudflare 요청·생성 명령에 도달하기 전에 중단한다. version upload·상세 확인·적용·workers.dev 활성화 명령도 새 전달 방식에 연결되기 전에는 열쇠를 읽지 않고 중단한다. 과거 읽기 전용 runner의 workers.dev 경로와 내부 FD 전용 npm 명령은 제거해 새 runner를 우회할 수 없게 했다. production account/media/release fingerprint는 `null`을 유지한다.
