...
**이관 카드 표시를 운영에 반영하고, 전수조사에 사용할 최신 콘텐츠 기준을 정리했다.** 운영 최신 반영본과 별도 미공개 작업본을 정본으로, 과거 덤프는 비교·재현용으로 사용한다. 최신 운영 전체 콘텐츠의 링크·대표이미지·요약 전수조사는 아직 실행하지 않았다. `DWNC-CORE-012`는 별도 착수 지시 전까지 `planned`를 유지한다. 진행 중인 계획은 `PLAN-09` 하나다.
...
다음에는 정리한 최신 콘텐츠 기준에 따라 전수조사 범위를 확인한다. 내부 카드 표시 수정은 운영 확인까지 마쳤다. 후속 전수조사는 운영 최신 반영본과 현재 공개 상태를 기준으로 하며, 미공개 작업본과 남아 있는 정적 대체 경로를 구분한다. 원문 일괄 재이관이나 덤프 삭제는 하지 않는다.
