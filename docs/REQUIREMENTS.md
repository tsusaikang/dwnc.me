...
**이전/다음 글도 같은 카테고리 안에서 이동하도록 보완하고 있다.** `DWNC-CORE-012`는 별도 착수 지시 전까지 `planned`를 유지한다. 진행 중인 계획은 `PLAN-09` 하나다.

사용자에게 이는 아래 이동 카드를 눌러도 현재 읽던 카테고리를 벗어나지 않는다는 뜻이다.
...
다음에는 같은 카테고리 이동의 구현·시험을 마치고 최종 소스 Git 자동 배포와 운영 확인을 진행한다. 다른 후속 항목은 별도 지시를 기다린다.
