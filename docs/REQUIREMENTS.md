...
| `PLAN-09` | 로그인형 편집기를 지속 운영할 CMS로 완성하고 공개 화면과 새 글·사진 보관을 보완한다. | 작업본 자동저장과 명시적 공개 반영을 분리하고 저장 실패·로그인 만료·수정 충돌에도 작성 내용을 유지한다. 기존 편집·이미지·미리보기·주소·597부터의 발행을 유지하고 기본 서식 도구와 기존 글 서식 보존을 제공하며 공개 화면과 탐색을 보완하고 독립 사본 한 곳을 마련한다. | `진행 중` |
...
사용자에게 이는 **입력은 작업본에 자동저장되고, 공개 반영을 눌렀을 때만 방문자에게 보이는 글이 바뀐다는 뜻**이다. 저장 실패·로그인 만료·수정 충돌 때는 현재 입력을 유지한 채 같은 화면에서 이어갈 수 있다. 실제 글과 사진을 수정하거나 시험 발행하지 않았다. 사용자가 요청한 기본 서식 도구와 기존 서식 보존도 로컬 구현·실행 시험과 브라우저 확인을 마쳤다. 다음에는 이 기능을 실제 관리자와 공개 사이트 프로그램에 적용한 뒤 공개 글 화면 통합과 대표이미지 연결을 진행한다. 새 서식 기능의 운영 적용은 승인됐지만 로그인 권한 동의가 자동 승인 검토에서 차단돼 아직 운영 미적용이다.
...
1. 기본 서식 편집은 로컬 구현·시험·빌드를 마쳤고 사용자가 실제 서비스 적용도 승인했다. 그러나 자동 승인 검토가 기존에 승인받은 세 가지 로그인 권한의 동의를 차단해 아직 서비스는 바뀌지 않았다. dwnc.me가 속한 Cloudflare 계정의 계정 조회·Workers 프로그램 변경·D1 데이터베이스 변경 권한 동의를 다시 확인받아야 한다. 실제 작업은 승인된 서식 기능 적용에만 한정한다.
2. 새 글·사진의 독립 사본 위치는 후속 순서에서 정한다. 공유·스크랩 글 10개와 과거 댓글의 처리도 별도 선택이며 현재 CMS 구현을 막지 않는다.

### 바로 다음 작업

1. 차단된 로그인 권한 동의를 확인받은 뒤 이미 승인된 형식 열 추가 → 공개 사이트 프로그램 → 관리자 프로그램 적용을 이어간다. 기존 글·사진·정적 화면 파일과 주소 연결은 유지하며 운영 적용 자체의 재승인은 요구하지 않는다.
2. 운영 적용 후 이전 관리자 화면은 미저장 입력을 처리한 뒤 새로 열어 새 UI를 사용한다. Git push와 실제 글 수정/삭제, 사진 저장소의 덮어쓰기/삭제, 주소 연결 설정 변경은 하지 않는다.
3. 이어 기존·신규 공개 글 화면을 통합해 대표이미지·경로 안내·관련 글·이전/다음 글·목록 카드 이미지를 제공하고 홈·목록·글·SNS 연결, query 일관성, 목록·검색 조회 경량화, RSS·사이트맵·공유 최신화 순으로 진행한다.
4. 이어 관리자 검색·필터, 빈 초안 삭제, 신규 이미지 최적화와 새 글·사진의 단순한 독립 사본 한 곳을 마련한다.
...
- **Evidence:**
  ...
  - 합성 내장 브라우저에서 선택 서식·실행취소·링크·표·새 글 #597·미리보기·공개 반영·재열기·이미지 선택 중 본문 무변경을 확인했다. 모바일 실측은 도구가 크기 변경을 적용하지 않아 미확인으로 기록한다. 실제 운영 글은 변경하지 않았으며 외부 활성화까지 미완료 상태를 유지한다.
  - 마지막 연속 크기·굵기·색 지정 후 입력도 실제 스타일 유지로 확인했다. 최종 source-only 공개 빌드와 관리자 번들 검사를 통과했다. 세부 변경·검증·운영 적용 순서는 `PROJECT_STATE.md`에 기록한다.
  - 실제 서비스 적용 승인은 완료됐다. 자동 승인 검토의 로그인 권한 동의 차단으로 인증 발급·운영 변경 없이 대기 중이며 운영 적용 자체의 승인은 유지한다.
