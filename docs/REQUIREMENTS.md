...
**진행 중인 계획은 `PLAN-05` 하나다.** 공개 미디어 2,758개 업로드와 업로드 뒤 목록·크기·저장정보 확인은 끝났고, 빠진 항목과 불필요한 항목은 없었다. 다만 실제 Cloudflare 저장소에서 파일 내용을 하나씩 다시 내려받아 원본과 비교하는 검사는 아직 하지 않았다. Codex 자체 브라우저는 열려 있지만 현재 화면의 Cloudflare 계정이 이 프로젝트 계정인지는 확인 전이다. 다음 Cloudflare 작업을 시작할 때 올바른 계정인지 한 번 확인하고, 같은 세션과 탭에서 계속하는 동안에는 반복하지 않는다. 브라우저 세션·탭·프로젝트가 바뀌었거나 계정 상태가 달라졌을 때만 다시 확인한다.
...
1. 실제 파일 내용을 확인하기 전에 현재 화면의 Cloudflare 계정이 이 프로젝트 계정인지 한 번 확인하고, 저장소가 외부에 공개되지 않았는지 읽기만 해서 확인한다.
2. 파일을 방문자에게 보여 줄 시험용 사이트 프로그램은 아직 만들지 않았다. 실제 파일 내용 전수 비교를 마친 뒤, 프로그램이 없을 때만 최소 차단용 프로그램을 한 번 만든다.
3. 새 글 작성 방식과 비공개 자료의 백업·복구 방식은 최종 운영 전에 결정해야 한다. 공유·스크랩 글 10개, 과거 댓글과 추가 개선은 사용자가 원할 때 정하는 후속 선택이며 Stage 3를 막지 않는다.

### 바로 다음 작업

1. 현재 열린 자체 브라우저에서 이 프로젝트의 올바른 Cloudflare 계정인지 한 번 확인한다. 맞으면 `Account ID`를 한 번 복사해 이미 준비된 전용 보관 절차로 저장한 뒤 저장소 비공개 상태를 읽기만 해서 확인하고, 실제 파일 2,758개를 한 번 전수 비교한 뒤 이번 작업에 쓴 짧은 수명 열쇠와 사용 불가능한 기존 활성 열쇠를 정리한다. 자체 브라우저가 자동 복사를 지원하지 않을 때만 사용자에게 `Copy` 한 번을 요청한다.
...
사용자가 지금 별도로 할 일은 없다. 다음 Cloudflare 작업 직전에 현재 열린 자체 브라우저의 계정이 다르거나 확인할 수 없을 때만 직접 로그아웃·로그인을 요청한다. Chrome과 Edge는 사용하지 않는다. 각 단계마다 새 승인을 받거나 독립검토를 반복하기 위해 멈추지 않으며, 실제 `dwnc.me` 주소 연결은 `PLAN-08`에서 사용자가 결정할 때만 진행한다.
...
- **Evidence:**
  ...
  - 깨끗한 commit `fb693f3bbbab0e88b205f9de944646057bdd49ea`에서 계정 번호를 받기 전 준비 확인을 실제로 1회 실행했고 결과는 `ready`였다. 이때 Mac 보관함의 기존 상태는 읽기 1회만 했고 저장·변경은 0회였다. 클립보드는 읽기·비우기·쓰기가 모두 0회였고 실제 초기화도 0회다. 자체 브라우저는 현재 열려 있으며 다음 실제 작업은 이 프로젝트 계정인지 한 번 대조하는 것이다. Chrome과 Edge는 사용하지 않는다.
