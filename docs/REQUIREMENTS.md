...
**일반 본문18px와 크기 프리셋 비례 확대를 운영에 반영했다.** `DWNC-CORE-012`는 별도 착수 지시 전까지 `planned`를 유지한다. 진행 중인 계획은 `PLAN-09` 하나다.

사용자에게 이는 보통 본문과 상대 크기 단계가 함께 커지고, 직접 지정한 px 크기는 그대로 남는다는 뜻이다.
...
다음에는 사용자가 문의한 모든 글 대표이미지와 최근 기록 구성의 대안을 비교한다. 아직 레이아웃 구현은 시작하지 않았으며 다른 후속 항목은 별도 지시를 기다린다.
