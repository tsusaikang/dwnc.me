...
기존 글·서식·사진·공개 주소를 보존하면서 블로그를 지속 운영한다. 편집기 사진 정렬·폭 선택·2개/3개 한 줄 배치의 로컬 구현·검증을 마쳤으며 운영 반영은 남아 있다. 신규 기능의 세부 범위는 [신규 기능 요구사항](신규_기능_요구사항_20260911_173052.md)에 둔다. Git 자동 배포 전환은 관리자 내부 읽기 확인만 남아 있다. 별도로 등록한 결함 6건은 착수 대기이며, 세부 범위와 완료 조건은 [결함 보완 요구사항](결함_보완_요구사항_20260909_223512.md)에 둔다.
...
**이미지 배치 기능 배포 진행.** `DWNC-CORE-013`의 구현·시험·내장 브라우저 확인 후 사용자가 2026-09-11 “그냥 배포해주라”로 운영 반영을 명시 승인했다. 기존 Git 자동 배포 경로로 최종 소스를 전달하고 양쪽 반영을 확인한다. `DWNC-OPS-007`은 관리자 Access 로그인 후 내부 읽기 확인 대기이며 `DWNC-CORE-012`의 결함 6건은 별도 착수 대기를 유지한다. 진행 중인 계획은 `PLAN-09` 하나다.

사용자에게 이는 사진 위치·크기·묶음을 고르는 기능이 로컬에서 준비됐지만 현재 홈페이지에서는 아직 사용할 수 없다는 뜻이다.

### 아직 결정할 일과 진행을 막는 조건

- 새 이미지 기능의 전송·운영 반영은 이번 명시 승인으로 진행하며 같은 승인을 다시 요청하지 않는다.
- 기존 결함 6건 보완은 사용자의 후속 착수 지시를 기다린다.
- 이번 소스 전달·자동 배포의 명시 승인과 push는 완료했다. 빌드 환경 차이로 생긴 오류를 보완했으며 양쪽 자동 빌드·배포와 공개 화면 확인을 완료했으며 관리자 Access 로그인 후 내부 읽기 확인이 남아 있다.
...
다음에는 명시 승인된 `DWNC-CORE-013`의 최종 소스를 Git으로 전달하고 양쪽 자동 배포를 확인한다. 운영 반영 시 최종 소스를 새로 빌드하고 실제 콘텐츠를 변경하지 않는 읽기 확인을 한다. `DWNC-OPS-007`은 사용자 로그인 후 읽기 확인하고, `DWNC-CORE-012`는 별도 착수 지시 전까지 보류한다.
...
- **Acceptance:**
  ...
  - 구현된 기능과 보존 조건은 [신규 기능 요구사항](신규_기능_요구사항_20260911_173052.md)을 유지한다. 기존 글·작업본·사진·공개 주소·미디어 접근 정책을 보존하고 실제 글을 시험 변경·삭제하지 않는다.
- **Evidence:**
  ...
  - 사용자가 2026-09-11 신규 기능의 배포를 명시 승인했으며 실제 반영을 진행한다. 실제 IME·운영체제 메뉴 실행 취소는 직접 확인하지 않았으며 관련 입력 경로는 합성 시험 근거만 있다. `CORE-012`의 별도 착수 대기를 유지한다.
