...
1. 티스토리를 참고한 작성 화면은 로컬 구현·시험·내장 브라우저 확인을 완료했다. 사용자 승인 후 실제 관리자 프로그램에도 반영했다. 관리자 로그인이 만료되어 사용자 재로그인 후 실제 화면 확인만 남았다. 기본 서식 기능의 운영 적용을 막던 로그인 권한 동의는 사용자 동의 후 해결됐다. 운영 적용은 완료됐으며 현재 이 기능의 남은 적용 장애물은 없다. 이전 운영 화면은 700px에서 확인했고, 새 로컬 작성 화면은 1280/700/390px 내장 브라우저 확인을 완료했다.
2. 새 글·사진의 독립 사본 위치는 후속 순서에서 정한다. 공유·스크랩 글 10개와 과거 댓글의 처리도 별도 선택이며 현재 CMS 구현을 막지 않는다.

### 바로 다음 작업

1. 자체 내장 브라우저에서 사용자 관리자 로그인이 끝나면 운영에 반영된 새 작성 화면의 목록·편집 이동·서식 도구·미리보기를 읽기로 확인한다. 재배포할 필요는 없다. 기존 입력 보존·작업본 자동저장·명시적 공개 반영과 오류 처리는 그대로 유지한다. 공개 프로그램·DB·사진·글 내용·주소 연결은 변경하지 않는다.
2. 기존·신규 공개 글 화면을 통합해 대표이미지·경로 안내·관련 글·이전/다음 글·목록 카드 이미지를 제공하고 홈·목록·글·SNS 연결, query 일관성, 목록·검색 조회 경량화, RSS·사이트맵·공유 최신화 순으로 진행한다.
3. 이어 관리자 검색·필터, 빈 초안 삭제, 신규 이미지 최적화와 새 글·사진의 단순한 독립 사본 한 곳을 마련한다. Git push와 실제 글 수정/삭제, 사진 저장소 덮어쓰기/삭제, 주소 연결 설정 변경은 하지 않는다.
...
- **Evidence:**
  ...
  - runtime source `347758395dece6868b39750e344cda60d8f14893`의 새 작성 화면을 사용자 승인 후 실제 관리자에 적용했고 활성 코드는 준비 번들과 일치한다. DB·공개 프로그램·사진·실제 글은 변경하지 않았다. 관리자 이메일 OTP 재로그인 후 운영 화면 확인이 남아 미완료 상태를 유지한다.
