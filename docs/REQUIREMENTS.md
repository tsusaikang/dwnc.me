...
**별도 결함 보완과 독립 사본·보관 정책의 착수 판단을 기다린다.** `DWNC-CORE-012`는 별도 착수 지시 전까지 `planned`를 유지한다. 진행 중인 계획은 `PLAN-09` 하나다.

사용자에게 이는 현재 서비스는 계속 이용할 수 있고, 남은 보완 범위는 별도로 선택한다는 뜻이다.
...
다음에는 사용자의 별도 지시에 따라 CORE-012 또는 독립 사본·보관 정책을 진행한다. 그 전에는 현재 운영 상태를 유지한다.
