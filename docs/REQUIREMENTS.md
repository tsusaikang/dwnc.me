...
**승인된 편집기 개선과 자체 작성 글 보완을 마감했다.** `DWNC-CORE-012`는 별도 착수 지시 전까지 `planned`를 유지한다. 진행 중인 계획은 `PLAN-09` 하나다.

사용자에게 이는 현재 서비스를 계속 이용할 수 있고, 남은 결함 보완과 백업 등은 별도 후속 작업이라는 뜻이다.
...
다음에는 사용자가 별도로 착수 지시한 후속 작업을 진행한다. CORE-012와 백업·보관 정책은 현재 임의로 시작하지 않는다.
