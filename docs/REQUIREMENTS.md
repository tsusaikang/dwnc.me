...
기존 글·서식·사진·공개 주소를 보존하면서 블로그를 지속 운영한다. 운영 최신 콘텐츠의 링크·대표이미지·요약 전수조사는 다음번 할 일로 등록했다. 별도로 등록한 결함 6건은 착수 판단을 기다린다. 세부 결함 범위와 완료 조건은 [결함 보완 요구사항](결함_보완_요구사항_20260909_223512.md)에 둔다. 이미 완료된 기능과 배포 결과는 완료 원장과 공식 상태에서 확인한다.
...
**운영 최신 콘텐츠의 링크·대표이미지·요약 전수조사를 다음번 착수할 할 일로 등록했다.** `DWNC-CORE-021`은 사용자 요청에 따라 `planned`이며 이번에는 조사하지 않는다. AI는 관련 운영 DB/API의 최신 반영본을 직접 읽고 미공개 작업본과 구분해야 한다. 과거 덤프를 최신 입력으로 대신 쓸 수 없다. 앞선 6개 글·7개 카드 조사는 과거 공개 원본을 대상으로 했으며 운영 DB 전체를 조회한 전수조사가 아니다. `DWNC-CORE-012`도 별도 착수 지시 전까지 `planned`를 유지한다. 진행 중인 계획은 `PLAN-09` 하나다.
...
- 운영 최신 콘텐츠 전수조사는 다음번 착수 지시를 기다리며, 이번에는 요구사항 등록만 한다.
...
이번에는 전수조사를 실행하지 않고 `DWNC-CORE-021`을 다음번 착수 대기로 유지한다. 다음에는 사용자의 후속 착수 지시에 따라 새 글과 기존 글 수정분을 포함한 운영 DB/API의 현재 전체 목록으로 대상을 확정한 뒤 최신 반영본과 실제 표시를 비교한다. 미공개 작업본과의 차이 및 남아 있는 정적 대체 경로는 구분한다. 조회·수정 범위와 사용자 수정 보존은 [AI 작업의 콘텐츠 기준](../AGENTS.md#ai-작업의-콘텐츠-기준)을 따른다.

### 이번 작업에서 하지 않는 것

- `CORE-021` 운영 최신 콘텐츠 전수조사 실행과 조사 전제의 콘텐츠 일괄 수정·공개
...
- **Priority:** `P1`
- **Acceptance:**
  - 사용자가 다음번에 조사하도록 지정했으므로 이번에는 할 일 등록만 하고, 후속 착수 지시 전에는 조사하지 않는다.
