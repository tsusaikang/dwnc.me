...
사용자에게 이는 **운영 서비스의 자동저장은 아직 공개 글에 바로 반영되지만, 새 버전을 적용하면 작업본에만 저장하고 사용자가 공개 시점을 정할 수 있다는 뜻**이다. 저장 실패·로그인 만료·다른 탭의 수정 충돌도 같은 편집 화면에서 처리한다. 이번 작업에서는 실제 글과 운영 서비스를 변경하지 않는다. 다음에는 완성된 관리자 기능을 실제 서비스에 적용할지 승인받는다.
...
1. 로컬 구현과 시험은 완료됐다. 실제 서비스에 빈 작업본 저장 공간을 추가하고 완성된 관리자 버전을 활성화하는 승인이 남는다. 기존 공개 글과 사진, 공개 사이트 주소는 그대로 유지하며 활성화 전까지 운영 관리자는 기존 자동저장 방식을 사용한다.
2. 새 글·사진의 독립 사본 위치는 후속 순서에서 정한다. 공유·스크랩 글 10개와 과거 댓글의 처리도 별도 선택이며 현재 CMS 구현을 막지 않는다.

### 바로 다음 작업

1. 완성된 작업본 저장·공개 반영 기능을 실제 관리자 서비스에 적용할지 승인받는다. 승인 후 빈 작업본 저장 공간 추가와 관리자 버전 활성화를 진행한다. 공개 사이트 프로그램이나 주소 연결을 바꿀 필요는 없다.
