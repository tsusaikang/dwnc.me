...
**같은 카테고리 글 이동을 배포하고 본문 기본 글꼴을 제목 계열로 바꾸고 있다.** `DWNC-CORE-012`는 별도 착수 지시 전까지 `planned`를 유지한다. 진행 중인 계획은 `PLAN-09` 하나다.

사용자에게 이는 같은 카테고리를 이어 읽고, 일반 본문을 제목과 같은 글꼴 계열로 볼 수 있다는 뜻이다.
...
다음에는 기본 글꼴 변경을 확인하고 카테고리 이동 보완을 포함한 최종 소스 Git 자동 배포와 운영 확인을 마친다. 다른 후속 항목은 별도 지시를 기다린다.
