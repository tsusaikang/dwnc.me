...
새로 확인된 누락: 원본 비공개247편은 로컬 보존만 됐고 운영 CMS에는 이전되지 않아 관리자 목록에도 없다. `DWNC-CORE-011`에서 관리자 전용 온라인 이전을 다룬다. 기존 공개 글과 기본 관리 기능 완료 상태는 유지하되 전체 원본 글의 관리자 이전 완료로 해석하지 않는다.
...
- **Evidence:**
  ...
  - 로컬 비공개 글 디렉터리247개 및 정규화 Markdown247개의 존재를 집계했다. 이번에는 비공개 내용을 출력하거나 외부 전송하지 않았고 운영 DB를 새로 조회하지 않았다.
