...
`DWNC-CORE-023` 긴 제목 입력칸 자동 줄바꿈·높이 조절을 진행한다. 진행 중인 계획은 `PLAN-09` 하나다. 별도 결함6건과 독립 사본은 기존 후속 조건을 유지한다.

사용자에게 이는 긴 제목을 끝까지 보면서 편집할 수 있도록 입력칸을 보완하는 중이라는 뜻이다.
...
다음에는 제목 입력칸 구현·관련 시험·실제 화면 확인을 마치고 기존 Git 자동 배포로 운영에 반영한다. 운영 콘텐츠와 미공개 작업본을 보존한다.
