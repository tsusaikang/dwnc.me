...
**진행 중인 계획은 `PLAN-05` 하나다.** 다음 단계인 `PLAN-06`은 아직 대기 상태지만, 시험용 사이트 프로그램을 안전하게 만들기 위한 로컬 모의검사는 미리 보강했다. 공개 미디어 2,758개의 업로드와 업로드 뒤 목록 확인은 끝났다. 인터넷 없는 예행연습에서는 로컬 원본 2,758개, 약 2.35GB를 사용해 파일 목록을 3회에 나눠 읽고 각 파일의 정보와 실제 내용을 한 번씩 확인했다. 모두 합해 5,519회 읽었으며 파일 추가·변경·삭제는 0회였다. 실제 Cloudflare 저장소의 모든 파일을 다시 내려받아 원본과 비교하는 검사는 남아 있다. Cloudflare 계정 번호는 사진 업로드용 열쇠와 섞지 않고 Mac의 안전한 보관함에서 불러오도록 만들었다. 이 번호는 로그인 열쇠가 아니라 올바른 계정을 고르는 확인 표지다. 계정 번호 저장 전 준비 확인은 이미 완료됐지만 실제 번호 저장과 초기화는 아직 0회다. Codex 자체 브라우저의 로그인 상태가 프로젝트마다 따로 유지된다고 보장할 수 없으므로, 다른 프로젝트에서 쓰던 Cloudflare 계정이 그대로 나타날 수 있다고 보고 작업한다. 브라우저에 로그인돼 있다는 사실만 믿지 않고, 이 프로젝트에 저장된 계정 확인값과 화면의 계정이 정확히 맞는지 Cloudflare 작업을 시작할 때마다 다시 확인한다. 시험용 사이트 프로그램을 처음 만드는 도중 프로그램이 멈춰도 같은 생성 명령을 다시 실행하지 않고 현재 상태만 두 번 확인하는 로컬 복구 장치도 완성했다. 시험용 사이트 프로그램이 예전 주소 349개를 직접 새 주소로 안내하는 기능과 원격 점검 프로그램의 로컬 안전 보강은 완료했다. 실제 시험용 주소에서 이 점검을 실행한 것은 아니다.

사용자에게 이는 **파일은 모두 시험용 저장소에 들어갔고 점검 프로그램도 전량 예행연습을 마쳤지만, 실제 저장소 내용을 한 개씩 다시 확인하는 마지막 검사는 남았다는 뜻**이다. 시험용 사이트 프로그램 자체를 올리거나 적용하지는 않았지만, 나중에 만들 때 잘못된 계정·주소·설정·코드가 끼어들면 멈추는 로컬 안전장치는 강화했다.
...
3. Codex 자체 브라우저의 로그인 상태가 프로젝트별로 분리된다는 보장은 확인되지 않았다. 같은 사이트에서 서로 다른 계정을 쓰는 프로젝트의 브라우저 작업은 동시에 하지 않고 하나씩 진행한다. 다른 프로젝트 작업 뒤 이 프로젝트로 돌아오면 계정을 다시 확인하며, 맞는 계정인지 알 수 없거나 다르면 설정 조회·변경과 열쇠 생성 전에 멈추고 사용자에게 직접 로그아웃·로그인을 요청한다.
4. 파일을 방문자에게 보여 줄 시험용 프로그램은 아직 만들지 않았다. 저장소의 모든 파일 내용이 원본과 같은지 확인한 뒤 진행한다.
5. 공유·스크랩으로 보이는 글 10개, 새 글 작성 방식, 과거 댓글, 비공개 자료의 암호화 백업 방식은 Stage 3와 관계없는 사용자 결정으로 남아 있다.

### 바로 다음 작업

1. 계정 번호 저장 사전검사는 이미 준비 완료다. Codex 자체 브라우저 안에 Cloudflare 페이지를 준비했지만, 브라우저 패널이 화면에 나타나지 않은 상태다. 사용자는 앱의 브라우저 버튼이나 `Command+Shift+B`를 눌러 패널을 연 뒤 이 대화에 **“자체 브라우저 열림”**이라고 알려 준다. 그 다음 화면에 나타난 계정이 이 프로젝트의 올바른 계정인지 확인한다. 맞는지 알 수 없거나 다른 계정이면 사용자가 직접 로그아웃하고 올바른 계정으로 로그인한 뒤 `Account ID` 옆 `Copy`를 한 번만 누른다. 그러면 초기화를 실행해 계정 번호를 Mac의 전용 보관함에 저장한다. Chrome과 Edge는 사용하지 않으며 로그인 정보와 비밀번호는 저장하지 않는다.
...
6. 운영용 이름의 Cloudflare 자원이 더 필요하면 같은 순서로 준비하되 실제 `dwnc.me` 주소가 새 사이트를 가리키도록 연결하지 않는다.

사용자가 지금 할 일은 앱의 브라우저 버튼이나 `Command+Shift+B`로 자체 브라우저 패널을 열고 **“자체 브라우저 열림”**이라고 알려 주는 것이다. 패널을 연 뒤 이 프로젝트의 올바른 Cloudflare 계정인지 먼저 대조하며, 계정이 다르거나 알 수 없을 때만 사용자가 직접 로그아웃·로그인한다. Chrome과 Edge는 앞으로 이 프로젝트에 사용하지 않는다. 실제 `dwnc.me` 주소 연결은 `PLAN-08`에서 사용자 결정이 있어야만 진행한다. 이미 승인된 시험용 작업은 안전 조건이 충족되면 단순히 승인을 다시 받기 위해 멈추지 않는다. 앞선 전량 예행연습을 저장할 당시 이번 변경은 로컬 파일 12개에만 한정됐고 인터넷 저장소로 보내지지 않았다. 지금 반영한 브라우저 정책은 로컬 문서 2개에만 한정한다.
...
- 현재 로컬 기준은 commit `fb693f3bbbab0e88b205f9de944646057bdd49ea`·tree `5e7169ca35a61a077265c98f6819fa28ccc5126d`·parent `d0b4a6c68c8a51b3211c97e320c5d7ea0310f437`다. 정확히 12개 파일의 `+1,990/-42` 변경을 묶었고, 변경 파일 집합 확인값은 `94fb0e5a8c2e67def5f4b1460431170eaae5f9505f78c7d900bea9446ead8374`다. 저장 직후 로컬 파일 상태는 깨끗했고 Git push는 0회다.
...
- **Evidence:**
  ...
  - 깨끗한 commit `fb693f3bbbab0e88b205f9de944646057bdd49ea`에서 계정 번호를 받기 전 준비 확인을 실제로 1회 실행했고 결과는 `ready`였다. 이때 Mac 보관함의 기존 상태는 읽기 1회만 했고 저장·변경은 0회였다. 클립보드는 읽기·비우기·쓰기가 모두 0회였고 실제 초기화도 0회다. Codex 자체 브라우저 안에 Cloudflare 페이지를 준비했지만 패널이 화면에 나타나지 않았다. 다음 순서는 **사용자가 앱의 브라우저 버튼이나 `Command+Shift+B`로 패널 열기 → “자체 브라우저 열림”이라고 알리기 → 이 프로젝트 계정인지 대조 → 필요할 때만 직접 로그아웃·로그인 → `Account ID` 옆 `Copy` 한 번 → 초기화**다. Chrome과 Edge는 사용하지 않는다.
...
- **Acceptance:**
  ...
  - 자체 브라우저의 로그인 상태가 프로젝트·작업마다 분리된다고 가정하지 않는다. 같은 사이트에서 서로 다른 계정을 쓰는 프로젝트의 브라우저 작업은 동시에 하지 않고 하나씩 진행하며, 다른 프로젝트를 다녀온 뒤에는 이 프로젝트 계정을 다시 확인한다.
  - 브라우저에 로그인돼 있다는 사실은 올바른 계정의 증거로 사용하지 않는다. 모든 Cloudflare 브라우저 작업을 시작하기 직전에 화면의 `Account ID` 또는 그 확인값이 이 프로젝트에 안전하게 저장된 계정 확인값과 일치하고, 일치하는 대상 계정이 정확히 한 개뿐인지 확인한다. 다르거나 확인할 수 없으면 설정 조회·변경과 열쇠 생성 전에 멈추고, 사용자가 직접 로그아웃·로그인한다.
...
- **Evidence:**
  ...
  - Cloudflare 페이지는 자체 브라우저 안에 준비했지만 브라우저 패널이 화면에 나타나지 않았다. 다음 사용자 동작은 앱의 브라우저 버튼이나 `Command+Shift+B`로 패널을 열고 **“자체 브라우저 열림”**이라고 알리는 것이다.
