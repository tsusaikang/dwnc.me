...
1. 운영에 반영한 새 작성 화면을 사용한다. 미저장 입력이 있는 이전 탭은 내용을 먼저 처리한 뒤 새로 연다. 기존 입력 보존·작업본 자동저장·명시적 공개 반영과 오류 처리는 그대로 유지한다. 공개 프로그램·DB·사진·글 내용·주소 연결은 변경하지 않는다.
