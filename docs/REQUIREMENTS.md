...
**자동 링크 운영 반영 준비 중.** `DWNC-CORE-017`은 티스토리 실제 UX 확인과 자체 작성 글 누락 링크 공개 보완을 마쳤다. 최종 입력·붙여넣기 브라우저 확인까지 마쳤고 기능 배포가 남았다. `DWNC-CORE-012`는 별도 착수 지시 전까지 `planned`를 유지한다. 진행 중인 계획은 `PLAN-09` 하나다.

사용자에게 이는 앞으로 입력하는 웹주소에 자동 링크를 붙이고 원하면 해제할 수 있도록 개선하며, 자체 작성 글의 누락만 보완한다는 뜻이다.

### 아직 결정할 일과 진행을 막는 조건

- 로그인 승인은 받았고 기존 프로젝트 계정 접근을 확인했다. 현재 사용자의 추가 결정은 필요하지 않다.
...
다음에는 최종 소스 Git 자동 배포와 운영 편집기 확인을 진행한다. 원래 링크가 없던 이전 글은 그대로 둔다. CORE-012는 별도 착수 지시를 기다린다.
...
- **Evidence:**
  - 후속 승인에 따라 핵심 구현과 로컬 회귀·브라우저·source-only 빌드를 마쳤다. [구현 진행 기록](history/자동링크_구현_진행_20260913_201937.md)에 기능 배포 전 상태와 티스토리 관찰·자체 글 링크 공개 보완 결과를 구분했다.
