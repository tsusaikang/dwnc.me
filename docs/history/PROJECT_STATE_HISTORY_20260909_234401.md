# 프로젝트 상태 이력 — 2026-09-09까지

이 문서는 이전 상태 문서의 작업·검증·결정 원문을 옮긴 역사 자료다. 각 시점의 완료·대기·현재 표현은 당시 기록이며 지금의 지시나 운영 상태가 아니다. 현행 정본은 [PROJECT_STATE.md](../../PROJECT_STATE.md)와 [현재 요구사항](../REQUIREMENTS.md)을 따른다. 아래 원문의 상대 링크만 새 위치에 맞췄다.

---

# dwnc.me 프로젝트 공식 상태

최종 갱신: 2026-09-09 KST — 확인된 결함6건의 보완 요구사항 등록, 구현 착수 대기 (코드 수정·배포 없음)

### 최신 요구사항 — DWNC-CORE-012 결함 보완 등록

- **사용자 요청/완료조건:** 확인된 결함 중 어렵거나 불가능한 항목을 구분하고 수정할 범위를 요구사항으로 작성·등재한다. 사용자가 바로 진행하지 말라고 명시했으므로 이번 완료는 문서 등록까지이며 구현·시험 추가·빌드·배포를 시작하지 않는다.
- `docs/결함_보완_요구사항_20260909_223512.md`를 작성하고 `docs/REQUIREMENTS.md`의 `DWNC-CORE-012`/`PLAN-09`에 `planned`로 등록했다. F01 사진삭제 실행취소, F02 CSS배경전용 사진 공개참조, F03 검색실패 재시도, F04 오래된 검색응답 무시, F05 초안생성 응답유실 중복방지, F06 사진업로드 응답유실 중복방지의6건이다. 이전 감사표는 변경하지 않고 근거로 연결했다.
- 수정 가능성: 검색2건은 낮음, CSS참조는 낮음~중간, 사진 실행취소·생성/업로드 재시도는 중간의 상대 난이도로 판단했다. 현재 소스에서 현실적 수정 경로가 있어 불가능/과도한 난도로 제외한 결함은 없다. 확정 공수·모든 환경의 해결 보증은 아니다. F06 응답유실은 소스확인 단계임을 유지하고 향후 합성 재현을 첫 인수조건으로 두었다.
- 경계: 백업·휴지통·일반삭제·과거버전복원·강제종료 복구·새첨부형식·실기기/대규모 장애 검증 등은 신규 기능 또는 미확인 범위이므로 이번6건에 넣지 않았다. 이를 불가능하다고 판정한 것은 아니다. 동일 사진을 별도로 삽입하려는 의도는 보존하며 기존 중복자료 정리·R2 덮어쓰기/삭제·전역 중복제거는 요구하지 않는다.
- 인수조건에는 합성 재현 해소, 주변 서식/작업본/공개본 보존, 비공개·예약·보호·소유권 유지, 재시도/동시처리/서로 다른 요청 구분, 향후 최종 소스 새 빌드와 전체 운영 반영·읽기 전용 확인을 명시했다. 이번 단계의 배포나 개발 착수 승인이 아니며 후속 사용자 착수 지시가 있을 때 시작한다.
- 역할: 하위 에이전트가 소스 기반 가능성 평가·상세 요구사항·원장 등재를, 메인이 범위·인수조건 검토와 공식 상태 통합을 맡았다. 기존 미커밋 점검 보고서/상태 변경은 보존했다. 서비스 코드·실제 콘텐츠·설정·사진·권한·원격 Git 변경은 없다. 다음 행동은 사용자 요구사항 검토와 후속 착수 지시 대기다.
- 문서 검증: requirements:validate PASS(current19/recent12/archive14), git diff --check PASS. 기존 원장의 CORE-011 done 항목을 완료 절로 내용 보존 이동하고 누락된 Priority P1을 보완했다. 최근 완료12개 유지 규칙에 따라 가장 오래된 DWNC-S3-011은 REQUIREMENTS_ARCHIVE.md로 원문 이동했다. 이는 요구사항 이력 정리이며 서비스 동작 변경은 아니다.

### 최신 점검 — 블로그 운영자 상황별 대응 범위

- **사용자 요청/완료조건:** 운영자가 겪는 경우를 명시적으로 나열하고 현재 대응 여부를 확인까지만 수행한다. 이전 수정·배포 승인보다 이번 확인 전용 범위를 우선하며 기능 수정·운영 반영은 하지 않는다. 점검은 완료했으나 모든 경우에 대응하는 상태로 판정하지 않는다.
- 산출물: `docs/운영자_상황별_대응_점검_20260909_223042.md`. 작성37·생애주기43·운영39의119개 시나리오 및 글/페이지/공지 각각20개의 상태전환, 총60분기를 명시했다. 관점 중복이 있으므로119는 독립 기능 수나 품질 점수가 아니다. 지원·제한/미지원·결함·미확인과 소스/합성시험/이번·기존 운영 실측을 구분한다.
- 확정 재현: 사진 삭제 후 실행취소로 복원되지 않음(메인 합성브라우저 사진1→삭제0→실행취소0); 이관 비공개 글의 CSS배경전용 미디어 공개·보호·예약 공개 후404; 검색 첫 통신실패 뒤 새로고침 전 재시도 불가; 검색어를 지운 후 늦은 이전 결과 표시. 새 초안/사진 생성 성공 후 응답 유실 재시도는 중복을 만들 수 있음(초안 중복 합성호출 확인, 업로드는 소스 근거로 구분).
- 미지원/미완료: 강제 종료 후 서버 미저장 입력 복구, 일반글 삭제·휴지통·과거 편집본 복원, 신규 HEIC/영상/일반파일 첨부, 일괄관리, 신규 글·사진 독립 백업·전체복원·내보내기. 실기기 키보드/IME·접근성 전체·부하급증·계정상실·플랫폼장애·비용/장애알림은 미확인이다. 댓글·방명록·구독·팀블로그 제외와 광고 후순위는 합의된 범위이며 결함과 구분한다.
- 검증: 하위 에이전트가 기존 작성/서식/작업본/생애주기/미디어/통계/권한 관련 합성 시험을 실행해 PASS. 추가 메모리DB 상태전환60분기 PASS. 검색 실제소스 VM과 CSS 공개참조 합성에서 결함 재현. 임시 상세자료 `/private/tmp/dwnc-audit-authoring.md`, `dwnc-audit-lifecycle.md`, `dwnc-audit-operations.md`, 재현 `/private/tmp/dwnc-audit-lifecycle-check.mjs`, `dwnc-search-audit.mjs`. 실제 장애·부하·콘텐츠복원 시험은 실행하지 않았다.
- 메인 현재 내장브라우저: 공개 홈/검색 결과/결과없음/전체글349연결, 관리자 전체596·비공개247 및 필터/관리메뉴 확인. 실제 비공개 제목·본문·식별자는 출력하지 않았다. 기존 사용자 탭을 보존하고 점검용 탭은 닫았다. 합성서버31935는 담당 에이전트가 종료코드0으로 종료했다. 메인은 브라우저·통합·공식상태를, 세 하위 에이전트는 분야별 실무 점검을 담당했다.
- 서비스 코드·실제 글·작업본·설정·사진·권한·DNS/route·Git 원격은 변경하지 않았다. 이번 변경은 점검 보고서와 공식 상태 문서뿐이다. 다음은 사용자가 결과를 검토하는 단계이며 자동으로 보완 개발·배포를 시작하지 않는다. CORE-011의 비공개 이전 완료는 유지하되, 알려진 향후 공개전환 결함 및 장기운영 공백을 이 점검 결과와 함께 이해한다.

### 최신 완료 — DWNC-CORE-011 비공개 글 관리자 이전

- **완료:** 승인된 원본 비공개247편과 참조 미디어2731개(사진2726·MP4 5, 총1,316,018,054bytes)를 운영 CMS에 비공개로 신규 이전했다. 메인 내장 브라우저에서 비공개 필터의 `247개의 글`, 대표 실제 본문과 사진2개 정상 로딩을 읽기만으로 확인했다. 편집·작업본 저장 연결은 합성 브라우저와 실제247편 메모리 정규화 시험으로 확인했으며 실제 글을 시험 삼아 수정하지 않았다. 최종 관리자 비공개 목록 탭을 전달용으로 남겼고 기존 사용자 탭은 보존했다.
- 최종 source `afde3155c4d34d4320bfaabe24cc1ac7e823d3bc` clean에서 public/admin을 새 빌드해 각각 `8c5784a4-7b8b-4872-8e77-e8d8cd2779eb`, `129aa5f7-08a8-4497-bd6c-85c1c971d2c8`를100% 활성화했다. 모듈 일치와 기존 settings/bindings 보존을 확인했다. public2436820bytes/SHA256 `b44ac344e5c0c4d0be4ae9407e228a40a38ecbc1b825849fd94af24aa0d0b7e0`, admin2563921bytes/SHA256 `4df8f14fd28d5fe2622860d0ea2c4c453f0c4297e8bf059ab1e34870e84b285b`. dist1420files/38677462bytes/tree `ca2cad092356a62bd30d8e65334c79b74e356c32d49746c18be67cd5c13a7c24`는 불변이므로 keepAssets:true로 자산 재업로드를 생략했고 schema 변경도 없다. 구성 `/private/tmp/dwnc-core011-config.json`, 빌드·시험 로그 `/private/tmp/dwnc-core011-source-build.log`, `/private/tmp/dwnc-core011-build-bundles.log`, `/private/tmp/dwnc-core011-final-check.log`, `/private/tmp/dwnc-core011-runtime-tests.log`.
- 운영 전송은 동일 승인 OAuth3권한의 메모리 전용 세션96155와 표준 임시 remote-bindings preview로 완료했다. optional exchange400은 표준 원래 세션 token fallback으로 처리했다. 첫 전송에서2편 완료 뒤 일시 연결 오류가 발생했으며 같은 세션 재개에서 동일2편·21미디어를 일치 확인 후 건너뛰었다. 최종 실행은 신규245편/2710미디어 + 기존일치2편/21미디어, 총247편/2731미디어이며 매 글의 신규삽입 후 private정책·본문·미디어행 일치를 확인했다. overwrite0/delete0, 기존 글/작업본 UPDATE0, 원본·예약번호 보존. 런타임 재업로드 없이 재개했으며 완료 후 인증 세션은 stop으로 정상 종료했다. 비밀·실제payload 파일은 작성하지 않았다.
- 이전 완료 후 공개 차단 검사 PASS: 대표 canonical3·alias3·파일유형별 미디어6 요청은404/no-store였고 home/archive/category/search/RSS/sitemap6면에 비공개 canonical 노출0. 검사 도우미 `/private/tmp/dwnc-private-public-check.mjs`는 내용·ID·주소를 출력하지 않는다. 관리자 시험도 제목·본문·사적분류·URL을 기록하지 않고 집계만 반환했다. 전송 도우미의 일시 연결 재시도 보완은 다음 세션용으로만 준비됐으며 정상 진행 중인 이번 세션을 교체하지 않았다.
- 외부iframe1은 원본 제목·링크로 보존하고, 원본 CSS 배경61참조와 SVG/ICO/MP4도 포함했다. 향후 CSS 배경전용 미디어를 공개 전환하려면 공개전환 참조 수집 보완이 필요하지만 이번 관리자 전용 이전에는 영향이 없고 공개 전환은 실행하지 않았다. 비공개 사진·본문의 일반 방문자 차단을 유지한다.
- 메인은 브라우저/사용자 인증/직렬 운영/공식 상태, 하위 에이전트는 importer·runtime·전송 준비·시험을 담당했다. Git push/raw deploy/DNS·route 변경/R2 덮어쓰기·삭제/기존 실제 글 변경·삭제는 없다. 이번 누락 해결의 미완료 작업은 없으며 PLAN-09 독립 사본·신규 이미지 최적화 등 별도 후속은 유지한다. 아래 승인 대기·미전송·로컬 구현 표현은 완료 전의 이력이다.

### 진단·구현 이력 — 기존 비공개 글의 관리자 이전 누락 (위 완료로 해소)

- **사용자 승인:** `그렇게 하도록 승인할게.`로 비공개247편의 본문·사진을 새 블로그의 비공개 저장소로 신규 이전해 로그인한 관리자만 조회·편집하도록 승인했다. 이 범위는 기존 로컬전용 자료의 외부전송 및 신규 CMS행/사진 생성을 허용한다. 원본 보존·기존 글/작업본/예약번호 유지·공개 전환 금지·R2 덮어쓰기/삭제 금지·비밀/비공개 콘텐츠 로그/Git/공개빌드 기록 금지는 계속 적용한다. 아래의 범위 동의 대기는 이 승인으로 해소됐다.
- 구현 배정: reconcile_scope는 별도 private importer/합성시험, private_runtime은 기존 관리자·미디어·공개차단 연결과 관련 최소 수정/시험, private_delivery는 메모리 전용 운영 전송 준비를 담당한다. 메인은 계약 통합·내장 브라우저·직렬 운영 실행·공식 상태를 맡는다. 현재 실제 비공개자료 외부전송은 아직 없다. 완료조건은 누락247 추가, 관리자 목록/본문/사진/편집 연결, 방문자 글·사진 차단, 기존 공개글/작업본 불변 확인이다.
- 로컬 구현/검토: `scripts/lib/private-post-import.mjs`와 집계 전용 CLI/합성시험을 추가했다. 원본은 메모리에서만 읽고 원본 본문 및 참조파일의 SHA/크기와 예약순번을 대조한다. 기존 `legacy_posts`·`content_operations(private)`·`legacy_media`를 한 D1 batch의 신규 INSERT로 넣으며 기존 행/작업본은 갱신하지 않는다. 완료된 동일 이전은 건너뛰고 충돌은 중단한다. 별도 DB migration은 없다.
- 참조 범위: 글247, 사진2726+로컬 MP4 5=2731파일, 총1,316,018,054bytes, 최대파일4,749,494bytes, 최대본문119,025bytes. CSS 배경의61개 url 참조까지 포함해 주소를 치환했고, 상대/로컬 미디어 경로 잔류0을 확인했다. 배경전용 ICO8/GIF1도 포함하며 원본 파일은 로컬에 계속 보존한다. 외부iframe1은 원본 제목/링크를 남겨 자동 외부요청을 없앴다. 공개 분류에 맞지 않는239편은 원래 분류계층/말단명을 관리자에 보존하며 전역분류에 사적명칭을 추가하지 않는다.
- runtime은 원래 분류의 현재값만 저장 허용하고 공개·예약·보호 전환에는 공용분류 선택을 요구한다. ICO도 imported 경로에서 인증된 관리자에게 제공한다. SVG는 원본을 유지하며 CSP sandbox로 스크립트/외부요청을 차단하고 MP4는 controls 및 byte Range를 지원한다. 비공개 글·미디어는 공개 정책 확인 뒤 차단한다. runtime/편집기 전체 시험 PASS, Astro 오류0/경고0, importer 신규삽입·원자rollback·재시도·기존작업본 보존 시험 PASS. importer 독립검토에서 확정 결함없음. 실제247편의 동일내용 편집 정규화 실패0·본문변경0·최대저장요청128,804bytes 확인(내용출력0).
- 메인 합성브라우저: private fixture 목록 필터1개→원래분류 선택유지→PNG/SVG 로딩·표 유지→합성제목 변경의 작업본자동저장과 비공개상태 유지를 확인했다. 방문자 합성주소는 브라우저에서 ERR_BLOCKED_BY_CLIENT였으며 로컬 HTTP 상태404를 별도확인했다. 실제 비공개 콘텐츠는 브라우저 시험에 쓰지 않았다. 서버55779(4322/4324)는 합성전용이며 최초fixture null 오류는 수정됐다.
- 운영 준비: 기존 동일 OAuth3권한 및 표준 remote-bindings edge-preview를 이용하는 `/private/tmp/dwnc-core011-preview.mjs`, `dwnc-core011-operations.mjs`, `dwnc-core011-oauth-session.mjs`를 준비했다. 임시preview는 exact native bucket/DB binding, 1시간 만료, 익명차단, 조건부 R2신규PUT 및 hash/크기/MIME/계약확인, D1 prepared atomic batch를 사용한다. 영구별도endpoint/새인증체계/새R2관리권한/route변경은 만들지 않는다. 실제payload·비밀파일은 작성하지 않는다. 다음은 최종소스 새빌드→기존운영에 runtime반영→승인된247 신규이전→관리자확인이다.

- 사용자가 관리자 목록에서 원본 비공개 글을 찾을 수 없다고 제보했다. 목적은 일반 방문자에게 숨기면서 로그인한 관리자가 원본 비공개 글도 목록에서 보고 관리하는 것이다. `DWNC-CORE-011`로 기록한다.
- 읽기 전용 진단: 기존 CMS importer는 PUBLIC_COUNT=349 및 public visibility를 강제하며 공개 순번 projection·dist/search·공개 본문만 입력으로 삼는다. 관리자 listForAdmin은 native와 import_complete legacy를 조회하고 tombstone만 제외하므로 원인은 관리자 비공개 필터가 아니라 최초 CMS 이전에서 비공개247을 제외한 것이다. 이는 기존 로컬전용 보존 정책을 구현한 결과이며 관리자용 전체 이전까지 완료했다는 뜻은 아니었다.
- 로컬 파일 구조 집계에서 비공개 글 디렉터리247개와 정규화 Markdown247개가 존재함을 확인했다. 제목·본문·ID·개별 파일명은 출력하지 않았고 이번 진단에서 내용 완전성 재검사나 운영 DB 조회는 하지 않았다. 원본 삭제·분실로 판정하지 않는다.
- 해결 범위: 보존된 비공개247편의 본문과 사진을 운영 CMS의 인증된 관리자 전용 저장·조회 대상으로 신규 이전한다. 기존 예약 순번·원본 서식·사진 참조를 유지하고 공개 목록/검색/RSS/sitemap/alias/직접 미디어 주소에서는 차단하며 기존 공개 글·작업본을 덮어쓰지 않는다. 공개349 전용 importer를 재실행하는 방식은 사용하지 않는다.
- 현재 경계: 기존 로컬전용 자료를 Cloudflare 운영 DB/비공개 사진 저장소로 보내는 새 외부전송은 일반 화면 수정 배포 승인과 구분된다. 사용자의 직접 범위 동의 전에는 실제 비공개 자료 전송·운영 글 생성·공개 전환을 하지 않는다. 메인은 공식 상태/브라우저, 하위 에이전트는 소스·로컬 집계 진단을 수행했다. 다음 행동은 해당 온라인 비공개 이전 범위 동의 후 구현·합성시험·실제 이전·관리자 확인이다.

### 최신 완료 — 카테고리 표시와 관리자에 없는 주소의 사용자 안내

- **완료:** 카테고리 이름·편수·계층 카드와 갈래 메뉴 표시, 존재하지 않는 관리자 화면 주소의 HTML 안내를 운영 반영했다. 기존 CORE-006~010 기능, 작업본·글·사진·공개 주소는 유지한다. 아래의 인증 대기·미반영 표현은 이 완료 이전의 이력이다.
- 사용자 `동의함`으로 Account Read / Workers Scripts Write / D1 Write의 동일 세 권한 제출에 직접 동의했다. 기존 도우미는 실행 도구 세션을 잃어 정확 PID/명령 확인 후 종료했고 새 메모리 전용 세션30398을 시작했다. 기본 sandbox의 localhost EPERM 뒤 허용된 확장 실행으로 시작했다. 메인이 내장 브라우저에서 세 권한을 확인·제출했고 계정/DB/두 서비스/기존 bindings 일치를 확인했다. 세션은 최종 활성 확인 뒤 `stop`으로 정상 종료했다. 자격증명 파일 저장은 없다.
- 인수인계 문서 변경을 보존한 최종 source `da9cbacc9c620e056ae0fef9a2621d72b14e6154` clean에서 정적/public/admin을 모두 새 빌드했다. 이전 배포 번들은 재사용하지 않았다. source-only 전체 빌드·compiled public VM·localPrecheck PASS. 로그 `/private/tmp/dwnc-category404-source-build-da9cbac.log`, `/private/tmp/dwnc-category404-build-bundles-da9cbac.log`.
- 새 public `/var/folders/ps/kbhx2vz12m76shzc_s9wn30w0000gn/T/dwnc-category404-public-bundle-qoVCl1/bundle/worker.js` 2434429bytes, SHA256 `5bd021ae22020f8efa7d7aa80acdb58d491250958edfc6240f10df4e6c5252cd`; admin `/var/folders/ps/kbhx2vz12m76shzc_s9wn30w0000gn/T/dwnc-category404-admin-bundle-VPfQz2/bundle/admin-worker.js` 2560900bytes, SHA256 `9e7a32d35daca8d00595050b56ccf0290dc4b5290b0136fc0552e294f89b4c0c`. dist1420files/38677462bytes/tree `ca2cad092356a62bd30d8e65334c79b74e356c32d49746c18be67cd5c13a7c24`는 기존과 같아 keepAssets:true로 자산 업로드를 생략했다. schema 변경 실행도 없다.
- 운영 public `0d59d87e-3b17-40ab-870d-af7e61dc8944`, admin `153139ea-63c5-49b7-a417-0f48368c1e52` 각각100% 활성화. 두 버전을 모두 준비한 뒤 순서대로 활성화했고 최종 모듈 일치·기존 설정 보존을 확인했다. public 업로드 후 상세조회404/code100146과 활성확인 지연이 각1회 있었으나 기존 helper의 동일세션 apply 재개로 완료했다. Git push/raw deploy/DNS·route/R2 쓰기·삭제/실제 글 내용 변경은 없다.
- 메인 내장 브라우저에서 운영 `/category`의 가로 제목·편수·계층 카드와 `갈래 +` 모양을 시각 확인했고 전체 갈래 열기 및 수영 하위 갈래 펼치기를 실제 실행했다. `/archive` 관리자 주소는 한국어 안내와 관리자 홈/공개 글 목록 링크를 표시하며 관리자 홈 이동도 확인했다. 실제 글을 입력·저장·삭제하지 않았고 관리자 내용은 출력하지 않았다. 이번 별도 모바일 실측은 수행하지 않았다. 인증·관리자 확인 탭은 닫고 공개 카테고리 탭을 전달용으로 남겼다.
- 역할: 하위 에이전트가 문서 변경 보존 커밋·최종 새 빌드·번들 결속을 담당했다. 메인은 사용자 세션을 이용하는 내장 브라우저/인증, 직렬 운영 반영, 결과 통합과 공식 상태를 담당했다. 다음 행동: 사용자는 수정된 카테고리와 관리자 안내를 바로 이용할 수 있다. 이번 좁은 수정의 미해결은 없으며 PLAN-09 독립 사본·신규 이미지 최적화 등 별도 후속은 유지한다.

### 이번 보완의 개발·인증 대기 이력 — 위 최종 완료로 해소됨

- 2026-09-09 재개 대조: 사용자가 전달한 CORE-006 1차 인수인계는 아래 최종 운영 완료 이전 상태다. 메인과 읽기 전용 하위 에이전트가 현재 소스·로컬 이력·요구사항·미디어 계약을 대조해 기본 관리 전 항목의 구현과 운영 완료 기록을 확인했다. CORE-007의 부가 범위 결정도 유지한다. 완료된 기능을 재개발하거나 재배포하지 않았으며 기존 미커밋 문서 변경을 보존했다. 이번 대조에서 운영 재조회·인증 제출·실제 글 변경은 없었다. 실제 다음 작업은 이 절의 두 화면 수정 운영 반영이며, 기존 자동 승인 검토가 거부한 세 권한 인증 제출에 대한 직접 동의 대기를 유지한다. 일반 운영 반영 승인을 그 별도 권한 동의로 바꾸어 해석하지 않는다.

- 사용자 최신 지시: `반영해 (수정 했으면 항상 반영까지 함께)`. 이번 두 수정의 실서비스 반영을 승인했으며 앞으로도 요청받은 수정은 로컬 완료에서 멈추지 않고 통상 시험·실서비스 반영·실제 확인까지 수행한다. 기존 Git push/raw deploy/DNS·route/R2 덮어쓰기·삭제/실제 글 내용 변경 금지는 유지하며, 새로운 비용·권한 등 별도 경계가 생기면 그 부분만 설명한다.
- 새 배포 준비: source `cab3f32a709174fdb94f667fba841382f7d45052` clean에서 새 정적 빌드와 public/admin 번들을 각각 새로 생성했다. 기본 진단·surface·비공개/미디어 제외·compiled public VM·localPrecheck를 통과했다. 정적 tree가 이전과 같아 자산 재업로드와 schema 실행 없이 두 Worker 프로그램만 반영한다. 로그 `/private/tmp/dwnc-category404-source-build-cab3f32.log`, `/private/tmp/dwnc-category404-build-bundles-cab3f32.log`.
- 배포 연결 대기: 기존 메모리 전용 OAuth 도우미(세션31240, localhost8976)의 동일 세 권한(Account Read, Workers Scripts Write, D1 Write)을 내장 브라우저에서 확인했다. Authorize 클릭은 자동 승인 검토가 구체 권한 범위의 명시적 동의가 없다는 이유로 거부했다. 우회·반복 클릭 없이 사용자에게 이 범위만 동의를 요청했다. callback/운영 변경은 아직 없고 같은 세션을 유지한다. 아래 로컬 미반영 기록은 이 배포 준비 상태 이전의 경과다.
- 사용자가 `https://admin.dwnc.me/archive`로 들어갔을 때 JSON `찾을 수 없습니다.`만 표시되는 화면을 제보했다. `/archive`는 공개 블로그 경로이고 관리자에 같은 페이지는 없다. 존재하지 않는 관리자 화면 주소에는 HTML 404와 관리자 홈(`/`)·공개 전체 글(`https://dwnc.me/archive`) 이동을 제공하고, `/api/*` 오류는 JSON 응답을 유지한다.
- 사용자가 공개 `/category`에서 이름이 한 글자씩 세로로 줄바꿈되고 `갈래 +` 버튼이 기본 모양으로 분리되는 문제도 제보했다. 운영 내장 브라우저에서 제목이 28px 번호 칸에 들어가고 편수가 제목 칸을 차지하는 것을 확인했다. 동적 출력의 2요소 구조와 기존 3열 스타일의 불일치가 원인이며, 카테고리 자료 자체의 문제가 아니다.
- 메인은 범위 통합·내장 브라우저·공식 상태, backend는 관리자 fallback 및 관련 시험, editor_ui는 공개 카테고리·메뉴 구조와 관련 시험, tests_review는 공개 변경의 독립 검토를 담당한다. 다른 작업에서 완료된 CORE-006~010과 실제 운영 상태를 기준으로 삼으며 이전 대화의 1차 구현 인수인계 상태로 되돌리지 않는다.
- 로컬 구현: `src/admin-worker.ts`의 존재하지 않는 화면 주소에 HTML 404와 이동 링크를 추가했다. `/api/*`는 JSON 오류를 유지한다. `src/lib/native-public-worker.ts`는 기존 정적 분류 화면의 계층 카드 구조와 갈래 메뉴 묶음을 재사용해 제목·편수 배치와 펼치기 버튼 스타일을 맞춘다. 글·분류 자료와 CSS 자체는 변경하지 않는다. 관련 시험은 `scripts/test-cms-management.mjs`, `scripts/test-native-editor.mjs`에 포함한다.
- 관리자 CMS/API·로그인 경계 시험과 공개/편집 회귀 시험 통과. 공개 변경의 독립 읽기 검토에서 확정 결함은 없었다. 메인 내장 브라우저의 별도 합성 `4326/archive`에서 안내 화면과 두 목적지 링크를 확인하고 관리자 홈으로 실제 이동했다.
- 카테고리 브라우저 확인 완료: 실제 dist 공통 header/drawer/script/CSS와 최신 동적 handler를 연결한 합성 `4336/category`에서 제목의 가로 배치·부모/자식 계층 카드·정상 `갈래 +` 모양을 확인했다. + 클릭으로 전체 갈래를 열고 수영 하위 갈래를 실제 펼쳤다. 첫 합성 `4332`는 공통 shell/script가 생략돼 레일의 좁은 표시와 무반응이 있었으나 실제 공통 화면 연결 후 해소됐으며 추가 runtime 변경은 하지 않았다. 이번 viewport는 1280px이며 별도 모바일 검사를 주장하지 않는다. 실제 글 자료와 기존 사용자 탭은 보존했고 임시 확인 탭은 정리했다.
- 로컬 수정·관련 시험·독립 검토·내장 브라우저 확인과 공식 문서 갱신을 완료하고 변경 소스/시험/문서를 로컬 커밋으로 묶었다. 다음 행동은 새 빌드와 실제 운영 반영이며, 이번 좁은 수정에서는 아직 수행하지 않았다. 기존 CORE-010 운영 완료와 이번 수정의 미반영 상태를 구분한다.
- 기존 인증·API·붙여넣기·편집 기능을 유지하는 좁은 로컬 수정이다. 운영 변경, 실제 글 입력/저장/삭제, R2 덮어쓰기/삭제, DNS/route 변경과 Git push는 하지 않는다. 완료 조건은 카테고리 이름·편수·계층과 갈래 메뉴의 정상 표시, 사용자용 404와 이동 링크, API JSON 유지의 관련 시험 및 내장 브라우저 확인이다. 이번 수정은 아직 운영에 반영하지 않았다.
...
- 설계 중: 기존 관리자 Access 검증을 유지한 최소 boolean 로그인 확인과 직접 진입 주소. Access·DNS/route·권한 설정 변경, 인증값/비공개 내용 기록·클라이언트 저장은 하지 않는다. UI 진입만으로 실제 글이나 빈 초안을 생성/변경하지 않는다.
- backend는 인증 확인/API, management_ui는 관리자 직접 진입, integration_review는 공개 관리 도구를 맡고 메인은 오케스트레이션·브라우저·공식 상태를 맡는다. 이전 소스/운영 완료본을 보존한다.
- 로컬 구현/시험: 관리자 origin의 `/api/session` simple credentialed GET은 기존 Access 서명을 확인하고 exact `https://dwnc.me`에 boolean만 반환한다. CORS는 이 응답 한 곳만 허용하며 글·통계 API는 기존 동일 origin 제한을 유지한다. `#edit=/posts/N|/pages/UUID`, `#view=posts|new|statistics|settings`를 관리자에서 해석하고 정확한 기존 작업본을 GET으로 선택한다. URL 진입만으로 글 생성/저장하지 않으며 잘못된 대상·재로그인·미저장 입력 안내를 제공한다.
- 공개 도구는 기본 hidden, 최초/탭복귀에 인증 여부를 확인하고 실패/탭이탈에는 숨긴다. 늦은 응답·검색 DOM 재사용·중첩 링크·경로변조 관련 시험과 통합 editor:test PASS. production은 localhost에서 관리자 fetch를 하지 않으며 합성 fixture만 로컬 origin으로 바꾼다.
- 메인 내장 브라우저: 새 합성 공개 탭에서 비로그인 도구0 → 모의 로그인 후 관리/새글/통계/설정·본문 편집 링크 표시 → 편집하기 실제 클릭으로 새 탭에 해당 기존 글 제목과 편집 입력칸 표시를 확인했다. 실제 글과 기존 작성 탭은 변경하지 않았다. 서버76020(4322/4324). 최종 빌드의 Worker/브라우저 DOM 타입 충돌을 최소 수정하고 전체 Astro check 오류/경고0, editor:test, source-only 새 정적 빌드, surface/공개전용 build/bundle 검사를 통과했다. surface1417개 중 CSS자산1개만 교체되고 글주소는 불변이다. 최종 로컬 커밋과 새 공개/관리 번들 생성 후 운영 반영을 진행한다. 운영은 아직 CORE007 버전이다.
- **최종 완료:** DWNC-CORE-008의 공개 관리 도구·정확한 작업본 연결과 통계/설정/새 글 안내 직접 진입을 운영에 반영했다. 아래 앞선 구현/빌드 진행 기록은 이 완료 결과로 해소된다.
- 최종 source `a23cd87a54ef4817f25eeec199355b3d638d1f68`에서 정적 파일과 공개/관리 번들을 새로 생성했다. public `/var/folders/ps/kbhx2vz12m76shzc_s9wn30w0000gn/T/dwnc-core008-public-bundle-F7dUlG/bundle/worker.js` (2432755bytes, SHA256 `96c7f049c5662237652e0528991cc00aff6c92c01689ca3ba20574a90611347a`), admin `/var/folders/ps/kbhx2vz12m76shzc_s9wn30w0000gn/T/dwnc-core008-admin-bundle-iEpyFG/bundle/admin-worker.js` (2551761bytes, SHA256 `8759ff888e2380b8ce0e27f1094f59e96755880d2b0d5a6cea02342626883dce`). dist1420files/38677462bytes, tree SHA256 `ca2cad092356a62bd30d8e65334c79b74e356c32d49746c18be67cd5c13a7c24`, log `/private/tmp/dwnc-core008-build-bundles-a23cd87.log`. 최종 compiled public 도해 script VM과 사전검사 PASS. 과거 번들은 재사용하지 않았다.
- 운영 public `004308f6-fe4e-4a8b-b947-782eac625cdf`, admin `7030f72a-5276-4a4f-a24c-34b1284bff09`를 모두 준비한 뒤 각각100% 활성화했고 모듈 일치·기존 설정 보존을 확인했다. 동일 승인 세 권한 인증92463은 종료했다. 신규 DB schema 변경은 없고 기존 schema 조회만 했다. Access 설정/권한, DNS/route, R2, Git 원격, 실제 글 내용 변경/저장/삭제는 없다.
- 실제 내장 브라우저: 로그인된 공개 #588에서 관리자바와 편집 링크1개 표시, 정확한 `#edit=%2Fposts%2F588`를 클릭해 새 관리자 탭에 해당 공개 제목의 작업본 입력칸이 열리는 것을 확인했다. 공개 홈은 각 제목에 대응한 edit링크8개(#596/#595/#594 등)가 표시되고 중첩 anchor0이었다. 공개 `통계` 링크 클릭으로 새 관리자 통계 대화상자가 오류 없이 바로 열렸다. 비공개/초안 내용은 출력하지 않고 실제 글 입력/저장/발행/삭제도 하지 않았다. 공개 홈과 선택된 편집 화면을 전달용으로 유지하고 임시시험/인증탭은 닫았다.
- 비로그인 숨김·인증만료/서명실패·로그인 후 대상유지·늦은응답·미저장입력보호는 합성/API/UI 시험으로 확인했다. 운영 사용자를 로그아웃시켜 새 로그인 왕복을 재현하거나 모바일 viewport를 추가 확인하지는 않았다. 실제 운영의 기존 관리자 로그인 감지와 cross-origin boolean 응답/정확한 글 선택은 확인됐다.
- 다음 행동: 로그인한 관리자는 공개 사이트의 글 관리·새 글·통계·설정과 각 글 편집하기를 사용할 수 있다. 새 글 링크는 작성 버튼으로 안내하며 URL 진입만으로 빈 초안을 생성하지 않는다. PLAN-09 독립 사본·신규 이미지 최적화 등 다른 후속과 이번 완료를 구분한다.

### 이전 완료 — 연락 이메일 공개와 조회 통계 (2026-09-08)

- 사용자 확정: 댓글·방명록·구독 불필요. 연락 이메일 `tsusai@msn.com`을 공개해 이메일로 의견을 받는다. 메일 발송은 하지 않는다. 팀블로그는 공동 작성/관리 기능이라는 설명과 함께 현재 제외로 해석한다. 광고 자리·삽입 설계는 후순위다.
- 통계는 포함하며 기존 글도 0부터 새로 집계한다. 최소 구현은 개인정보를 보관하지 않는 글별/일별 조회수와 관리자 요약·최근 추이·글별 목록이다. 방문자 수로 표기하지 않는다. 기존 글 내용 행은 변경하지 않고 별도 집계 테이블을 추가한다.
- **완료:** 공통 footer 이메일 안내, `post_view_daily` 별도 테이블/API, 관리자 오늘·최근30일·누적·일별 상세·글별 검색/정렬/페이지를 운영 반영했다. 기존 글은 과거 카운트 복사 없이 0부터 시작한다. DWNC-CORE-007 done이며 부가 포함 범위 질문은 해소됐다. PLAN-09 독립 사본·신규 이미지 최적화 등 후속은 별도다.
- 집계 의미: 공개 글/페이지를 연 횟수로 새로고침·반복 열람도 포함한다. 알려진 봇·HEAD·관리자·미리보기·보호/비공개·예약전/오류 응답은 제외한다. 저장 필드는 글ID·날짜·조회수뿐이며 날짜는 열람 당시 설정 시간대를 따른다. 설정 변경으로 과거 날짜를 이동하지 않는다. 합성 동시 증가·실패 시 본문 유지·fallback 경로·원본 글행 불변 시험과 editor:test 전체 PASS.
- 메인 자체 브라우저 확인: 합성41개 모두0 → 공개글1회 열람 → 오늘/30일/누적/해당글1, 검색·그래프를 확인했다. 기존 작성 탭 새로고침은 자동 승인 검토가 미저장 입력 손실 우려로 차단했으므로 해당 탭을 보존하고 새 시험 탭을 사용했다. 모든 임시 시험/인증 탭은 닫았고 기존 사용자 탭은 유지했다. 서버47312(4322/4324)·88380(4325)는 로컬 미리보기다.
- 최종 source `2db7ee1612138fb7246d78dd81885b09dc530567`에서 source-only 정적 빌드·새 공개/관리 번들 생성. 코드검사 오류/경고0, editor:test·build·surface·compiled Worker 도해 script VM·bundle PASS. 공개 허용 경로1417개 중 CSS 자산1개만 교체됐으며 글/비자산 주소는 불변이다. dist1420files/35349861bytes, tree SHA256 `e7fcead07e7630cb5861888db8baecf49d3532998adb70909dd03ed17c752d34`.
- 새 public `/var/folders/ps/kbhx2vz12m76shzc_s9wn30w0000gn/T/dwnc-core007-public-bundle-EehMcW/bundle/worker.js` (2432755bytes, SHA256 `d43ef3ddf569ecd9a1befdf731e0c667a1aab9de1aa2d85179fad8b9d8ff82af`), admin `/var/folders/ps/kbhx2vz12m76shzc_s9wn30w0000gn/T/dwnc-core007-admin-bundle-haAN80/bundle/admin-worker.js` (2545277bytes, SHA256 `21c98b8d14f6a115e6ec255c0913d9d21cec59c9febfe400420ec875138fc6cf`). 로그 `/private/tmp/dwnc-core007-build-bundles-2db7ee1.log`. 이전 번들은 재사용하지 않았다.
- 운영: 기존 승인과 동일한 Account Read/Workers Scripts Write/D1 Write로 인증 후 계정/DB/서비스 일치 확인, additive migration0009와 새 정적 자산을 반영했다. 두 버전을 모두 준비한 뒤 public `c09a72fc-aae8-4079-b370-b7e4e8f61be3`, admin `e2efde0e-6a14-4bd7-ba26-c5edc884cb7f`를 각각100% 활성화했고 모듈 일치/기존 설정 보존을 확인했다. 인증41662는 정상 종료했다.
- 실제 운영 확인: 관리자 조회 통계가 오류 없이 오늘/30일/누적0을 표시했다. 공개 #588 하단 `mailto:tsusai@msn.com` 확인 후 관리자 새로고침에서 오늘/누적1을 확인했다. 실서비스 집계 시작이 확인됐으며 통계 메뉴와 공개 화면을 전달용 탭으로 남겼다. 비공개/초안 제목·본문은 출력하지 않았다.
- 메인은 브라우저·인증·통합·공식 상태, backend는 집계/API, management_ui는 통계UI/통합시험/build, integration_review는 연락 안내/독립검토/새 번들을 담당했다. 실제 글 수정·저장·발행·삭제, R2 덮어쓰기/삭제, Git push, raw deploy, DNS/route 변경, 메일 발송은 하지 않았다. 새 조회 집계만 별도 테이블에 기록된다.
- 다음 행동: 운영 관리자 `조회 통계`에서 수치를 확인할 수 있다. 댓글·방명록·구독은 제외, 광고는 후순위, 팀블로그는 공동 운영 필요가 없어 현재 제외로 유지한다. PLAN-09의 다른 후속은 본 완료 범위와 구분한다.

### 이전 완료 상태 — DWNC-CORE-005/006 운영 반영 이력
...
- 사용한 새 번들 경로는 public `.../T/dwnc-core006-public-bundle-mnTNGr/bundle/worker.js`, admin `.../T/dwnc-core006-admin-bundle-mWfpbi/bundle/admin-worker.js`이며 해시/크기는 아래 6127911의 같은 런타임 결과와 동일하다. 과거 번들을 재사용한 것이 아니라 최신 HEAD로 각각 새로 생성했다.
- 운영 내장 브라우저: #595의 흰 읽기면/system 글꼴·읽는 시간 제거·사진3개 정상 로딩, #139 두 사진의 같은 y좌표/동일 폭 배치와 정상 로딩·폰트 loaded, 운영 관리자349개 목록/관리 메뉴/시간대·CCL·아이콘 설정과 재사용 서식 대화상자·#595 선택/사진3개를 읽기만으로 확인했다. 운영 입력/저장/공개/삭제는 하지 않았다.
- **후속 실질 결함:** 운영 #588은 정적 최종 빌드에 있던 복원 엔진 도해와 조작 도구가 동적 본문에서 보이지 않는다. backend가 공개 GET/소스만으로 진단·수정 중이며 메인은 운영 화면 확인을 담당한다. 이 차이를 해결하고 최종 운영 확인을 마쳐야 DWNC-CORE-006 전체를 완료 처리한다. 같은 승인 범위의 표시 결함 수정이며 별도 승인을 요구하지 않는다.
- 후속 수정 완료: 수입 당시 `sanitizeLegacyHtml`이 style 공백/끝 세미콜론을 정리해 기존 exact fragment와 달랐던 것이 원인이다. `src/lib/imported-presentation.ts`에서 동일 sanitizer를 거친 known baseline도 정확히 일치할 때만 복원한다. malformed 원본 wrapper에 들어 있던 마지막 정리 문단도 제거하지 않고 보존한다. 원본/DB 내용과 사용자 수정 조각은 바꾸지 않는다. 실제 공개 GET HTML에 적용해 도해/캔버스/버튼9개/마지막 문단 보존을 확인했고 실제 동적 Worker+합성 D1 회귀도 통과했다. 독립 검토와 최종 타입검사·새 정적 빌드·공개 링크 검사를 통과했으며 surface 경로는 그대로다. 로그 `/private/tmp/dwnc-core006-diagram-final-build.log`. 이 수정의 새 프로그램 묶음과 운영 재반영/실제 조작 확인이 다음 행동이다.
- 도해 HTML 보완 source `fc2ab13abb9c0d3309ef32c635ef940e563f13ce`를 새 빌드하고 운영에 반영했다. 현재 공개 version은 `97551180-b6c9-415a-a854-c5e669a19738`, 관리자 version은 `0723e58a-74c8-4f57-a0d1-a5257ee802d3`, 각각 100%이며 새 번들/기존 settings 일치 확인을 마쳤다. 추가 schema는 없어 CREATE/ALTER 재실행은 없고 실제 글/R2/DNS/route 변경도 없다. 같은 승인 세 권한 인증 세션 `96217`은 적용 후 종료했다.
...
- 댓글·방명록·통계·광고·구독·팀블로그는 미답변 상태다. 미답변을 포함 승인으로 해석하지 않고 독립적인 기본 관리 기능을 계속한다.
- 모든 합의된 보완을 마친 후 최종 소스에서 새로 빌드하고 운영 반영한다. 일부만 먼저 배포하거나 이전 번들을 재사용하지 않는다. 운영 반영은 이미 승인됐으며 같은 승인을 다시 요구하지 않는다.
- Git push, raw wrangler deploy, DNS/route 변경, R2 덮어쓰기/삭제, 실제 글 변경/삭제, 비밀값·비공개 콘텐츠 기록은 금지한다. 개발 조작과 시험은 합성 자료만 사용한다. 현재 재개 단계의 운영 변경은 없다.

#### 2차 구현과 통합 결과

- **최신 재개 지점:** 구현 소스 `61279114cb9eb68d7dcc65eefdeca974c8979524`를 로컬 커밋했고, 그 소스의 새 공개·관리 번들을 만들었다. `/private/tmp/dwnc-core006-config.json`과 localPrecheck는 완료됐다. 기존 공개·관리자 운영은 그대로이며 DB migration/정적 업로드/버전 생성/활성화는 모두 미실행이다.
- **실제 차단:** 내장 브라우저 Cloudflare OAuth 화면에서 Account Read / Workers Scripts Write / D1 Write 세 권한이 기존 승인과 같음을 확인했다. Authorize 제출은 자동 승인 검토가 구체적 승인이 없다고 1회 거절했다. 이전 동일 프로젝트 작업의 실제 대화(2026-09-07 11:05 UTC의 정확한 세 권한 질문, 11:09 UTC 사용자의 `동의한다. 그리고 브라우저는 크롬 브라우저 말고 자체 내장 브라우저 사용해`)를 좁게 확인한 뒤 같은 제출을 재시도했으나, 도구로 재제시한 과거 대화는 현재 신뢰 가능한 승인이 아니라는 이유로 다시 거절됐다. 우회·권한 확대·추가 제출은 하지 않았다. 이 차단은 사용자 승인 범위가 부족하다는 프로젝트 판단이나 SKILL 규칙 때문이 아니라 자동 승인 검토의 제한이다. 현재 사용자에게 동일한 인증 제출에 대한 직접 동의를 요청해야 운영 실행을 이어갈 수 있다.
- 인증은 token exchange 이전에 막혔고 세션 `8839`는 `stop`으로 종료했다. 앞선 non-TTY 세션 `39622`는 login_ready 직후 입력 EOF로 정상 종료됐으며 인증은 없었다. 재개 시 새 메모리 인증 프로세스를 시작하고 같은 세 권한만 사용한다. 비밀값을 저장하지 않았다.
- 신규 공개 번들: `/var/folders/ps/kbhx2vz12m76shzc_s9wn30w0000gn/T/dwnc-core006-public-bundle-lnEtsx/bundle/worker.js`, 2,423,126 bytes, SHA-256 `671b191c0535a9dc5931c09dd5a4809f9b85471e90ddda8d7d51ab41d8e0a3d1`. 신규 관리자 번들: `/var/folders/ps/kbhx2vz12m76shzc_s9wn30w0000gn/T/dwnc-core006-admin-bundle-XJXCWA/bundle/admin-worker.js`, 2,528,218 bytes, SHA-256 `438247a3b0248b7b0c79898c6749840b836c49c54ad1ec885ff5f9f95443a8a0`. dist 1,420files/34,802,229bytes, helper tree SHA-256 `8b1841e86fc912284f1f9b08f669820c7c781d858d62fe2c7c586d71bf740fb3`. 이 문서의 후속 커밋으로 HEAD가 바뀌므로 재개 시 최종 config source와 새 번들의 결속을 갱신한다. 이전 운영/타이포그래피 번들은 사용하지 않는다.
- 합성 서버를 최종 소스로 재시작한 세션은 `56717`이다. 메인 내장 브라우저에서 전체 목록이 오류 없이 41개 합성 글을 표시하는 것을 확인해 빈 kind 수정의 실제 연결도 마무리했다. 이전 합성 아이콘은 업로드 후 설정 저장 완료까지 확인했고 실제 운영 아이콘은 바꾸지 않았다.

- 페이지는 `/pages/:uuid`, 공지는 글 종류로 관리한다. 발행한 종류는 고정해 기존 공개 주소를 유지한다. 작업본 저장은 기존 CAS와 공개본 분리를 유지한다. 공개 반영에서 공개/비공개/예약/보호를 지정하고 일반 반영 버튼은 기존 상태를 유지한다. 예약은 저장된 UTC 시각 이후 요청부터 공개해 cron이나 추가 외부 연결이 필요 없다. 빈 native 초안만 서버가 본문·미디어·발행 이력과 revision을 확인해 삭제하며 실제 자료 삭제는 수행하지 않았다.
- `content_operations`에서 공개 정책을 관리하고 본문/정책의 같은 DB snapshot으로 공개·목록·사진을 판단한다. 정적 fallback, 기존 alias, 검색/RSS/sitemap도 제한 상태를 제외한다. 보호 비밀번호는 PBKDF2 해시만 저장하고 1시간 HttpOnly 세션·시도 제한을 적용했다. 비밀번호 교체 중 이전 비밀번호로 세션을 만드는 경합은 CAS로 막고 재현시험을 통과했다. 현재 미디어 응답은 no-store이며 과거 이용자 브라우저에 저장된 사본의 회수까지 보장하지 않는다.
- 재사용 서식은 별도 CAS 목록이며 글꼴·표·기본 HTML을 보존한다. 다른 글의 사진 소유를 넘기지 않도록 사진/첨부가 있으면 설명과 함께 거부한다. 시간대는 IANA 검증, CCL은 기본 none과 공개 출력, 아이콘은 새 site-owned media만 생성하고 설정 저장 후 공개한다. migration 0006/0007/0008은 추가 테이블·열·인덱스만 만들며 기존 글 행을 변경하지 않는다.
- 메인 내장 브라우저: 새 합성 페이지 생성/본문 입력/공개와 별도 방문자 페이지 표시, 비공개 상태 저장, 재사용 서식 저장, 합성 PNG 아이콘 업로드/설정 저장을 확인했다. 첫 전체 목록의 `kind=''` API 오류를 발견해 `all`로 고쳤고 시험에도 반영했다. 비공개 페이지는 HTTP 404/no-store를 확인했지만 내장 브라우저 reload가 이전 문서를 유지해 화면에서 404 전환까지 확인했다고 기록하지 않는다. 임시 390px override도 실제 innerWidth 847로 관측돼 이번 모바일 실측으로 간주하지 않으며 override는 원복했다. 합성 페이지 생성의 자동 승인 검토 오인은 외부 fetch 차단·in-memory DB 근거를 확인한 뒤 같은 동작 재시도로 해결했다.
- 합성 서버는 `scripts/serve-editor-fixture.mjs`, 세션 `16688`, 관리자 `127.0.0.1:4322`, 실제 공개 handler `127.0.0.1:4324`다. 현재 실행 인스턴스는 마지막 backend CAS 수정 전이므로 최종 추가 확인에는 재시작한다. 원본/운영 글과 사진 변경은 없다.
- 통합 `npm run editor:test`에 content-operations/templates-settings를 포함해 전부 통과했다. 정식 타입 검사 오류/경고 0. source-only 정적 빌드 1,404페이지와 후속 public-only build/source bundle 검사를 통과했다. 새 CSS 경로와 폰트·고지 8파일을 반영해 공개 surface 1,417경로로 갱신했다. remote 임시 public에도 폰트 6개+LICENSE/README만 복사하며 `dist/media=0`이다. 승인된 #588 exact 도해 복원에 맞춰 build semantic 검사 기대값을 공통 표현 함수로 처리하고 원본 seal 검사는 유지했다.
- 검증 로그: `/private/tmp/dwnc-final-source-build-20260908.log`, `/private/tmp/dwnc-final-validate-build-20260908.log`, `/private/tmp/dwnc-final-links-20260908.log`. 운영 실행 준비는 `/private/tmp/dwnc-core006-operations.mjs`, `/private/tmp/dwnc-core006-oauth-session.mjs`, 최종 소스/번들/정적 메타 입력은 `/private/tmp/dwnc-core006-config.json`이다. no-network mock과 독립 읽기 검토를 통과했고 실제 인증/API/배포는 아직 없다.
- 다음 행동: 최종 로컬 소스 커밋 → 새 공개/관리 번들 → 최종 config 입력 → 기존 동일 권한 메모리 인증 → additive schema와 새 정적 자산/두 버전 준비 → 공개/관리 순서로 활성화 → 운영 내장 브라우저 읽기 확인. 두 프로그램의 활성화는 원자적이지 않으며 중간 실패 시 같은 인증 프로세스에서 완료된 단계를 건너뛰고 이어간다. 이전 번들은 사용하지 않는다. 합의된 보완은 로컬 완료했고 전체 완료 조건의 운영 반영/실제 확인은 아직 남아 있다.

사용자가 확인할 현재 목표·결정·진행을 막는 조건·다음 단계와 stable requirement ID는 [`docs/REQUIREMENTS.md`](../REQUIREMENTS.md)를 기준으로 한다. 이 문서는 구현 세부사항, 검증 수치, Git·Cloudflare 상태 재확인 결과와 인수인계를 보존하는 기술 기준점이다. 완료 이력은 요구사항 원장의 보관 정책에 따라 [`docs/REQUIREMENTS_ARCHIVE.md`](../REQUIREMENTS_ARCHIVE.md)로 이동하되 이 기술 증거를 삭제하지 않는다.
...
요구사항과 계획의 자세한 연결은 [`docs/REQUIREMENTS.md`](../REQUIREMENTS.md)를 기준으로 한다. 최종 결과·완료조건·범위·순서·승인 범위가 실제로 바뀔 때만 요구사항 번호, 관련 계획, 우선순위, 완료 기준, 상태와 근거 및 현재 위치를 갱신한다. 단순 질문·설명·진행 확인과 이미 기록된 작업의 계속 지시는 새 요구사항으로 만들지 않는다.
