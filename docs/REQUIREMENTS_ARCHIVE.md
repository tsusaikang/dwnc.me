...
### `DWNC-CORE-024` — 본문 서식 포함·제외 붙여넣기 구분
- **Status:** `done`
- **Updated-at:** `2026-09-17`
- **Plans:** `PLAN-09`
- **Priority:** `P1`
- **Acceptance:**
  - 사용자가 확정한 일반 관례대로 Cmd+V는 서식 포함, Shift+Cmd+V는 무서식이다. Ctrl+Shift+V와 Mac Option+Shift+Cmd+V도 같은 무서식 의도로 다룬다. 브라우저 기본 붙여넣기를 가능한 유지하며 다음 붙여넣기에 단축키 의도가 남지 않게 한다.
  - 무서식 의도에서 HTML 원문 자동변환 등 특수 처리가 서식을 다시 만들지 않도록 한다. 서식 포함은 기존 지원 서식을 유지한다. 사진·자동링크·커서·실행 취소·자동저장 영향은 선택 후 확인한다.
  - 실제 콘텐츠·미공개 작업본을 시험 목적으로 수정하지 않고 합성 시험 후 기존 Git 배포와 운영 확인을 한다.
- **Evidence:**
  - 사용자가 추천을 확인한 뒤 “그냥 관례대로 할게”라고 선택했다. 공식 Google Docs 안내는 Cmd+Shift+V 무서식, Apple 안내는 Option+Shift+Cmd+V 스타일 맞추기다.
  - 착수 전에는 일반 문장의 무서식 삽입은 동작했으나 HTML 형태의 text/plain 자동변환과 이미지 처리가 단축키 의도를 구분하지 않았다.
  - [완료 결과와 검증 한계](history/본문_붙여넣기_단축키_20260917_013542.md): 0e3d5e2의 관련 회귀6개·최종 빌드·합성 브라우저·양쪽 운영 배포·운영 목록과 코드 반영 확인 완료. 실제 물리 키보드 조합은 자동화 도구 제약으로 확인하지 못했으며 합성 두 경로·실행 취소·저장 검증과 구분한다.


### `DWNC-CORE-025` — 관리자 목록에서 제목과 공개 상태 구분
- **Status:** `done`
- **Updated-at:** `2026-09-22`
- **Plans:** `PLAN-09`
- **Priority:** `P1`
- **Acceptance:**
  - 관리자 글 목록에서 공개·비공개 등 상태 문구가 제목의 일부처럼 붙어 보이지 않도록 제목과 상태를 시각적으로 구분한다.
  - 실제 저장된 제목을 수정하지 않고 글 순번·유형·공개 상태·미공개 변경 안내·날짜·정렬·검색·편집 진입을 유지한다. 긴 제목과 작은 화면에서도 구분할 수 있어야 한다.
  - 관련 회귀와 로컬 내장 브라우저 확인을 마치고 최종 소스를 기존 Git 자동 배포로 반영한 뒤 운영 목록을 읽기 확인한다. 작업본·반영본·원본·사진·공개 주소를 변경하지 않는다.
- **Evidence:**
  - 2026-09-22 사용자가 관리자 목록의 제목에 `공개`가 붙어 보이는 문제를 지적하고 보완을 요청했다. 첨부 화면의 600편 표시는 사용자 제공 자료이며 이번 운영 조회 완료의 근거로 쓰지 않는다.
  - [완료 결과와 확인 한계](history/관리자목록과_사진사이입력_20260922_114542.md): 기능 소스 `c062486`과 문서 후속 `c630931`의 공개·관리자 Git 자동 빌드·배포 성공을 확인했다. 운영 관리자 목록에서 제목 단독 행과 순번·유형·공개 상태 별도 행을 실제 확인했다. 기존 전체 편집기 시험의 홈 카드 검사 실패는 그대로 기록하며 관련 회귀·최종 빌드 통과와 구분한다.


### `DWNC-CORE-026` — 연속 사진 사이 빈 공간 클릭으로 문단 입력
- **Status:** `done`
- **Updated-at:** `2026-09-22`
- **Plans:** `PLAN-09`
- **Priority:** `P1`
- **Acceptance:**
  - 후속 정정: 사진 사이뿐 아니라 맨 처음 사진 앞·맨 마지막 사진 뒤에서도 같은 클릭 방식으로 텍스트 문단을 입력할 수 있다. 사진이 한 장뿐인 경우와 사진 그룹의 앞뒤에도 일관되게 동작하며 그룹 내부 구조를 임의로 나누지 않는다.
  - 편집기에서 연속한 사진 블록 사이 빈 공간을 클릭하면 해당 위치의 텍스트 문단에 커서를 놓아 바로 입력할 수 있다. 기존 빈 문단이 있으면 재사용하고 같은 곳을 반복 클릭해 불필요한 문단을 계속 늘리지 않는다.
  - 사진 선택·이동·크기/정렬 도구 조작과 문단 삽입을 구분한다. 원본 사진·순서·그룹·캡션·기존 문단 서식을 보존하고 사진 그룹 내부를 임의로 분리하지 않는다.
  - 삽입 문단과 입력이 기존 실행 취소/다시 실행, 작업본 자동저장·revision 충돌 처리·명시적 공개 반영 분리와 함께 동작하며 저장 후 재열어도 유지된다.
  - 관련 회귀와 로컬 내장 브라우저에서 PC·작은 화면의 클릭·입력·실행 취소·저장/재열기를 확인한다. 최종 소스를 기존 Git 자동 배포로 반영하고 운영 편집기 코드와 화면을 읽기 확인한다. 실제 글을 시험 수정하거나 새 공개 글을 만들지 않는다.
- **Evidence:**
  - 2026-09-22 사용자가 연속 사진 사이 빈 공간을 클릭해 텍스트 문단을 입력하는 편집 동작 보완을 요청했다. 이 요청은 기존 `CORE-012`의 결함 6건 착수나 실제 콘텐츠 수정으로 확대하지 않는다.
  - [최초 반영 결과와 당시 확인 한계](history/관리자목록과_사진사이입력_20260922_114542.md): 기능 소스 `c062486`과 문서 후속 `c630931`의 공개·관리자 Git 자동 빌드·배포 성공을 확인했다. 로컬 1280px/390px 사진 사이 클릭·입력·실행 취소·자동저장/재열기를 확인했고 운영 편집기에 새 안내와 처리 코드가 반영됐다. 운영 실제 글에는 입력·저장·공개하지 않았다. 기존 전체 편집기 시험의 홈 카드 검사 실패는 그대로 기록하며 관련 회귀·최종 빌드 통과와 구분한다.
  - 2026-09-22 사용자 후속 지적으로 첫 사진 앞·마지막 사진 뒤에서 같은 방식의 텍스트 입력이 되지 않는다는 실사용 문제가 확인됐다. 최초 검증은 사진 사이에 한정됐고 양끝 실사용은 확인하지 않았다. `CORE-026`을 재개하며 새 ID·재승인 없이 기존 보완 범위에서 수정·관련 회귀·로컬 브라우저·최종 Git 자동 배포·운영 읽기 확인까지 마친다.
  - **양끝 후속 정정 완료:** `19a3bd7`의 관리자·공개 Git 자동 배포 성공과 운영 #600의 앞뒤 36px 본문 클릭 영역을 확인했다. 사진 9개·문단 0개·마지막 저장 11:35:13 KST는 유지했고 실제 글에 입력·저장·공개하지 않았다. PC/390px 실제 입력·실행 취소·저장/재열기와 사진 한 장·그룹의 합성 회귀는 [같은 완료 기록](history/관리자목록과_사진사이입력_20260922_114542.md)에 구분했다. 최초 검증이 사진 사이에 한정돼 양끝 문제를 놓친 사실도 보존한다.

### `DWNC-CORE-027` — 공개 사이트의 상시 관리자 로그인 진입 링크
- **Status:** `done`
- **Updated-at:** `2026-09-22`
- **Plans:** `PLAN-09`
- **Priority:** `P1`
- **Acceptance:**
  - 공개 사이트의 공통 헤더에 `관리자 로그인` 링크를 항상 제공해 로그인 상태가 없거나 만료돼도 관리자 주소를 직접 입력하지 않고 기존 관리자 로그인 화면으로 들어갈 수 있다.
  - 인증된 관리자임이 확인되면 같은 진입 링크를 `관리자 화면`으로 표시한다. 확인 실패·세션 만료·네트워크 오류·JavaScript 미실행 상태에서도 기본 로그인 링크가 사라지지 않고 일반 링크로 동작한다.
  - 기존 관리자 도구막대·새 글·통계·편집 링크는 인증 확인을 통과한 경우에만 표시한다. 공개 진입 링크 추가를 인증 우회나 일반 방문자에 대한 편집 기능 노출로 확대하지 않는다.
  - 기존 관리자 편집기의 세션 만료 시 새 탭 로그인과 작성 화면 복귀·재시도 경로를 읽기와 관련 시험으로 확인하고 현재 입력 보존·revision 충돌 방지·작업본/공개 분리를 유지한다. 새 자동 복구나 브라우저 로컬 보관 기능은 추가하지 않는다.
  - PC와 390px 화면에서 링크를 찾고 사용할 수 있어야 하며 기존 공개 헤더·탐색·관리 도구 표시를 유지한다. 관련 시험·내장 브라우저 확인·최종 소스 빌드·기존 Git 자동 배포·운영 읽기 확인을 마친다.
  - 기존 24시간 세션·허용 계정·권한·Cloudflare Access/SSO 정책을 변경하지 않는다. 운영 글·작업본·사진·공개 상태도 변경하지 않는다.
- **Evidence:**
  - 2026-09-22 사용자가 오래 열린 관리자 세션이나 노트북을 들고 장소를 옮겨 사용하는 중 로그인이 풀리면 공개 화면에 로그인 버튼이 없어 관리자 주소를 직접 입력해야 하는 불편을 지적했다. 개별 로그인 해제 원인을 단정하거나 세션 연장을 전제하지 않고 진입 경로를 보완한다.
  - [완료 기록](history/공개사이트_관리자로그인_진입_20260922_222656.md)에 구현·관련 세션 복귀 확인·브라우저·배포 결과를 구분해 기록했다.
  - **완료 근거:** 최종 소스 `44f6e14`의 공개·관리자 CI Success와 운영 공개 헤더의 로그인 진입·인증 후 관리자 화면 문구·기존 관리 도구 표시를 확인했다. 링크 주소의 별도 새 탭에서 현재 601개 관리자 목록이 열렸고 운영 390px DOM 경계상 가로 넘침이 없었다. 최초 공개 CI 실패와 시간 제한 경합 수정, 클릭 자동 새 탭 및 확대된 스크린샷 도구의 확인 한계는 같은 기록에 보존했다. 기존 사용자 탭·운영 콘텐츠·세션 기간·권한 정책은 변경하지 않았다.

### `DWNC-CORE-028` — 본문 기준 표 자체의 왼쪽·가운데·오른쪽 배치
- **Status:** `done`
- **Updated-at:** `2026-09-22`
- **Plans:** `PLAN-09`
- **Priority:** `P1`
- **Acceptance:**
  - 표 안에 커서를 둔 뒤 `더보기 → 표 편집`에서 `표 위치`의 왼쪽·가운데·오른쪽을 선택할 수 있게 한다. 셀 안 텍스트 정렬과 표 전체의 본문 기준 위치를 명확히 구분한다.
  - 표 위치 변경은 대상 표에만 적용하고 셀 정렬·텍스트·표 행/열·병합·너비·기존 서식과 주변 본문을 보존한다. 기존 표 생성·행/열 조작·셀 서식 등 지원하던 표 관리 동작을 유지한다.
  - 실행 취소/다시 실행으로 위치 변경을 복원할 수 있고 작업본 자동저장·저장/재열기·미리보기·공개 표시에서도 선택한 표 위치가 유지된다. 기존 revision 충돌 방지와 작업본/공개 반영 분리를 유지한다.
  - 관련 회귀와 로컬 브라우저에서 PC·작은 화면의 표 위치·셀 정렬 구분·보존·저장/재열기를 확인한다. 최종 소스를 기존 Git 자동 배포로 반영하고 운영 편집기·공개 표현을 읽기 확인한다.
  - 실제 운영 글·작업본·사진·공개 상태는 시험 수정하지 않는다. 표 전체 선택·삭제 전용 UI 등 별도 기능으로 범위를 확대하지 않고 기존 별도 결함 6건도 착수하지 않는다.
- **Evidence:**
  - 2026-09-22 사용자가 셀 안 글자 정렬은 되지만 표 자체를 본문 기준으로 좌·중·우 배치할 수 없다고 지적했다. 직전 표 기능 질의는 설명·기존 기능 확인이었으며 표 코드 업데이트는 없었다.
  - [완료 기록](history/표자체_위치정렬_20260922_230711.md)에 원인·구현·관련 시험·로컬 및 운영 확인을 구분해 기록했다.
  - **완료 근거:** `00dfa76`의 공개·관리자 CI Success와 운영 `더보기 → 표 편집`의 표 위치 좌·중·우 선택 및 안내 표시를 확인했다. 실제 배치 조작·실행 취소·저장/재열기·미리보기·공개 Worker 표시는 로컬 합성 글에서 확인했으며 운영에서는 위치 버튼을 누르거나 본문 입력·저장·공개하지 않았다. 기존 표 class 보존을 위한 독립 검토·회귀와 마지막 저장시각 불변을 같은 완료 기록에 남겼다.

### `DWNC-CORE-029` — 사진 편집 반복 적용의 틀 중첩 방지와 한정 작업본 정리
- **Status:** `done`
- **Updated-at:** `2026-09-23`
- **Plans:** `PLAN-09`
- **Priority:** `P1`
- **Acceptance:**
  - 사진 편집 적용 시 선택한 정확한 DOM 요소를 교체하고 `execCommand('insertHTML')`의 선택 범위 삽입에 의존해 기존 사진 틀 안에 새 틀을 중첩하지 않는다. 반복 적용 후에도 사진 틀이 늘어나거나 뒤 텍스트가 사진 안으로 들어가지 않는다.
  - 원본 사진·기존 캡션·본문·서식을 보존한다. 캡션을 변경하지 않은 적용에서는 기존 캡션 내부 서식을 그대로 유지한다. 주변 문단과 사진 뒤 텍스트의 위치도 보존한다.
  - 로컬 합성 재현과 관련 회귀·브라우저에서 반복 적용·실행 취소/다시 실행·자동저장·저장/재열기·사진 뒤 입력을 확인한다. 기존 작업본/공개 반영 분리와 revision 충돌 방지를 유지한다.
  - 최종 소스를 기존 Git 자동 배포로 공개·관리자 양쪽에 반영하고 운영 편집기 적용을 읽기 확인한다. 실제 글에 재현 시험 입력을 하지 않는다.
  - 해당 글은 쓰기 직전 최신 작업본·반영본을 다시 조회하고 확정된 빈 사진 틀 2개와 중복된 바깥 틀 3개만 제거한다. 사용자 수정으로 구조가 달라졌으면 이전 숫자를 강제 적용하지 않고 현재 구조로 범위를 다시 확인한다. 가장 안쪽 사진·캡션과 사진 밖 본문·기존 서식을 보존해 작업본으로만 저장하고 공개 반영하지 않는다.
  - 사용자 후속 요청에 따라 수정 후 다른 글의 같은 문제를 전수조사한다. 이번 작업 중 운영 DB/API에서 조회한 현재 전체 목록과 모든 작업본·공개본을 기준으로 새 글·사용자 변경분을 포함하고 조회 시점·범위를 기록한다. 과거 이관 덤프를 대용하지 않는다.
  - 정상적인 중첩·사진 묶음·캡션 구조와 손상 징후를 구분해 확정 문제·검토 필요·접근 미확인으로 보고한다. 운영 데이터 접근이 막히면 확인하지 못한 범위와 이유를 명시하며 정상으로 판정하지 않는다. 다른 글의 일괄 수정은 이번 조사 승인에 포함하지 않는다.
  - 운영 고유 제목·본문·이미지 경로·개인정보를 문서·Git에 기록하지 않는다. 다른 글·원본 사진·공개 상태를 변경하거나 기존 별도 결함 6건으로 범위를 넓히지 않는다.
- **Evidence:**
  - 2026-09-22 사용자가 #599에서 사진 편집 적용을 반복하면 테두리가 늘고 텍스트가 안으로 들어간다고 알렸으며 텍스트를 직접 밖으로 꺼내 저장했다고 설명했다.
  - 이번 메인의 최신 운영 API·관리자 DOM 조회에서 마지막 저장 23:36:05 KST, 작업본과 공개 내용 동일, 빈 figure 2개·문단 3개·같은 형태의 figure 4겹 중첩(최심부 사진 1개·캡션 1개)을 확인했다. 이 구조는 조회 시점 기준이며 후속 쓰기 직전에 다시 확인한다.
  - 이어 사용자가 수정 후 다른 글도 전수조사하도록 요청했다. 현재 전체 목록·작업본·공개본의 읽기 조사로 범위를 넓히며 다른 글 일괄 수정으로 확대하지 않는다.
  - [완료 기록](history/사진편집_중첩방지_20260922_233908.md)에 재발 방지·확인 범위·한정 작업본 정리 결과를 구분해 남긴다.

  - **완료 근거(2026-09-23 마감):** 소스 `c8f867d`의 양쪽 Git 자동 배포와 운영 실행 소스 반영을 확인했다. 최신 운영 601글의 반영본 601개·별도 작업본 335개를 모두 조사했고 다른 실제 글의 같은 손상 징후는 없었다. 별도 비공개 합성 시험 글 #598의 중복 틀 1개는 조사만 했으며 수정하지 않았다. #599는 최신 revision 확인 후 2026-09-22 23:59:46.817 KST에 빈 틀 2개·중복 바깥 틀 3개만 제거해 작업본으로 저장했다. 관리자 재열기에서 사진·캡션 각 1개·문단 3개·중첩 0을 확인했고 반영본 revision 61과 내용은 그대로, 작업본 revision은 62다. 임의 공개 반영은 하지 않았다. 상세 조회 범위·로그인된 D1 읽기·원문 파일 보관 한계와 정규화 비교는 같은 완료 기록을 따른다.

### `DWNC-CORE-030` — 본문·사진 배치와 분리한 사진 설명 기본 가운데 정렬
- **Status:** `done`
- **Updated-at:** `2026-09-23`
- **Plans:** `PLAN-09`
- **Priority:** `P1`
- **Acceptance:**
  - 사진 설명에 별도의 정렬을 지정하지 않은 경우 기본 가운데 정렬로 표시한다. 본문 문단의 정렬과 사진 자체의 왼쪽·가운데·오른쪽 배치에서 기본 설명 정렬을 분리한다.
  - 사진 설명에 사용자가 명시적으로 지정한 기존 정렬은 보존하며 기본값으로 덮어쓰지 않는다. 원본 사진·캡션 내용과 내부 서식·주변 문단·기존 사진 그룹과 배치를 보존한다.
  - 편집기·미리보기·공개 화면에서 같은 기본값과 명시적 정렬 보존 규칙을 적용한다. 관련 회귀와 로컬 브라우저에서 본문/사진 배치의 영향을 받지 않는 기본 가운데 정렬과 기존 명시적 정렬 유지를 확인한다.
  - 최종 소스 빌드·기존 Git 자동 배포와 운영 읽기 확인을 마친다. 실제 글·작업본의 내용 쓰기나 공개 반영은 하지 않으며 저장된 콘텐츠를 일괄 재작성하지 않는다.
  - 기존 작업본/공개 분리·revision 충돌 처리·사진 편집 중첩 방지를 유지한다. 별도 기능이나 기존 결함 6건으로 범위를 넓히지 않는다.
- **Evidence:**
  - 2026-09-23 사용자가 사진 설명의 기본 정렬을 본문과 분리해 가운데로 표시하도록 요청했다. 기존에 명시한 설명 정렬은 유지하는 범위다.
  - [완료 기록](history/사진설명_기본가운데정렬_20260923_003128.md)에 최신 운영 읽기·구현·확인 범위를 구분해 기록한다. 이 등록 시점에 이번 요청의 운영 재조회·구현·배포 완료를 주장하지 않는다.

  - **완료 근거:** `8fb004a`의 공개·관리자 Cloudflare 자동 배포가 모두 성공했다. 운영 관리자 새로고침에서 기본 설명 CSS와 `captionFormatAlignment` 도구막대 처리 반영, 기존 명시적 가운데 정렬 보존을 확인했고 운영 공개 글에서 공통 가운데 정렬 스타일시트가 실제 로딩됨을 확인했다. 기본값·명시적 정렬의 편집기/미리보기/저장 재열기/합성 공개 확인과 최종 빌드 결과는 같은 완료 기록에 구분했다. 운영 콘텐츠 쓰기·공개 반영은 0이다.

### `DWNC-CORE-031` — 신규 사진 업로드 최적화와 브라우저 기술 설명 글 초안
- **Status:** `done`
- **Updated-at:** `2026-09-23`
- **Plans:** `PLAN-09`
- **Priority:** `P1`
- **Acceptance:**
  - 파일 첨부와 클립보드 붙여넣기의 공통 업로드 경로에서 정지 이미지의 실제 투명도를 확인하고 불투명 이미지를 JPEG로 변환·재압축한다. 최대 1,600만 화소 이내로 종횡비를 유지하며 구체적인 JPEG 품질은 실제 브라우저 시험 후 확정한다.
  - 실제 투명도를 사용하는 PNG는 유지하고 애니메이션을 정지 이미지로 훼손하지 않는다. PNG라는 이유만으로 불투명 화면 캡처·도표를 일괄 제외하지 않고 변환 후 가독성을 확인한다. 이미 작게 압축된 입력을 불필요하게 키우지 않는다.
  - 사용자의 로컬 원본 파일은 변경하지 않는다. 업로드할 결과 바이트를 기준으로 SHA-256·MIME·파일 크기를 일치시키고 원본과 변환본을 불필요하게 함께 업로드하지 않는다. 기존 소유권·새 key·작업본/공개 분리·revision 충돌 처리를 유지한다.
  - 평균 약 1MB는 검토 목표이며 파일당 크기나 모든 사진의 평균을 보장하지 않는다. 복잡한 사진의 더 큰 출력과 형식 보존 예외를 실제 시험 범위와 함께 설명한다.
  - 관련 회귀와 로컬 브라우저의 실제 canvas 변환으로 크기·해상도·투명도·애니메이션·첨부/클립보드 연결·출력 바이트를 확인한다. 최종 소스 빌드·기존 Git 자동 배포를 양쪽에 반영하고 운영을 읽기 확인한다. 기존 실제 글을 시험 수정하지 않는다.
  - 브라우저가 사진을 읽고 판별·변환한 뒤 서버로 보내는 기술 원리를 설명하는 블로그 글을 필요한 도식과 함께 완성해 관리자에 미공개 초안으로 저장한다. 실제 구현과 아직 확인하지 않은 부분을 구분하고 공개 발행하지 않는다. 이 글은 사용자가 요청한 콘텐츠이며 합성 시험 글과 구분한다.
  - 기존 글·저장 사진의 일괄 변환·삭제, Google 포토 설정, 요금제·권한·세션 정책 변경은 이번 실행 범위가 아니다. 비공개 초안 본문·개인 사진 정보는 공개 저장소에 기록하지 않으며 별도 결함 6건으로 범위를 넓히지 않는다.
- **Evidence:**
  - 2026-09-23 사용자가 앞선 투명도·압축 정책 검토에 대해 “그렇게 처리해줘”라고 실행을 요청했고 브라우저 기술 원리의 블로그 글 작성도 별도로 요청했다. 신규 업로드 기능 구현·운영 반영과 미공개 글 초안 작성의 두 목표로 진행한다.
  - 정책 검토 근거는 [사진 저장·조회 비용과 실측 비교](사진저장_조회비용_20260923_005427.md), 실행 결과는 [완료 기록](history/신규사진_업로드최적화와_기술글초안_20260923_014740.md)을 따른다. 정책 검토 당시 미구현 기록과 이번 실행 요청을 구분한다.

  - **완료 근거(2026-09-23 02:16 KST):** 기능 소스 `0963121`을 포함한 문서 후속 `1846ab5`의 공개·관리자 Cloudflare 배포가 모두 성공했다. 관리자 전체 새로고침·초안 재열기 후 사진 첨부 패널의 새 용량 절감·투명도/움직임 보존 안내를 확인했다. 관련 회귀·최종 빌드·로컬 실제 canvas 시험과 운영 화면 반영을 구분하며 새 압축 경로의 운영 실제 파일 업로드는 수행하지 않았다. 요청한 기술 글의 본문·도식·태그·요약·분류를 미공개 저장하고 새로고침 재열기·PC 미리보기를 확인했다. 도식은 배포 전 기존 업로드 경로로 첨부됐으며 글을 공개 발행하지 않았다. 기존 사진 변환·삭제와 사용자 기존 탭/글 수정은 없다. 배포 접수 지연과 최종 해소, 확인 범위는 같은 완료 기록에 보존했다.

### `DWNC-CORE-032` — HDR·색역을 보존한 신규·기존 첨부 사진 2560px 최적화
- **Status:** `done`
- **Updated-at:** `2026-09-23`
- **Plans:** `PLAN-09`
- **Priority:** `P1`
- **Acceptance:**
  - 파일 첨부와 클립보드 업로드는 긴 변 최대2560px·비율 유지·무확대, JPEG 품질80을 적용한다. 목표500KB~1MB는 강제 상한이 아니다.
  - HDR/P3 JPEG는 색역·HDR 지원과 원본 표시 밝기 범위를 유지한다. 처리 실패 시 조용히 SDR로 바꾸지 않는다. 실제 투명·애니메이션 등 기존 형식 예외와 작은 입력 팽창 방지를 유지한다.
  - 현재 운영 DB를 기준으로 새 블로그 직접 업로드 `media/native/` 사진을 native/legacy 미디어 양쪽에서 조사하고 같은 축소 기준을 적용한다. 과거 네이버·티스토리 이관 사진은 제외한다.
  - 반영본·미공개 작업본·대표사진 URL을 유지하고 내부 객체 매핑만 조건부 교체하여 사용자 수정·공개 상태·사진 설명·글 내용을 보존한다. 기존 immutable key는 덮어쓰지 않는다.
  - 관련 실행 시험과 실제 이미지 변환 결과를 확인하고 편집기 양쪽 배포·운영 반영, 기존 사진 변환 결과와 예외를 확인한다.
- **Evidence:**
  - 사용자 2026-09-23 요청: 긴 변2560 확정 후 새 블로그 생성 이후 기존 첨부 사진과 편집기 업로드 양쪽에 적용.
  - [원본 네 장의 HDR/P3 보존 축소 비교](history/HDR_P3보존_사진압축비교_20260923_172048.md).

  - 편집기 양쪽 운영 배포와 기존 HDR/P3 JPEG 11개 교체 완료. 시험용3개 제외 신규첨부20개 전체 긴 변2560px 이하, 글/작업본6개 revision 불변. 후속 명시 승인으로 R2 원본11개와 로컬 작업용 원본 사본15개를 삭제했고 저장소는 교체 전보다96720689bytes 감소했다. 임시 인증도 폐기했다. [완료 근거와 원본 정리](history/사진2560_HDR보존_적용_20260923_200845.md).
...
### `DWNC-CORE-033` — URL 단독 문단 자동 링크 카드와 기존 글 전수 적용
- **Status:** `done`
- **Updated-at:** `2026-09-25`
- **Plans:** `PLAN-09`
- **Priority:** `P1`
- **Acceptance:**
  - 직접 입력, 주소만 붙여넣기, 본문 전체 붙여넣기에 같은 기준을 적용하며 한 문단 전체가 웹주소 하나일 때만 링크 카드로 표시한다.
  - 문장 속 링크·여러 주소가 섞인 문단·코드·기존 카드와 무관한 서식은 보존한다. 비동기 조회가 뒤늦게 사용자 편집을 덮어쓰지 않으며 실행 취소/재실행을 지원한다.
  - 내부 링크는 공개 가능한 정보만 사용하고 외부 정보 조회 실패 시에도 본문 표시와 편집을 유지한다.
  - 이번 작업 시점 최신 운영 전체 목록의 반영본과 별도 작업본을 전수조사하고 같은 규칙을 기존 글에 적용한다. 과거 이관 덤프는 입력으로 사용하지 않는다.
  - 공개 상태와 미공개 변경을 보존하며, 콘텐츠 쓰기가 필요한 경우 revision 충돌 방지를 유지한다. 공통 표시 변환으로 충족하면 불필요한 본문 일괄 덮어쓰기는 하지 않는다.
  - 로컬 회귀 검사, Git 배포, 운영 반영 확인 및 전수조사 결과를 기록한다.
- **Evidence:**
  - 2026-09-25 사용자가 URL 단독 문단 기준 통일과 전체 글 조사·반영 및 적극적인 하위 에이전트 활용을 승인했다.
  - 최신603편·940본문 조사, 양쪽8354a19 배포 성공, 공개 대상18편 재조회 및 관리자 표시 확인. [완료 기록](history/단독URL_링크카드_20260925_005040.md).
